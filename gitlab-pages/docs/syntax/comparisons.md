...
You can use functions such as `Test.Compare.eq` to compare non-comparable types tests, but these functions are not available in smart contracts.
...
You can use functions such as `Test.Compare.eq` to compare lists, sets, and maps in tests, but not in smart contracts.
