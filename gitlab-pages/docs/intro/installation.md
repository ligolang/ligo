...
If you've [installed 🐳 Docker](https://docs.docker.com/install/), you can run the latest [LIGO release 0.50.0](./changelog.md):
...
Windows:
> ```dos
> docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.50.0`
> ```
> For convenience you can alias the above command
> ```dos
> doskey ligo=docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.50.0 $*
> ```
