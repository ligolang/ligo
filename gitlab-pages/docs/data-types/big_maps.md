...
By contrast, elements in big-maps are lazily deserialized, which means that when a contract reads elements from a big-map, it loads only the elements that it needs to access, instead of the entire big-map.
...
However, the way big-maps are stored and deserialized causes these limitations that maps don't have:
