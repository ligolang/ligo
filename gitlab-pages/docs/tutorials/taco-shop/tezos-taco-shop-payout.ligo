...
function buy_taco (const taco_kind_index : nat; var taco_shop_storage : taco_shop_storage) : return is {
    ...
    if Tezos.get_amount(unit) =/= current_purchase_price then
    ...
    const donationAmount : tez = Tezos.get_amount(unit) / 10n;

    const operations : list (operation) = list [
      Tezos.transaction (unit, Tezos.get_amount(unit) - donationAmount, receiver);
