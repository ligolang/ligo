...
If you've [installed 🐳 Docker](https://docs.docker.com/install/), you can run the latest [LIGO release 0.48.01](./changelog.md):

Linux or OSX:
> ```sh
> docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.48.01
> ```
> For convenience you can alias the above command
> ```sh
> alias ligo="docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.48.01"
> ```
...
Windows:
> ```dos
> docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.48.01`
> ```
> For convenience you can alias the above command
> ```dos
> doskey ligo=docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.48.01 $*
> ```
...
You can get the rolling release [here](https://gitlab.com/ligolang/ligo/-/jobs/2816932316/artifacts/raw/ligo), make it executable, and you are done!

```zsh
wget https://gitlab.com/ligolang/ligo/-/jobs/2816932316/artifacts/raw/ligo
chmod +x ./ligo
```
...
First, download [the package](https://gitlab.com/ligolang/ligo/-/jobs/2816932316/artifacts/raw/ligo.deb), and then install using: 
