...
Linux or OSX:
> ```sh
> docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.72.0
> ```
> For convenience you can alias the above command
> ```sh
> alias ligo='docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.72.0'
> ```
...
Windows:
> ```dos
> docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.72.0`
> ```
> For convenience you can alias the above command
> ```dos
> doskey ligo=docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:0.72.0 $*
> ```
...
You can download the installer from here hosted on our Gitlab (broken link, Windows versions are no longer shipped): `https://gitlab.com/ligolang/ligo/-/jobs/4884590379/artifacts/raw/ligo_installer.exe?inline=false` .
