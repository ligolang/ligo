...
First download [the package](https://gitlab.com/ligolang/ligo/-/jobs/11403991547/artifacts/raw/ligo.deb) and then install it with this command:
...
```bash
docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:1.11.5
```

Windows:

```bash
docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:1.11.5
```
...
```bash
alias ligo='docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:1.11.5'
```

Windows:

```dos
doskey ligo=docker run --rm -v "%CD%":/cd -w /cd ligolang/ligo:1.11.5 $*
```
...
You can download the current release [here](https://gitlab.com/ligolang/ligo/-/jobs/11403991547/artifacts/raw/ligo), make it executable, and you are done!
Optionally, you can put it somewhere in your `PATH` for easy access.

```bash
wget https://gitlab.com/ligolang/ligo/-/jobs/11403991547/artifacts/raw/ligo
chmod +x ./ligo
