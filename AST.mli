...
val instr_to_region : 'x instruction -> Region.t

val core_pattern_to_region : 'x core_pattern -> Region.t

val local_decl_to_region : 'x local_decl -> Region.t

type 'x visitor = {
  ass_instr       : 'x ass_instr -> unit;
  bind_to         : (Region.t * 'x variable) option -> unit;
  ...
  psome           : (Region.t * 'x core_pattern par) reg -> unit;
  ptuple          : ('x core_pattern, Region.t) nsepseq par -> unit;
  raw             : ('x core_pattern * Region.t * 'x pattern) par -> unit;
  record_type     : 'x record_type -> unit;
  sepseq          : 'x.string -> ('x -> unit) -> ('x, Region.t) sepseq -> unit;
  set             : ('x expr, Region.t) nsepseq braces -> unit;
  single_instr    : 'x single_instr -> unit;
  some_app        : (Region.t * 'x arguments) reg -> unit;
  step            : (Region.t * 'x expr) option -> unit;
  storage_decl    : 'x storage_decl reg -> unit;
  string          : string reg -> unit;
  sugar           : ('x core_pattern, Region.t) sepseq brackets -> unit;
  sum_type        : ('x variant, Region.t) nsepseq reg -> unit;
