...
let _S constant f new_cont =
  f (fun (acc : impl) -> new_cont RopeImplementation.(cat acc (of_string constant)))

let z = Other _S

let _d f new_cont =
  f (fun (acc : impl) (i : int) ->
      new_cont RopeImplementation.(cat acc (of_string (string_of_int i))))
...
let ( ~% ) : type a b. (((impl -> a) -> a) -> b) t -> b = function
  | S str -> _S str start
  | Other f -> f start

let ( % ) : type a b. a -> (a -> b) t -> b =
 fun fmt1 fmt2 ->
  match fmt2 with
  | S str -> _S str fmt1
  | Other f2 -> f2 fmt1

let ( #% ) fmt arg = (fmt finish) arg
