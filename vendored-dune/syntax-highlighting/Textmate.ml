...
module JSON = struct
  let highlight_to_textmate : string -> Core.highlight_name -> string =
   fun syntax -> function
  ...
  let rec capture syntax (i : int * Core.highlight_name) =
    ( string_of_int (fst i)
    , `Assoc [ "name", `String (highlight_to_textmate syntax (snd i)) ] )
  ...
  and pattern_kind (t : Core.t) (name : string) (syntax : string) = function
    | Core.Begin_end { meta_name; begin_; end_; patterns } ->
      ...
      in
      let begin_ =
        String.concat spaces (List.map (fun f -> (fst f).Core.textmate) begin_)
      in
      ...
      let patterns =
        ...
        (if List.mem name special
        then Fun.id
        else ( @ ) (List.map (fun c -> Core.Name_ref c) comments))
          patterns
      in
    ...
    | Match { match_; match_name } ->
      ...
      | Some s -> [ "name", `String (highlight_to_textmate syntax s) ]
      | None -> [])
      @ [ "match", `String match_; "captures", captures syntax captures_ ]

  and repository (t : Core.t) syntax r : Yojson.Safe.t =
    `Assoc
      (List.map
         (fun (i : Core.pattern) -> i.name, `Assoc (pattern_kind t i.name syntax i.kind))
         r)

  and language_features : Core.language_features -> Yojson.Safe.t =
   fun l ->
    `Assoc
  ...
  and comments : string Core.language_features_comments -> (string * Yojson.Safe.t) list =
   fun c ->
    [ ( "comments"
      , `Assoc
          [ "lineComment", `String c.line_comment
          ; ( "blockComment"
            , `List [ `String (fst c.block_comment); `String (snd c.block_comment) ] )
          ] )
    ]

  and brackets : string -> (string * string) list -> (string * Yojson.Safe.t) list =
   fun name l ->
    [ name, `List (List.map (fun b -> `List [ `String (fst b); `String (snd b) ]) l) ]

  and to_yojson : string -> Core.t -> (Yojson.Safe.t * Yojson.Safe.t, Core.error) result =
   fun syntax s ->
    ok
    @@ ( `Assoc
...
module Validate = struct
  let rec check_reference repository = function
    | Core.Self_ref | Core.String_ref -> ok true
    | Core.Name_ref r ->
      let open Helpers in
      ...
      if exists_repo || exists_builtin
      then ok true
      else error (Core.Referenced_rule_does_not_exist r)
  ...
  and pattern_kind name repository = function
    | Core.Begin_end { begin_; end_; patterns; _ } ->
      ...
      if String.trim begin_ = ""
      then error (Core.Begin_cant_be_empty name)
      else if String.trim end_ = ""
      then error (Core.End_cant_be_empty name)
      else (
        let patterns =
          List.fold_left
            (fun a c -> if is_error a then a else check_reference repository c)
            (ok true)
            patterns
        in
  ...
  let syntax (s : Core.t) =
    ...
    let patterns =
      List.fold_left
        (fun a (c : Core.pattern) ->
          if is_error a then a else pattern_kind c.name repository c.kind)
        (ok true)
        repository
    in
    ...
    let patterns =
      List.fold_left
        (fun a (c : Core.reference) ->
          if is_error a then a else check_reference repository c)
        curr
        s.syntax_patterns
    in
    fold ~ok ~error patterns
...
let add_comments s =
  ...
  { s with
    syntax_patterns =
      Core.Name_ref "block_comment" :: Core.Name_ref "line_comment" :: s.syntax_patterns
  ; repository =
      { name = "block_comment"
      ; kind =
          Begin_end
...
let to_jsons : string -> Core.t -> (Yojson.Safe.t * Yojson.Safe.t, Core.error) result =
 fun syntax s ->
