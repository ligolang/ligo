...
let cli_error msg = red_exit (Printf.sprintf "Command-line error: %s\n" msg)
...
let check_cli () =
  ...
  | `SyntaxError msg | `FileNotFound msg -> cli_error msg
  | `Help buf | `CLI buf -> print_and_quit (Buffer.contents buf)
  | `Version ver -> print_and_quit (ver ^ "\n")
  | `Conflict (o1, o2) -> cli_error (Printf.sprintf "Choose either %s or %s." o1 o2)
  | `Done ->
    (match CLI.Preproc_CLI.extension with
    | Some ext when ext <> File.extension ->
      let msg = Printf.sprintf "Expected extension %s." File.extension in
      cli_error msg
    | _ -> ())
...
type window = < last_token : token option ; current_token : token (* Including EOF *) >
...
let preprocess () : (Lexing.lexbuf, unit) Stdlib.result option =
  ...
  then (
    ...
    | Error (Some buffer, Region.{ value; _ }) ->
      if CLI.Preproc_CLI.show_pp
      then Printf.printf "%s%!" (Buffer.contents buffer)
      else ();
      print_in_red value;
      Some (Stdlib.Error ())
