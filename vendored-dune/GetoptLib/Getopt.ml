...
| _ -> fail (sprintf "Option %s does not accept argument" opt)

let extract_handle opt = function
  _, _, Some handle, _ -> handle
 | _ -> fail (sprintf "Option %s must have an argument" opt)

let parse opts others args first last =
  let find_long opt =
    try List.find (fun (_,l,_,_) -> opt = l) opts with
      Not_found -> fail (sprintf "Unknown option --%s" opt) in

  let find_short opt =
    try List.find (fun (l,_,_,_) -> opt = l) opts with
      Not_found -> fail (sprintf "Unknown option -%c" opt) in
          ...
          if l >= 2 && s.[1] = '-' then
            if l = 2 then skip (succ no) (* -- *)
              ...
              match index_option s '=' with
                Some i -> (* long option with argument *)
                  ...
                  let handle =
                    extract_arg_handle ("--" ^ opt) (find_long opt) in
                  handle arg;
                  aux (succ no)
            ...
            | (_, _, _, Some handle) as o ->
                ...
                if l > 2 then (* immediate argument *)
                  ...
                  (* no argument *)
                  let handle = extract_handle s o
                  in (handle (); aux (succ no))
            | _ -> fail "Parse error"
        else (others s; aux (succ no))
  in aux first
...
let only_once var msg =
  Some (fun x -> if !var = "" then var := x else fail msg)
