...
let dummy_tezos_context ?tezos_context () =
  Option.value
    ~default:(Lwt.map (fun env -> Ok env.Init_proto_alpha.tezos_context) @@ dummy_environment ())
...
let ty_eq (type a b)
    ?tezos_context
    (a:(a,_) ty) (b:(b,_) ty) : ((_, _) eq, Tezos_base.TzPervasives.tztrace) Lwt_result.t
  =
  let open Lwt_result.Let_syntax in
  let%bind tezos_context = dummy_tezos_context ?tezos_context () in
...
let parse_michelson_fail (type aft aftr)
  ...
  dummy_tezos_context ?tezos_context () >>=? fun tezos_context ->
...
let interpret ?options (instr:('a, 'b, 'c, 'd) kdescr) bef : (_*_) tzresult Lwt.t  =
  ...
  let%bind {
    ...
    now ;
    level ;
  } = Option.value ~default:(make_options ()) (Option.map Lwt.return options) in
...
let typecheck_contract ?environment contract =
  let (>>=) = Lwt_syntax.(let*) in
  let (>>=?) = Lwt_result_syntax.(let*) in
  dummy_environment_result ?environment () >>=?? fun env ->
...
let typecheck_view ?environment input_ty output_ty storage_ty view_code =
  ...
  parse_michelson_ty ~tezos_context:env.tezos_context storage_ty >>= fun storage_ty ->
  match storage_ty with
  | Ok (Ex_ty storage_ty) ->
    Script_ir_translator.parse_view ~elab_conf env.tezos_context storage_ty view >>= fun x -> (
    match x with
...
let pack (data_ty: ('a,_) ty) (data: 'a) : bytes tzresult Lwt.t =
  Lwt.bind (dummy_environment ()) (fun env ->
  pack_data env.tezos_context data_ty data >>=?? fun (packed,_) -> Lwt_result.return packed)
...
let to_bytes = fun michelson ->
  Lwt.map
    ...
    (Lwt.map (X_error_monad.force_ok ~msg:"Internal error: could not serialize Michelson")
      (prims_of_strings michelson))

let to_hex = fun michelson ->
  Lwt.map (Hex.of_bytes) (to_bytes michelson)
