...
(** Like [Map.find], but also returns the key along with the data.
    ...
    It might seem redundant to give a key to a map only to return the same one, however,
    it's possible that the original key and the provided key compare equally but contain
    different fields, such as is the case for variables or labels, that ignore locations
    during comparisons. *)
val find_kv
  : 'key 'data 'cmp.
