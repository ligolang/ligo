...
let convert : type output . display_format:(output display_format) -> no_colour:bool -> displayable -> output =
...
let to_json : displayable -> json = convert ~display_format:Json ~no_colour:true (* TODO NP : Right ? *)

let bind_format :
  'value format -> 'error format -> ('value,'error) result format =
  fun value_format error_format ->
    let pp ~display_format ~no_colour f a = match a with
    | Error (e) -> error_format.pp ~display_format ~no_colour f e
    | Ok (v) -> value_format.pp ~display_format ~no_colour f v in
...
let map (fmt : 'b format) ~(f : 'a -> 'b) : 'a format =
  { pp = (fun ~display_format ~no_colour ppf x -> fmt.pp ~display_format ~no_colour ppf (f x));
