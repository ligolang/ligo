  ...
  outputs =
    ...
    in utils.lib.eachSystem haskellSystems (system:
      let
        ...
        grammars = import ./../debugger/ligo-debugger/grammar { pkgs = haskellPkgs; };

        weeder-hacks = import haskell-nix-weeder { pkgs = pkgs-legacy; };

        ligo-debugger-stack-project = pkgs:
          (pkgs.haskell-nix.callPackage ./../debugger/ligo-debugger { inherit grammars; inherit weeder-hacks; });
