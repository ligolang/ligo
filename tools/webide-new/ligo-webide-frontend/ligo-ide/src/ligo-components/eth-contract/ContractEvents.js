...
class ContractEvents extends PureComponent {
  ...
  getEventLogs = async (selectedEvent) => {
    ...
    const { maxGap } = this.props.contract;
    ...
    if (typeof rangeTo !== "number") {
      ...
      if (rangeFrom >= rangeTo) {
        notification.error(
          "Invalid Range",
          "The value of <b>from</b> must be smaller than the value of <b>to</b>."
        );
        return;
      }
      if (rangeTo - rangeFrom > 50 * maxGap - 1) {
      ...
      <div className="fixed-table">
        <div>
          <table className="table table-sm table-hover table-striped">
              ...
              <tr>
                ...
                {columns.map(({ name, type }) => (
                  <th key={`table-col-${name}`} scope="col">
                    <div>
                      <div style={{ lineHeight: "1.1rem" }}>{name}</div>
                      <div
                        style={{
                          lineHeight: "0.8rem",
                          fontVariant: "none",
            ...
            <tbody>{this.renderTableBody(this.state.logs, columns)}</tbody>
          </table>
        </div>
      </div>
    );
          ...
          if (type === "address") {
            content = (
              <a
                ...
                onClick={() => history.push(`/account/${content}`)}
                className="text-body"
              >
                {content}
              </a>
  ...
  render() {
    ...
    return (
      <div className="d-flex flex-column align-items-stretch h-100">
        <div className="d-flex border-bottom-1">{this.renderEventSelector()}</div>
                ...
                <InputGroup size="sm">
                  ...
                  <InputGroupAddon addonType="append">
                    <Button
                      color="secondary"
                      size="sm"
                      onClick={() => this.setState({ rangeFrom: "", rangeTo: "" })}
                    >
                      Clear
                    </Button>
                  </InputGroupAddon>
