...
export default class AbiStorageModal extends PureComponent {
  ...
  viewAbi = async data => {
    ...
    notification.success("ABI Updated", "The ABI record is updated in the storage.");
    this.refresh();
  };
  ...
  newAbi = async (inputName, inputCodeHash) => {
    ...
    notification.success("ABI Added", "A new ABI record is added to the storage.");
    this.refresh();
  };
      ...
      <tr key={`abi-${codeHash}`} className="hover-flex">
        ...
        <td align="right">
          <div className="d-flex flex-row justify-content-end hover-show">
            ...
            <DeleteButton onConfirm={() => this.deleteAbi(codeHash)} />
          </div>
        </td>
      </tr>
    );
