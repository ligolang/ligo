...
import { Button, Modal, DebouncedFormGroup } from "~/base-components/ui-components";

import keypairManager, { KeypairInputSelector } from "~/base-components/keypair";
import { DockerImageInputSelector } from "~/base-components/docker";
...
export default class CreateInstanceButton extends PureComponent {
  ...
  onCreateInstance = async () => {
    ...
    if (
      ...
      notification.error(
        "Failed",
        `You have an instance named ${this.state.name}, please use a different name.`
      );
      return;
    }
