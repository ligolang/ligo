...
import { Card } from "~/base-components/ui-components";
import redux from "~/base-components/redux";
import { DockerImageButton } from "~/base-components/docker";
...
export default class InstanceList extends PureComponent {
  ...
  onNodeLifecycle = (name, lifecycle, params) => {
    ...
    if (lifecycle === "stopped") {
      ...
      notification.info(
        `${process.env.CHAIN_NAME} Instance Stopped`,
        `<b>${name}</b> stops to run.`
      );
    } else if (lifecycle === "started") {
      redux.dispatch("UPDATE_UI_STATE", { localNetwork: runningState });
      notification.success(
        `${process.env.CHAIN_NAME} Instance Started`,
        `<b>${name}</b> is running now.`
      );
    }
  };
      ...
      return (
        <tr>
          <td align="middle" colSpan={6}>
            (No {process.env.CHAIN_EXECUTABLE_NAME_IN_LABEL} instance)
          </td>
        </tr>
      );
    }
  ...
  render() {
    return (
      <>
        ...
        >
          <div className="flex-grow-1 overflow-auto">{this.renderTable()}</div>
        </Card>
        <InstanceConfigModal ref={this.configModal} onRefresh={this.refreshInstances} />
      </>
    );
  }
}
