...
import { Button, UncontrolledTooltip } from "~/base-components/ui-components";
...
export default class CompilerButton extends PureComponent {
  ...
  render() {
    const { className, size = "sm", color = "default", readOnly = false } = this.props;
    ...
    if (this.state.building) {
      icon = (
        <>
          <span key="building-icon" className="hover-hide">
            <i className="fas fa-spinner fa-spin" />
          </span>
          <span key="stop-build-icon" className="hover-show">
