...
import { TableCard, Badge } from "~/base-components/ui-components";
...
export default class AccountTransactions extends PureComponent {
  ...
  loadMore = async () => {
    ...
    if (Array.isArray(txs)) {
      this.setState({
        ...
        hasMore: total
          ? this.state.txs.length + txs.length < total
          : txs.length === this.state.size,
      });
    } else {
      this.setState({ error: txs });
    }
  ...
  renderTableBody = () => {
    ...
    if (this.state.loading) {
      ...
      rows.push(
        <tr key="txs-loadmore" className="bg-transparent">
          <td align="middle" colSpan={8}>
            <span className="btn btn-sm btn-secondary" onClick={this.loadMore}>
              Load More
            </span>
          </td>
        </tr>
  ...
  render() {
    const { TransactionHeader } = this.props;
    if (this.state.hide) {
      return null;
    }
      ...
      <TableCard
        title={
          <div className="d-flex flex-row align-items-end">
            ...
            <Badge pill className="ml-1 mb-1">
              {total}
            </Badge>
          </div>
        }
