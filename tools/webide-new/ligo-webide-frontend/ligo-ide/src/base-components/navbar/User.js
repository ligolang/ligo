...
class User extends Component {
  ...
  renderDropdownMenus = profile => {
    const links = [
      { name: 'Install', link: 'https://ligolang.org/docs/intro/installation/', icon: 'fa-tools', key: 'install' },
      { name: 'Docs', link: 'https://ligolang.org/docs/intro/introduction/', icon: 'fa-book', key: 'docs' },
      { name: 'Tutorials', link: 'https://ligolang.org/docs/tutorials/getting-started/getting-started/', icon: 'fa-graduation-cap', key: 'tutorials' },
      { name: 'Blog', link: 'https://forum.tezosagora.org/tag/ligo', icon: 'fa-comment', key: 'blog' },
      { name: 'Ask Questions', link: 'https://ligolang.org/contact/', icon: 'fa-question', key: 'ask-questions' },
      { name: 'Changelog', link: 'https://ligolang.org/docs/next/intro/changelog/', icon: 'fa-list-ul', key: 'changelog' },
      { name: 'Cheat Sheet', link: 'https://ligolang.org/docs/api/cheat-sheet', icon: 'fa-hand-holding', key: 'cheat-sheet' }
    ]
      ...
      ...links.map(link => {
        return (
          <DropdownItem key={link.key} onClick={() => window.open(link.link, '_blank')}>
            <i className={`fas ${link.icon} w-3 mr-2`} />{link.name}
          </DropdownItem>
        )
      })
    ]
