...
export default class QueueButton extends PureComponent {
  ...
  renderDropdownItems = (pending, txs, QueueItem) => {
    ...
    if (!txsItems.length) {
      txsItems.push(
        <DropdownItem disabled key="disable">
          (None)
        </DropdownItem>
      );
    }
    ...
    const allTransactions = (
      <DropdownItem onClick={this.openAllTransactionsModal}>
        <div className="d-inline-block w-3">
  ...
  render() {
    ...
    if (BaseQueueManager.pending.length) {
      icon = (
        <div key="icon-pending" className="d-inline-block w-3 mr-1">
          <i className="fas fa-spin fa-spinner" />
        </div>
      );
    } else {
      icon = (
        <div key="icon-no-pending" className="d-inline-block w-3 mr-1">
    ...
    return (
      <>
        ...
        <Modal ref={this.txModal} title={title || "Transaction"} textCancel="Close">
          <TransactionDetails
            {...otherProps}
            tx={tx}
        ...
        <Modal ref={this.allTxsModal} title="All Transactions" />
      </>
    );
  }
}
