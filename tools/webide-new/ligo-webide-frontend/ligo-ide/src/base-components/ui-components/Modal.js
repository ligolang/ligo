...
export default class BaseModal extends PureComponent {
  ...
  renderConfirmButton = ({
    ...
    return (
      <Button key="modal-confirm-btn-pending" color={colorConfirm} className="ml-2" disabled>
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <ModalBody
          className={classnames("d-flex flex-column", !overflow && !scrollable && "overflow-auto")}
        >
          {children}
          {errorComponent}
        </ModalBody>
        <ModalFooter style={{ justifyContent: "space-between" }}>
          ...
          <div>
            ...
            {(!noCancel || !footerCancelIcon) && textCancel && (
              <Button hidden={footerCancelIcon} color={colorCancel} onClick={this.toggle}>
                {textCancel}
              </Button>
            )}
