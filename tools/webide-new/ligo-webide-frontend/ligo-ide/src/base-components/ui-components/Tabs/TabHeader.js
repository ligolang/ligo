...
class TabHeaderItem extends PureComponent {
  ...
  renderCloseBtn() {
    ...
    if (!onCloseTab || showClose) {
      return <span className="nav-item-close-times empty" />;
    }
    ...
    return connectDragSource(
      connectDropTarget(
        ...
        >
          <div
            className={classnames(
              "btn d-flex flex-row align-items-center border-0 w-100",
              size && `btn-${size}`
            )}
          >
            <div className="nav-item-content d-flex flex-row">
              <div className="nav-item-text">
                <span key={tab.key}>{tabText}</span>
              </div>
            </div>
            {this.renderCloseBtn()}
          </div>
      ...
      <div className="nav-top-bar overflow-hidden">
        <DndProvider backend={HTML5Backend}>
          <div className="nav-wrap w-100 d-flex">
            <ul
              onWheel={this.handleWheelThrottled.bind(this)}
              ref={this.tabsRef}
              className={classnames("d-flex nav nav-tabs ", className)}
            >
              {tabs.map((tab, index) => {
            ...
            {ToolButtons.map((btn, index) => {
              ...
              return (
                <div key={id} onClick={btn.onClick} title={btn.tooltip}>
                  <div
                    id={id}
                    className={classnames("btn btn-transparent rounded-0", size && `btn-${size}`)}
                  >
          ...
          <Menu animation={false} id="tab-context-menu">
            {this.treeNodeContextMenu?.map((item, index) =>
              item ? (
                <Item key={item.text} onClick={() => item.onClick(this.state.selectNode)}>
                  {item.text}
                </Item>
              ) : (
