...
import { Button, ListGroupItem } from "~/base-components/ui-components";
...
export default class ListItemDocker extends PureComponent {
  ...
  refresh = async () => {
    ...
    if (await dockerChannel.check()) {
      ...
      if (process.env.OS_IS_LINUX) {
        notification.error(
          "Fail to run docker",
          "Make sure the non-root user has privileges to run docker"
        );
      }
    } else {
  ...
  startDocker = async () => {
    ...
    if (success) {
      ...
      notification.error(
        "Fail to run docker",
        "Something went wrong when starting Docker, please try again."
      );
    }
  ...
  renderButton = () => {
    switch (this.state.docker) {
      ...
      case "NONE":
        return (
          <Button color="primary" onClick={this.installDocker}>
            Install
          </Button>
        );
      case "INSTALLED":
        if (process.env.OS_IS_LINUX) {
          return (
            <Button
              color="primary"
              onClick={() =>
                fileOps.openLink("https://docs.docker.com/engine/install/linux-postinstall")
              }
            >
              Need Privileges
            </Button>
        ...
        return (
          <Button color="primary" onClick={this.startDocker}>
            Start Docker
          </Button>
        );
