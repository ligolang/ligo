...
import { Modal, Button } from "~/base-components/ui-components";
...
export default class Markdown extends Component {
  ...
  renderSwitchToEditorBtn = () => {
    return (
      <Button color="primary" size="sm" className="ml-2" onClick={this.onEditButton}>
        {this.display ? (
          <span key="mode-edit">
            <i className="fas fa-pencil-alt" />
          </span>
        ) : (
          <span key="mode-render">
  ...
  async togglePublic() {
    let saved = true;
    for (const key in modelSessionManager.sessions) {
  ...
  async confirmTogglePublic() {
    ...
    const isPublic = await modelSessionManager.projectManager.togglePublic(
    ...
    notification.success(
      "Change Visibility Successful",
      `This project is now <b>${isPublic ? "public" : "private"}</b> ${
        isPublic ? "and visible to anyone with the link." : "and only visible to yourself."
      }`
    );
  }
  ...
  renderTogglePublicButton = () => {
    ...
    return (
      <Button
        ...
        style={
          this.state.togglePublicToggling
      ...
      >
        {this.state.togglePublicToggling && (
          <span key="mode-toggling">
            <i className="fas fa-spinner fa-pulse" /> Toggling
          </span>
        )}
  ...
  render() {
    ...
    let { value } = this.props.modelSession;
          ...
          <div className="container">
              ...
              components={{
                a: (props) => {
                  ...
                  if (props.href.startsWith("http://") || props.href.startsWith("https://")) {
                    return (
                      <span className="link" onClick={() => this.openLink(props.href)}>
                        {props.children}
                      </span>
                    );
                  }
                  return (
                    <span className="link" onClick={() => this.openFile(props.href)}>
                      {props.children}
                    </span>
                  );
                },
            ...
            <Modal
              ...
              children={
                this.state.togglePublicSaved ? (
                  <span>
                    Are you sure to change this project to
                    <b>{this.state.isPublic ? " private" : " public"}</b>?
                    {this.state.isPublic
                      ? " Private projects are only visible to your self."
                      : " Public projects are visible to anyone with the link."}
                  </span>
                ) : (
