...
const FileTree = forwardRef(({ projectManager, onSelect, initialPath, contextMenu }, ref) => {
  ...
  isBlankAreaRightClick && (
    treeNodeContextMenu = treeNodeContextMenu
      .filter(item => item && (item.text === 'New File' || item.text === 'New Folder'))
  )
  ...
  const removePersist = () => {
    if (persistDOM) {
      persistDOM.className = persistDOM.className.toString()
        .replace(' persist--active', '')
      setPersist(null)
    }
  }
  ...
  const disableChildren = (node, needDisable) => {
    node.className = needDisable ? node.className + ' node--disable' : node.className.replaceAll('node--disable', '')
    if (node.children) {
