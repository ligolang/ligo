...
instance (HasLigoClient m, Log m) => HasScopeForest Standard m where
  scopeForest projDir reportProgress pc = do
    fbForest <- scopeForest @Fallback projDir reportProgress pc
    ...
    where
      tryMergeWithFromCompiler fbForest = do
        lgForest <- scopeForest @FromCompiler projDir reportProgress pc
