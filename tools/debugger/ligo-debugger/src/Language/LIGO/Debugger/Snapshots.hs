...
runInstrCollect = \case
  instr@(T.ConcreteMeta (embeddedMeta :: EmbeddedLigoMeta) inner) -> \oldStack -> do
    ...
    preExecutedStage embeddedMeta stack inner
    newStack <- runInstrImpl runInstrCollect instr stack
    postExecutedStage embeddedMeta stack newStack inner
      ...
      returnStack <-
        case (instr, oldStack, newStack) of
          ...
          (EXEC{}, _ :& StkEl oldLam :& _, StkEl lam :& stkEls) -> do
            ...
            forM_ lmVariables \_ -> do
              logMessage [int||Attempting to remove stack frame|]
              csStackFramesL %= \case
                _ :| (x : xs) -> x :| xs
                other -> other
