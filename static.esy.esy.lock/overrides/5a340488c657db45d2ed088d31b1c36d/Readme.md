...
`esy-libtool` is sample package, [`hello`](https://github.com/esy/test-hello-c), packaged for [`esy`](https://esy.sh/).
...
## How to use `esy-libtool`?

`esy-libtool` can be used from both NPM and directly from Github.

### From NPM 

`esy-libtool` is deployed on NPM can be found
[here](https://www.npmjs.com/package/TODO).

You can simply run `esy add esy-libtool` to install it, or specify it in
...
{
  "dependencies": {
    "esy-libtool": "esy-packages/esy-libtool"
  }
}
```
...
```diff
  "dependencies": {
+   "esy-libtool": "esy-packages/esy-libtool#<commit hash>"
  }
```
