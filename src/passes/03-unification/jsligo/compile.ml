...
let compile_path (t : T.simple_path reg) : O.Mod_variable.t Nonempty_list.t =
...
let rec type_expr' (type_expr : Eq'.ty_expr) : Folding'.ty_expr =
  ...
  | T_path simple_path ->
    let T.{ path; selected } = simple_path.value in
    (match path with
...
let compile_val_binding (node : T.val_binding reg)
  ...
  let type_params, rhs_type =
    match rhs_type with
    | None -> None, None
    | Some (T.T_for_all { value = vars, type_expr; _ }) ->
  ...
  | D_Import s ->
    let import =
      ...
      | ImportFrom { value = { imported; file_path; _ }; _ } ->
        ...
        in
        let imported = Nonempty_list.map ~f:TODO.esc_var imported in
        let module_str = file_path#payload in
        O.Import.Import_selected { imported; module_str }
    in
