...
type typer_error = [
  ...
  | `Typer_could_not_remove of Ast_typed.type_constraint_simpl
    ...
    | `Typer_label_do_not_match (la , lb , loc ) ->
      Format.fprintf f
        "@[<hv>%a@.Labels do not match: Expected %a but got %a .@]"
          Snippet.pp loc
          Ast_typed.PP.label la
          Ast_typed.PP.label lb
...
let rec error_jsonformat : typer_error -> Yojson.Safe.t = fun a ->
  ...
  | `Typer_label_do_not_match (la , lb , loc ) ->
    ...
    let content = `Assoc [
      ...
      ("expected", Ast_typed.Yojson.label la) ;
      ("actual", Ast_typed.Yojson.label lb) ;
    ] in
    json_error ~stage ~content
  | `Typer_pattern_do_not_match loc ->
  ...
  | `Typer_could_not_remove constraint_ ->
    let message = `String (Format.asprintf "Heuristic requested removal of a constraint that cannot be removed: %a" Ast_typed.PP.type_constraint_simpl constraint_) in
    let content = `Assoc [
      ("message", message);
    ] in
    json_error ~stage ~content
