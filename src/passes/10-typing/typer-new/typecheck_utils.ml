...
module All_vars(Plugins : Plugins) = struct
  ...
  let all_vars (state : ('typer_error, 'plugin_states) Typesystem.Solver_types.typer_state) =
    ...
    let aux1 : type_variable * constructor_or_row  -> type_variable list = fun (tv, cor) ->
      ...
      | `Row r -> tv :: r.tv :: LMap.to_list r.tv_map in
...
let other_check all_constraints assignments =
  ...
  let rec toposort : (type_variable PolySet.t) ->
    ...
    = fun already_seen unification_var repr find_assignment ->
      ...
      if Set.mem repr_unification_var already_seen then 
        ...
        | Some c_or_r ->
          let args : type_variable list = match c_or_r with `Constructor k -> k.tv_list | `Row r -> LMap.to_list r.tv_map in
  ...
  type canon_constructor_or_row = [
    | `Constructor of < c_tag : Ast_typed.constant_tag ; tv_list : type_variable list >
    | `Row         of < r_tag : Ast_typed.row_tag      ; tv_map  : type_variable LMap.t >
  ]
  ...
  let hashcons =
    fun repr find_assignment (canon, first) unification_var ->
      ...
      let map_args f cor : canon_constructor_or_row = match cor with
        | `Constructor k -> `Constructor (object method c_tag = k.c_tag method tv_list = List.map f k.tv_list end)
        | `Row r -> `Row (object method r_tag = r.r_tag method tv_map = LMap.map (fun v -> f v) r.tv_map end)
      in
  ...
  let hashcons : (type_variable list) ->
                 ...
                 (type_variable, type_variable) PolyMap.t result =
    fun unification_vars repr find_assignment ->
     (* do the loop here *)
     ...
     let compare_canon_constructor_or_row (a : canon_constructor_or_row) (b : canon_constructor_or_row) =
      ...
      | (`Constructor k1, `Constructor k2) ->
        Ast_typed.Compare.constant_tag k1#c_tag k2#c_tag
        <? fun () ->
        ...
        List.compare ~compare:Var.compare k1#tv_list k2#tv_list
      | (`Row         r1, `Row         r2) ->
        Ast_typed.Compare.row_tag r1#r_tag r2#r_tag
        <? fun () ->
        (* We know that variables are unique for their content here,
           so a simple Var.compare ensures equality of the whole
          type expression pointed to by that var *)
        LMap.compare Ast_typed.Compare.type_variable r1#tv_map r2#tv_map
