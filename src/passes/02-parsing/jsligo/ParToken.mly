...
%token                  <string Wrap.wrap> BlockCom "<block_comment>" [@recover.expr mk "<invalid-block-comment>" $loc]
%token                  <string Wrap.wrap> LineCom  "<line_comment>"  [@recover.expr mk "<invalid-line-comment>" $loc]
...
%token <lexeme Wrap.wrap> REM     "%" [@recover.expr mk "%" $loc]
...
%token <lexeme Wrap.wrap> BOOL_NOT "!"  [@recover.expr mk "!" $loc]
...
%token <lexeme Wrap.wrap> PLUS_EQ  "+=" [@recover.expr mk "+=" $loc]
%token <lexeme Wrap.wrap> MINUS_EQ "-=" [@recover.expr mk "-=" $loc]
%token <lexeme Wrap.wrap> MULT_EQ  "*=" [@recover.expr mk "*=" $loc]
%token <lexeme Wrap.wrap> REM_EQ   "%=" [@recover.expr mk "%=" $loc]
%token <lexeme Wrap.wrap> DIV_EQ   "/=" [@recover.expr mk "/=" $loc]
...
%token <lexeme Wrap.wrap> Default  "default" [@recover.expr mk "default" $loc]
%token <lexeme Wrap.wrap> Else     "else"    [@recover.expr mk "else" $loc]
%token <lexeme Wrap.wrap> Export   "export"  [@recover.expr mk "export" $loc]
%token <lexeme Wrap.wrap> For      "for"     [@recover.expr mk "for" $loc]
%token <lexeme Wrap.wrap> If       "if"      [@recover.expr mk "if" $loc]
%token <lexeme Wrap.wrap> Import   "import"  [@recover.expr mk "import" $loc]
%token <lexeme Wrap.wrap> Let      "let"     [@recover.expr mk "let" $loc]
%token <lexeme Wrap.wrap> Of       "of"      [@recover.expr mk "of" $loc]
%token <lexeme Wrap.wrap> Return   "return"  [@recover.expr mk "return" $loc]
%token <lexeme Wrap.wrap> Break    "break"   [@recover.expr mk "break" $loc]
...
%token <lexeme Wrap.wrap> As        "as"        [@recover.expr mk "as" $loc]
%token <lexeme Wrap.wrap> Namespace "namespace" [@recover.expr mk "namespace" $loc]
%token <lexeme Wrap.wrap> Type      "type"      [@recover.expr mk "type" $loc]

(* Virtual tokens *)

%token <lexeme Wrap.wrap> ZWSP [@recover.expr mk "" $loc]
