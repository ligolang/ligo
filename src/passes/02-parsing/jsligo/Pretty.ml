...
let print_attribute state ?(dec_in_com = false) (node : Attr.t wrap) =
  ...
  let thread = match val_opt with
               ...
               | _ -> thread in
  let thread =
    if dec_in_com then string "// " ^^ thread ^^ hardline else thread
  in group (print_comments node#comments ^/^ thread)

let print_attributes state ?(dec_in_com = false) thread attributes =
  ...
  match drop_comment_attr attributes with
    []    -> thread
  | attrs -> let print = print_attribute ~dec_in_com state in
...
and top_let_to_const_in_value_decl
  ?(stdlib=false) (node : value_decl Region.reg) =
  let Region.{region; value} = node in
...
and print_S_Attr
  ?(stdlib=false) ?(decl_kind=true) ?(dec_in_com=true) ?(classes=false)
  ?(static=false) ?(let_to_const=false)
  state (node : attribute * statement) =
  let attributes, stmt = unroll_S_Attr node in
  let thread = print_statement ~decl_kind ~dec_in_com ~classes
                               ~static ~let_to_const state stmt
...
and print_block
  ?(stdlib=false) ?(decl_kind=true) ?(dec_in_com=true) ?(classes=false)
  ?(let_to_const=false) ?(static=false)
  state (node : statements braces) =
  let print = print_statements ~decl_kind ~dec_in_com ~classes
                               ~static ~let_to_const state in
  print_braces ~force_hardline:true state print node

and print_statements
  ?(stdlib=false) ?(decl_kind=true) ?(dec_in_com=true) ?(classes=false)
  ?(static=false) ?(let_to_const=false)
  state (node : statements) =
  let print = print_statement_semi ~decl_kind ~dec_in_com ~classes
                                   ~static ~let_to_const state in
  print_ne_list (break 1) print node

and print_statement_semi
  ?(stdlib=false) ?(decl_kind=true) ?(dec_in_com=true) ?(classes=false)
  ?(static=false) ?(let_to_const=false)
...
and print_ImportAlias ?(stdlib=false) state (node : import_alias reg) =
...
and print_M_Alias ?(stdlib=false) (node : namespace_name) = token node
...
and print_D_Interface ?(stdlib=false) state (node : interface_decl reg) =
...
and print_I_Type
  ?(stdlib=false) state (node : intf_type reg) =
...
and print_I_Const
  ?(stdlib=false) ?(decl_kind=true) state (node : intf_const reg) =
  let {kwd_const; const_name; const_optional; const_type} = node.value in
  let prefix = if decl_kind then token kwd_const ^^ space else empty in
  let thread =
...
and print_D_Namespace
  prefix ?(stdlib=false) ?(classes=false) state (node : namespace_decl reg) =
  ...
  let is_nested : bool =
    ...
    |> List.map ~f:fst
    |> is_nested in
  let translate_to_a_class : bool =
    classes && not is_nested && has_entrypoints
  in
  (* To translate or not *)
  if translate_to_a_class then
    ...
    let filter_export stmt stmt_semi acc =
      let _, decl = stmt.value in
      filter_decl decl stmt_semi acc in
    let filter_stmt (stmt, _ as stmt_semi) acc =
      let fun_and_val, others = acc in
      match stmt with
        S_Decl decl -> filter_decl decl stmt_semi acc
      | S_Export stmt -> filter_export stmt stmt_semi acc
      | S_Attr attr_stmt ->
    ...
    let fun_and_val_stmts, other_stmts =
    ...
    let const = List.map ~f:fun_to_val fun_and_val_stmts
    in
    ...
    in
    (* Printing the implementation of interfaces *)
    let class_doc =
...
and print_D_Type ?(stdlib=false) state (node : type_decl reg) =
...
and print_val_binding ?(stdlib=false) state (node : val_binding reg) =
...
and print_S_Export
  ?(stdlib=false) ?(decl_kind=true) ?(classes=false) ?(let_to_const=false)
  ?(static=false) state (node : export_stmt reg) =
...
and print_S_Return ?(stdlib=false) state (node : return_stmt reg) =
...
and print_S_Switch ?(stdlib=false) state (node : switch_stmt reg) =
...
and print_E_Function ?(stdlib=false) state (node : function_expr reg) =
...
and print_E_Match ?(stdlib=false) state (node : match_expr reg) =
...
and has_array_all_clauses
  ?(stdlib=false) (node : match_clause reg Ne.t) : bool =
  let hd :: _ = node in
...
and print_DefaultClause ?(stdlib=false) state (node : match_default reg) =
  print_match_default state node

and print_match_default ?(stdlib=false) state (node : match_default reg) =
...
and print_selection ?(stdlib=false) state = function
...
and print_P_CtorApp ?(stdlib=false) state (node : pattern variant_kind) =
...
and print_legacy_variant :
 'a. (state -> 'a -> document) -> state -> 'a legacy_variant reg -> document =
 fun print state node ->
  let {attributes; tuple} = node.value in
  let tuple = print_legacy_variant_args print state tuple in
  print_attributes state tuple attributes

and print_legacy_variant_args :
 'a. (state -> 'a -> document) -> state -> 'a legacy_variant_args brackets -> document =
 fun print state ->
  print_brackets state (fun inside ->
    ...
    let rec separate_map = function
      []            -> empty
    | (sep', x)::xs -> token ~sep:(break 1) sep' ^^ print state x ^^ separate_map xs
