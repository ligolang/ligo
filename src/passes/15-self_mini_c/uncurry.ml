...
let rec usage_in_expr (f : Value_var.t) (expr : expression) : usage =
  ...
  (* interesting cases: *)
  | E_variable x ->
    if Value_var.equal f x (* if we got here, f wasn't only used in applications *)
    then Other
    else Unused
  | E_application _ ->
  ...
  | E_literal _ -> Unused
  | E_closure { binder; body } -> self_binder [ binder ] body
  ...
  | E_iterator (_, ((v1, _), e1), e2) -> usages [ self_binder [ v1 ] e1; self e2 ]
  | E_fold (((v1, _), e1), e2, e3) -> usages [ self_binder [ v1 ] e1; self e2; self e3 ]
  | E_fold_right (((v1, _), e1), (e2, _), e3) ->
    usages [ self_binder [ v1 ] e1; self e2; self e3 ]
  | E_if_bool (e1, e2, e3) -> usages [ self e1; self e2; self e3 ]
  ...
  | E_let_in (e1, _, ((v2, _), e2)) -> usages [ self e1; self_binder [ v2 ] e2 ]
  | E_tuple exprs -> usages (List.map ~f:self exprs)
  | E_let_tuple (e1, (vars, e2)) ->
    usages [ self e1; self_binder (List.map ~f:fst vars) e2 ]
  | E_proj (e, _i, _n) -> self e
  | E_update (expr, _i, update, _n) -> usages [ self expr; self update ]
  | E_raw_michelson _ -> Unused
  | E_inline_michelson (_, arguments) -> usages (List.map ~f:self arguments)
  | E_global_constant (_hash, args) -> usages (List.map ~f:self args)
  | E_create_contract (_p, _s, _code, args) -> usages (List.map ~f:self args)
  | E_let_mut_in (e1, ((x, _), e2)) -> usages [ self e1; self_binder [ x ] e2 ]
  | E_deref x -> if Value_var.equal f x then Other else Unused
  | E_assign (x, e) -> if Value_var.equal f x then Other else self e
  | E_for (start, final, incr, ((x, _), body)) ->
    usages [ self start; self final; self incr; self_binder [ x ] body ]
  | E_for_each (coll, _coll_type, (xs, body)) ->
    usages [ self coll; self_binder (List.map ~f:fst xs) body ]
...
let uncurry_rhs (depth : int) (expr : expression) : expression =
  ...
  ; type_expression =
      { expr.type_expression with
        type_content = T_function (comb_type arg_types, ret_type)
      }
  }


let rec uncurry_in_expression (f : Value_var.t) (depth : int) (expr : expression)
    : expression
  =
...
let uncurry_expression : expression -> expression =
  map_expression (fun e ->
      ...
      | E_let_in (e1, inline, ((v, _t), e2)) ->
        let return e1 e2 =
          { e with content = E_let_in (e1, inline, ((v, e1.type_expression), e2)) }
        in
