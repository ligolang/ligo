...
let typer_1 : Location.t -> string -> (Location.t -> type_expression -> (type_expression, typer_error) result) -> typer = fun l s f lst _ ->
  match lst with
  | [ a ] -> (
      let%bind tv' = f l a in
      ok (tv')
    )
...
let typer_2 : Location.t -> string -> (Location.t -> type_expression -> type_expression -> (type_expression, typer_error) result) -> typer = fun l s f lst _ ->
  match lst with
  | [ a ; b ] -> (
      let%bind tv' = f l a b in
      ok (tv')
    )
...
let typer_3 : Location.t -> string -> (Location.t -> type_expression -> type_expression -> type_expression -> (type_expression, typer_error) result) -> typer = fun l s f lst _ ->
  match lst with
  | [ a ; b ; c ] -> (
      let%bind tv' = f l a b c in
      ok (tv')
    )
  | _ -> fail @@ wrong_param_number l s 3 lst

let typer_4 : Location.t -> string -> (Location.t -> type_expression -> type_expression -> type_expression -> type_expression -> (type_expression , typer_error) result) -> typer = fun l s f lst _ ->
  match lst with
  | [ a ; b ; c ; d ] -> (
      let%bind tv' = f l a b c d in
      ok (tv')
    )
  | _ -> fail @@ wrong_param_number l s 4 lst

let typer_5 : Location.t -> string -> (Location.t -> type_expression -> type_expression -> type_expression -> type_expression -> type_expression -> (type_expression, typer_error) result) -> typer = fun l s f lst _ ->
  match lst with
  | [ a ; b ; c ; d ; e ] -> (
      let%bind tv' = f l a b c d e in
      ok (tv')
    )
  | _ -> fail @@ wrong_param_number l s 5 lst

let typer_6 : Location.t -> string
  -> (Location.t -> type_expression -> type_expression -> type_expression -> type_expression -> type_expression -> type_expression -> (type_expression, typer_error) result) -> typer = fun l s f lst _ ->
  match lst with
  | [ a ; b ; c ; d ; e ; f_ ] -> (
      let%bind tv' = f l a b c d e f_ in
      ok (tv')
    )
  | _ -> fail @@ wrong_param_number l s 6 lst

let constant' loc name cst = typer_0 loc name (fun _ _ -> ok cst)
