...
module Make (Comments    : Comments.S)
  ...
  struct
    ...
    let read: Lexing.lexbuf -> ('token, message) Stdlib.result =
      fun _ ->
        match !tokens with
        | hd :: tl ->
          (match !window with
          | None ->
            window := Some (
              object
                ...
                method current_token = hd
              end);
          | Some win ->
            window := Some (
              object
                method last_token = Some win#current_token;
                method current_token = hd
              end));
          tokens := tl;
          Ok hd
        | [] ->
          Ok (Token.eof Region.ghost)

    let get_win: (unit -> window option) = fun () -> !window

    let scan: Lexing.lexbuf -> (token, message) Stdlib.result = fun lexbuf ->
      match !reader with
        Some reader ->
          reader lexbuf
      | None ->
