...
let rec type_module ~raise ~middle_end_options ~init_context ~backend_options (p:I.module_) : O.module_ =
  let aux (c, acc:(context * O.declaration Location.wrap list)) (d:I.declaration Location.wrap) =
    let (c, d') = type_declaration' ~raise ~middle_end_options ~backend_options c d in
    (c, d' :: acc)
  in
...
and type_declaration' : raise: typer_error raise -> backend_options:Compiler_options.backend -> middle_end_options: Compiler_options.middle_end -> context -> I.declaration Location.wrap -> context * O.declaration Location.wrap =
fun ~raise ~backend_options ~middle_end_options c d ->
...
match Location.unwrap d with
  ...
  | Declaration_constant { binder = { ascr = None ; var ; attributes=_ } ; attr  ; expr} -> (
    ...
    let expr =
      ...
      type_expression' ~middle_end_options ~backend_options c expr in
  ...
  | Declaration_constant { binder = { ascr = Some tv ; var ; attributes=_ } ; attr ; expr } ->
    ...
    let expr =
      ...
      type_expression' ~middle_end_options ~backend_options ~tv_opt:tv env expr in
    let rec aux t = function
      | [] -> t
      | (abs_var :: abs_vars) -> t_for_all abs_var () (aux t abs_vars) in
    let type_expression = aux expr.type_expression (List.rev av) in
  ...
  | Declaration_module {module_binder;module_; module_attr = {public}} -> (
    let module_ = type_module ~raise ~middle_end_options ~backend_options ~init_context:c module_ in
...
and type_expression ~raise ~middle_end_options ~backend_options : ?env:Environment.t -> ?tv_opt:O.type_expression -> I.expression -> O.expression
  = fun ?env ?tv_opt e ->
    let c   = Context.init ?env () in
    let res = type_expression' ~raise ~middle_end_options ~backend_options c ?tv_opt e in
    res

and type_expression' ~raise ~middle_end_options ~backend_options ?(args = []) ?last : context -> ?tv_opt:O.type_expression -> I.expression -> O.expression = fun context ?tv_opt e ->
  let return expr tv =
    let () =
      match tv_opt with
      | None -> ()
  ...
  | E_constructor {constructor = Label s as constructor ; element} when String.equal s "M_left" || String.equal s "M_right" -> (
    ...
    let expr' = type_expression' ~raise ~middle_end_options ~backend_options context element in
    ( match t.type_content with
      | T_sum c ->
  ...
  (* Record *)
  | E_record m ->
      let m' = O.LMap.map (type_expression' ~raise ~middle_end_options ~backend_options context) m in
  ...
  | E_record_update {record; path; update} ->
    let record = type_expression' ~raise ~middle_end_options ~backend_options context record in
    let update = type_expression' ~raise ~middle_end_options ~backend_options context update in
     ...
     let lambda =
         ...
         | Some tv' ->
            ...
            | Some { type1 = input_type ; type2 = _ } ->
               ...
               | Some _ -> lambda in
     let (lambda,lambda_type) = type_lambda ~raise ~middle_end_options ~backend_options context lambda in
  ...
  | I.E_type_abstraction {type_binder;result} ->
    ...
    let result  = type_expression' ~raise ~middle_end_options ~backend_options context result in
  ...
  | E_constant {cons_name=( C_LIST_FOLD | C_MAP_FOLD | C_SET_FOLD | C_FOLD) as opname ;
      ...
      let (v_col , v_initr ) = Pair.map ~f:(type_expression' ~raise ~middle_end_options ~backend_options context) (collect , init_record ) in
      ...
      let body = type_expression' ~raise ~middle_end_options ~backend_options ?tv_opt:(Some tv_out) e' result in
  ...
  | E_constant {cons_name= C_LOOP_LEFT as opname;
      ...
      let tv_lst = List.map ~f:get_type lst' in
      let (opname',tv) = type_constant ~raise ~middle_end_options ~backend_options opname e.location tv_lst tv_opt in
      return (E_constant {cons_name=opname';arguments=lst'}) tv
  | E_constant {cons_name=C_CREATE_CONTRACT as cons_name;arguments} ->
      let lst' = List.map ~f:(type_expression' ~raise ~middle_end_options ~backend_options context) arguments in
      let () = match lst' with
      ...
      let tv_lst = List.map ~f:get_type lst' in
      let (name', tv) =
        type_constant ~raise ~middle_end_options ~backend_options cons_name e.location tv_lst tv_opt in
      return (E_constant {cons_name=name';arguments=lst'}) tv
  | E_constant {cons_name=C_SET_ADD|C_CONS as cst;arguments=[key;set]} ->
      let key' =  type_expression' ~raise ~middle_end_options ~backend_options context key in
      ...
      in
      let set' =  type_expression' ~raise ~middle_end_options ~backend_options context ~tv_opt:tv set in
      let tv_set = get_type set' in
      let tv_lst = [tv_key;tv_set] in
      let (name', tv) = type_constant ~raise ~middle_end_options ~backend_options cst e.location tv_lst tv_opt in
      return (E_constant {cons_name=name';arguments=[key';set']}) tv
  | E_constant {cons_name=C_MAP_ADD as cst; arguments=[key;value;map]} ->
      let key' = type_expression' ~raise ~middle_end_options ~backend_options context key in
      let val' = type_expression' ~raise ~middle_end_options ~backend_options context value in
      let tv_key = get_type key' in
      ...
      in
      let map' =  type_expression' ~raise ~middle_end_options ~backend_options context ~tv_opt:tv map in
  ...
  | E_constant {cons_name = C_POLYMORPHIC_ADD;arguments} ->
      ...
      let (name', tv) =
        type_constant ~raise ~middle_end_options ~backend_options cst e.location tv_lst tv_opt in
      return (E_constant {cons_name=name';arguments=lst'}) tv
  ...
  | E_application { lamb = ilamb ; args=_} ->
     ...
     let args = List.map ~f:(type_expression' ~raise ~backend_options ~middle_end_options context) args in
     let lamb = type_expression' ~raise ~backend_options ~middle_end_options ~args ?last:tv_opt context lamb in
  ...
  | E_matching {matchee;cases} -> (
    let matchee' = type_expression' ~raise ~middle_end_options ~backend_options context matchee in
    ...
    | E_variable matcheevar ->
      let case_exp = Pattern_matching.compile_matching ~raise ~err_loc:e.location ~type_f:(type_expression' ~middle_end_options ~backend_options ~args:[] ?last:None) ~body_t:(tv_opt) matcheevar eqs in
  ...
  | E_let_in {let_binder = {var ; ascr = None ; attributes=_} ; rhs ; let_result; attr } ->
     ...
     let rhs = type_expression' ~raise ~backend_options ~middle_end_options context rhs in
     let binder = var in
     let rec aux t = function
       | [] -> t
       | (abs_var :: abs_vars) -> t_for_all abs_var () (aux t abs_vars) in
     ...
     let let_result = type_expression' ~raise ~backend_options ~middle_end_options e' let_result in
  ...
  | E_let_in {let_binder = {var ; ascr = Some tv ; attributes=_} ; rhs ; let_result; attr } ->
    ...
    let rhs = type_expression' ~raise ~backend_options ~middle_end_options ~tv_opt:tv context rhs in
    let rec aux t = function
      | [] -> t
      | (abs_var :: abs_vars) -> t_for_all abs_var () (aux t abs_vars) in
    let type_expression = aux rhs.type_expression (List.rev av) in
    ...
    let let_result = type_expression' ~raise ~backend_options ~middle_end_options context let_result in
    return (E_let_in {let_binder = binder; rhs; let_result; attr }) let_result.type_expression
  ...
  | E_mod_in {module_binder; rhs; let_result} ->
    let rhs = type_module ~raise ~backend_options ~middle_end_options ~init_context:context rhs in
  ...
  | E_mod_alias {alias; binders; result} ->
    ...
    let result = type_expression' ~raise ~middle_end_options ~backend_options e' result in
  ...
  | E_raw_code {language;code} ->
    ...
    let code = type_expression' ~raise ~middle_end_options ~backend_options context code in
  ...
  | E_recursive {fun_name; fun_type; lambda} ->
    ...
    let (lambda,lambda_type) = type_lambda ~raise ~middle_end_options ~backend_options e' lambda in
  ...
  | E_ascription {anno_expr; type_annotation} ->
    ...
    let expr' = type_expression' ~raise ~backend_options ~middle_end_options ~last:tv ~tv_opt:tv context anno_expr in
    let type_annotation =
  ...
  | E_module_accessor {module_name; element} ->
    ...
    in
    let element = type_expression' ~raise ~middle_end_options  ~backend_options ~args ?last ?tv_opt module_env element in
    return (E_module_accessor {module_name; element}) element.type_expression


and type_lambda ~raise ~middle_end_options ~backend_options e {
      binder ;
      output_type ;
      result ;
    } =
      ...
      let body = type_expression' ~raise ~middle_end_options ~backend_options ?tv_opt:output_type e' result in
      let output_type = body.type_expression in
      (({binder; result=body}:O.lambda),(t_arrow input_type output_type ()))



and type_constant ~raise ~middle_end_options ~backend_options (name:I.constant') (loc:Location.t) (lst:O.type_expression list) (tv_opt:O.type_expression option) : O.constant' * O.type_expression =
  let typer = Constant_typers.constant_typers ~raise ~middle_end_options ~backend_options loc name in
  let tv = typer lst tv_opt in
  (name, tv)

let type_program ~raise ~middle_end_options ~backend_options ?env m = type_module ~raise ~middle_end_options ~init_context:(Context.init ?env ()) ~backend_options m
let type_declaration ~raise ~middle_end_options ~backend_options ?env d = snd @@ type_declaration' ~raise ~middle_end_options (Context.init ?env ()) ~backend_options d
