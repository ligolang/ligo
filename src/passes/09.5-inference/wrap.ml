...
let rec type_expression_to_type_value : T.type_expression -> O.type_value = fun te ->
  ...
  | T_app {type_operator;arguments} -> (
    ...
    T.(let f t = Var.is_name type_operator (Stage_common.Constant.to_string t) in
      match (f , arguments) with
      | ( f , [] ) when (f unit) -> Some (C_unit , [])
      | ( f , [] ) when (f never) -> Some (C_never , [])
      | ( f , [] ) when (f string) -> Some (C_string , [])
      | ( f , [] ) when (f nat) -> Some (C_nat , [])
      | ( f , [] ) when (f tez) -> Some (C_mutez , [])
      | ( f , [] ) when (f timestamp) -> Some (C_timestamp , [])
      | ( f , [] ) when (f int) -> Some (C_int , [])
      | ( f , [] ) when (f address) -> Some (C_address , [])
      | ( f , [] ) when (f bytes) -> Some (C_bytes , [])
      | ( f , [] ) when (f key_hash) -> Some (C_key_hash , [])
      | ( f , [] ) when (f key) -> Some (C_key , [])
      | ( f , [] ) when (f signature) -> Some (C_signature , [])
      | ( f , [] ) when (f operation) -> Some (C_operation , [])
      | ( f , [] ) when (f chain_id) -> Some (C_chain_id , [])
      | ( f , [o] ) when (f option) -> Some (C_option, [o])
      | ( f , [p] ) when (f set) -> Some (C_set, [p])
      | ( f , [ k ; v ]) when (f map) -> Some (C_map, [k;v])
      | ( f , [ k ; v ]) when (f big_map) -> Some (C_big_map, [k;v])
      | ( f , [ k ; v ]) when (f map_or_big_map) -> Some (C_map, [k;v])
      | ( f , [l] ) when (f list) -> Some (C_list, [l])
      | ( f , [c] ) when (f contract) -> Some (C_contract, [c])
      | ( _ , _ ) -> None
      )
    in
