   ...
   fun e ->
    ...
    | E_annot (ae, ty) as e ->
      ...
      | E_literal (Literal_string lit)
        when is_ty_var ty (to_string Key_hash) && is_standard (Ligo_string.get_type lit)
        ->
        ...
        e_key_hash ~loc s
      | E_literal (Literal_string lit)
        when is_ty_var ty (to_string Signature) && is_standard (Ligo_string.get_type lit)
        ->
        let s = Ligo_string.extract lit in
        let _ =
      ...
      | E_literal (Literal_string lit)
        when is_ty_var ty (to_string Key) && is_standard (Ligo_string.get_type lit) ->
        let s = Ligo_string.extract lit in
        let _ =
          trace_option
            ~raise
        ...
        e_key ~loc s
      | E_literal (Literal_string lit)
        when is_ty_var ty (to_string Timestamp) && is_standard (Ligo_string.get_type lit)
        ->
      ...
      | E_literal (Literal_string lit)
        when is_ty_var ty (to_string Chain_id) && is_standard (Ligo_string.get_type lit)
        -> e_chain_id ~loc (Ligo_string.extract lit)
      | E_literal (Literal_string lit)
        when is_ty_var ty (to_string Address) && is_standard (Ligo_string.get_type lit) ->
      ...
      | E_literal (Literal_bytes lit) when is_ty_var ty (to_string Chest_key) ->
        e_chest_key ~loc lit
      | E_literal (Literal_string lit)
        when is_ty_var ty (to_string Bytes) && is_standard (Ligo_string.get_type lit) ->
        let str = Ligo_string.extract lit in
