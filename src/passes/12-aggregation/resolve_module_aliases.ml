...
module Aliases = struct
  ...
  let rec pp ppf {inside} =
    Format.fprintf ppf "%a" (PP_helpers.list_sep_d (fun ppf (k,(t,v)) -> Format.fprintf ppf "%a => (%a,%a)" Module_var.pp k pp t PP_helpers.((list_sep_d Module_var.pp)) v )) @@ MMap.to_kv_list inside
  ...
  let diff_path path module_path =
    ...
    let rec aux path module_path = match (path,module_path) with
...
let rec expression path : Aliases.t -> AST.expression -> AST.expression = fun aliases e ->
  ...
  | E_module_accessor {module_path;element} ->
    let _,module_path = List.fold ~init:(aliases,path) module_path ~f:(
      fun (a,_module_path) mvar ->
...
and compile_declaration path aliases (d : AST.declaration) : Aliases.t * AST.declaration option =
  ...
  | D_module {module_binder;module_;module_attr} ->
      let mod_aliases,path,module_  = compile_module_expr module_binder (path) aliases module_ in
      let aliases = Aliases.push aliases module_binder mod_aliases path in
      match module_ with None -> return_n aliases
      | Some module_ -> return_s aliases @@ AST.D_module {module_binder;module_;module_attr}

and compile_declaration_list path aliases (program : AST.program) : Aliases.t * AST.program =
...
and compile_module_expr mvar path : Aliases.t -> AST.module_expr -> Aliases.t * Module_var.t list * AST.module_expr option =
  fun aliases mexpr ->
    ...
    | M_module_path (hd,tl) -> (
    let aliases,module_path = List.fold ~init:(aliases,path) (hd::tl) ~f:(
      fun (a,_module_path) mvar ->
        let aliases,path' = Aliases.get a mvar in
        aliases, path') in
