...
let swap_declaration : Scope.swapper -> declaration -> declaration = fun swaper decl ->
  ...
  | D_irrefutable_match  { pattern ; expr ; attr } ->
    let pattern = swap_pattern swaper pattern in
    let expr = swap_expression swaper expr in
    Location.wrap ~loc:(Location.get_location decl) @@ D_irrefutable_match  { pattern ; expr ; attr }
...
let declaration : Scope.t -> declaration -> Scope.t * declaration = fun scope decl ->
  ...
  | D_irrefutable_match  { pattern ; expr ; attr  } ->
    let scope,pattern = pattern_new scope pattern in
    let _,expr = expression scope expr in
    scope, Location.wrap ~loc:(Location.get_location decl) @@ D_irrefutable_match  { pattern ; expr ; attr }
