...
type abs_error = [
  | `Concrete_reasonligo_wrong_pattern of string * Raw.pattern
  | `Concrete_reasonligo_unsupported_let_in of Raw.pattern list
  | `Concrete_reasonligo_unknown_predefined_type of Raw.type_constr
  | `Concrete_reasonligo_untyped_fun_param of Raw.variable
  ...
  | `Concrete_reasonligo_abstraction_tracer of Raw.expr * abs_error
  | `Concrete_reasonligo_abstraction_type_tracer of Raw.type_expr * abs_error
  | `Concrete_reasonligo_bad_deconstruction of Raw.expr
  | `Concrete_reasonligo_only_constructors of Raw.pattern
  | `Concrete_reasonligo_unsupported_sugared_lists of Raw.wild
  | `Concrete_reasonligo_corner_case of string
  | `Concrete_reasonligo_unknown_built_in of string
  ...
  | `Concrete_reasonligo_program_tracer of Raw.declaration list * abs_error
  ]

let wrong_pattern expected actual = `Concrete_reasonligo_wrong_pattern (expected,actual)
let unsupported_let_in_function patterns = `Concrete_reasonligo_unsupported_let_in patterns
let unknown_predefined_type name = `Concrete_reasonligo_unknown_predefined_type name
let untyped_fun_param var = `Concrete_reasonligo_untyped_fun_param var
let untyped_recursive_fun reg = `Concrete_reasonligo_recursive_fun reg
let unsupported_tuple_pattern p = `Concrete_reasonligo_unsupported_tuple_pattern p
let unsupported_cst_constr p = `Concrete_reasonligo_unsupported_constant_constr p
let unsupported_non_var_pattern p = `Concrete_reasonligo_unsupported_non_var_pattern p
...
let abstracting_expr_tracer t err = `Concrete_reasonligo_abstraction_tracer (t,err)
let abstracting_type_expr_tracer t err = `Concrete_reasonligo_abstraction_type_tracer (t,err)
let bad_deconstruction t = `Concrete_reasonligo_bad_deconstruction t
let only_constructors p = `Concrete_reasonligo_only_constructors p
let unsupported_sugared_lists region = `Concrete_reasonligo_unsupported_sugared_lists region
let corner_case desc = `Concrete_reasonligo_corner_case desc
let unknown_built_in name = `Concrete_reasonligo_unknown_built_in name
...
let program_tracer decl err = `Concrete_reasonligo_program_tracer (decl,err)
...
let rec error_ppformat : display_format:string display_format ->
  ...
  | Human_readable | Dev -> (
    match a with
    | `Concrete_reasonligo_wrong_pattern (expected_name,actual) ->
      Format.fprintf f
        "@[<hv>%a@Wrong pattern: expected %s got %s@]"
        Location.pp_lift (Raw.pattern_to_region actual)
        (Cst_reasonligo.ParserLog.pattern_to_string ~offsets:true ~mode:`Point actual)
        expected_name
    | `Concrete_reasonligo_unsupported_let_in expr ->
      Format.fprintf f
    ...
    | `Concrete_reasonligo_untyped_fun_param variable ->
      Format.fprintf f
    ...
    | `Concrete_reasonligo_unsupported_tuple_pattern p ->
      Format.fprintf f
        "@[<hv>%a@The following tuple pattern is not supported yet:@\"%s\"@]"
        Location.pp_lift (Raw.pattern_to_region p)
        (Cst_reasonligo.ParserLog.pattern_to_string ~offsets:true ~mode:`Point p)
    | `Concrete_reasonligo_unsupported_constant_constr p ->
      Format.fprintf f
        "@[<hv>%a@Constant constructors are not supported yet@]"
        Location.pp_lift (Raw.pattern_to_region p)
    | `Concrete_reasonligo_unsupported_non_var_pattern p ->
      Format.fprintf f
    ...
    | `Concrete_reasonligo_abstraction_tracer (expr,err) ->
      Format.fprintf f
        "@[<hv>%a@Abstracting expression:@\"%s\"@%a@]"
        Location.pp_lift (Raw.expr_to_region expr)
        (Cst_reasonligo.ParserLog.expr_to_string ~offsets:true ~mode:`Point expr)
        (error_ppformat ~display_format) err
    | `Concrete_reasonligo_abstraction_type_tracer (te,err) ->
      Format.fprintf f
        "@[<hv>%a@Abstracting type expression:@\"%s\"@%a@]"
        Location.pp_lift (Raw.type_expr_to_region te)
        (Cst_reasonligo.ParserLog.type_expr_to_string ~offsets:true ~mode:`Point te)
        (error_ppformat ~display_format) err
    | `Concrete_reasonligo_bad_deconstruction expr ->
      Format.fprintf f
        "@[<hv>%a@Bad tuple deconstruction \"%s\"@]"
        Location.pp_lift (Raw.expr_to_region expr)
        (Cst_reasonligo.ParserLog.expr_to_string ~offsets:true ~mode:`Point expr)
    | `Concrete_reasonligo_only_constructors p ->
      Format.fprintf f
        "@[<hv>%a@Currently, only constructors are supported in patterns@]"
        Location.pp_lift (Raw.pattern_to_region p)
    | `Concrete_reasonligo_unsupported_sugared_lists wild ->
      Format.fprintf f
        "@[<hv>%a@Currently, only empty lists and constructors (::) are supported in patterns@]"
        Location.pp_lift wild
    | `Concrete_reasonligo_corner_case desc ->
      Format.fprintf f "Corner case: %s" desc
    | `Concrete_reasonligo_unknown_built_in bi ->
    ...
    | `Concrete_reasonligo_michelson_type_wrong (texpr,name) ->
      Format.fprintf f
        "@[<hv>%a@Argument %s of %s must be a string singleton@]"
          ...
          (Cst_reasonligo.ParserLog.type_expr_to_string ~offsets:true ~mode:`Point texpr)
          name
    | `Concrete_reasonligo_michelson_type_wrong_arity (loc,name) ->
      Format.fprintf f
        ...
        Location.pp loc
        name
    | `Concrete_reasonligo_program_tracer (decl,err) ->
      Format.fprintf f
...
let rec error_jsonformat : abs_error -> Yojson.t = fun a ->
  ...
  | `Concrete_reasonligo_wrong_pattern (expected_name,actual) ->
    ...
    let actual = (Cst_reasonligo.ParserLog.pattern_to_string ~offsets:true ~mode:`Point actual) in
    let content = `Assoc [
      ("message", message);
      ("location", `String loc);
  ...
  | `Concrete_reasonligo_unsupported_tuple_pattern p ->
    ...
    let pattern = Cst_reasonligo.ParserLog.pattern_to_string ~offsets:true ~mode:`Point p in
    let content = `Assoc [
      ("message", message );
      ("location", `String loc);
  ...
  | `Concrete_reasonligo_abstraction_tracer (expr,err) ->
    ...
    let expr = Cst_reasonligo.ParserLog.expr_to_string ~offsets:true ~mode:`Point expr in
  ...
  | `Concrete_reasonligo_abstraction_type_tracer (te,err) ->
    ...
    let loc = Format.asprintf "%a" Location.pp_lift (Raw.type_expr_to_region te) in
    let expr = Cst_reasonligo.ParserLog.type_expr_to_string ~offsets:true ~mode:`Point te in
    let children = error_jsonformat err in
    let content = `Assoc [
      ("message", message );
      ("location", `String loc);
