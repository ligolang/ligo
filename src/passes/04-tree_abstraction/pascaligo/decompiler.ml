...
let rec decompile_type_expr : dialect -> AST.type_expression -> _ result = fun dialect te ->
  ...
  | T_tuple tuple ->
    let* tuple = bind_map_list (decompile_type_expr dialect) tuple in
  ...
  | T_app {type_operator; arguments} ->
    ...
    let* lst = bind_map_list (decompile_type_expr dialect) arguments in
...
and decompile_pattern : dialect -> AST.type_expression AST.pattern -> (CST.pattern,_) result =
  fun dialect pattern ->
    ...
    | AST.P_list pl -> (
      ...
      match pl with
      | AST.Cons (pa,pb) ->
        let* plst = bind_map_list (decompile_pattern dialect) [pa;pb] in
      ...
      | AST.List [] ->
        ret (PNil ghost)
      | AST.List plst ->
        let* plst = bind_map_list (decompile_pattern dialect) plst in
    ...
    | AST.P_tuple lst ->
      let* pl = bind_map_list (decompile_pattern dialect) lst in
...
and decompile_to_tuple_expr : dialect -> AST.expression list -> (CST.tuple_expr,_) result = fun dialect expr ->
  let* tuple_expr = bind_map_list (decompile_expression ~dialect) expr in
...
and decompile_eos : dialect -> eos -> AST.expression -> ((CST.statement List.Ne.t option)* (CST.expr option), _) result = fun dialect output expr ->
  ...
  | E_application {lamb;args} ->
    ...
    let* args = bind (decompile_to_tuple_expr dialect) @@ get_e_tuple args in
    (match output with
      Expression ->
  ...
  | E_constructor {constructor;element} ->
    ...
    let* element = bind (decompile_to_tuple_expr dialect) @@ get_e_tuple element in
  ...
  | E_matching {matchee; cases} -> (
    ...
    match output with
    | Expression ->
      let* cases = bind_map_list (aux (decompile_expression ~dialect)) cases in
    ...
    | Statements ->
      let* cases = bind_map_list (aux (decompile_if_clause dialect)) cases in
      let* cases = list_to_nsepseq cases in 
      let cases : _ CST.case = {kwd_case=ghost;expr;kwd_of=ghost;enclosing;lead_vbar;cases = wrap cases} in
  ...
  | E_tuple tuple ->
    let* tuple = bind_map_list (decompile_expression ~dialect) tuple in
    let* tuple = list_to_nsepseq tuple in
    return_expr @@ CST.ETuple (wrap @@ par tuple)
  | E_map map ->
    let* map = bind_map_list (bind_map_pair (decompile_expression ~dialect)) map in
    let aux (k,v) =
  ...
  | E_set set ->
    let* set = bind_map_list (decompile_expression ~dialect) set in
...
and decompile_lambda : dialect -> (AST.expr, AST.ty_expr) AST.lambda -> _ = fun dialect {binder;result} ->
    ...
    let* param_type = bind_map_option (bind_compose (ok <@ prefix_colon) (decompile_type_expr dialect)) binder.ascr in
    ...
    let* result,ret_type = match result.expression_content with
      ...
      let* ret_type = bind_compose (ok <@ prefix_colon) (decompile_type_expr dialect) type_annotation in
...
and decompile_declaration ~dialect : AST.declaration Location.wrap -> (CST.declaration, _) result = fun decl ->
  ...
  | Declaration_constant {binder; attr; expr} -> (
    ...
    | E_recursive {lambda; _} ->
      ...
      ok @@ CST.FunDecl (wrap fun_decl)
    | _ ->
      let* const_type = bind_map_option (bind_compose (ok <@ prefix_colon) (decompile_type_expr dialect)) binder.ascr in
      let* init = decompile_expression ~dialect expr in
...
and decompile_module ?(dialect=Verbose): AST.module_ -> (CST.ast, _) result = fun prg ->
  let* decl = bind_map_list (decompile_declaration ~dialect) prg in
