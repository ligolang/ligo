 ...
 fun ~display_format ~no_colour f a ->
  ...
  | Human_readable | Dev ->
    ...
    | `Concrete_pascaligo_expected_field_name reg ->
      Format.fprintf f "@[<hv>%a@.Expected a field name@]" snippet_pp_lift reg
    ...
    | `Concrete_pascaligo_ignored_attribute loc ->
      Format.fprintf f "@[<hv>%a@.Attribute being ignored@]" snippet_pp loc
    | `Concrete_pascaligo_expected_variable loc ->
      Format.fprintf f "@[<hv>%a@.Expected a declaration name@]" snippet_pp loc
