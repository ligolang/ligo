  ...
  fun e ->
    ...
    | `Concrete_jsligo_unsupported_pattern_type pl ->
      let message = Format.sprintf 
        ...
        @.  If this is pattern matching over variants, then a constructor of a variant is expected.\
        @.\
        @.  Other forms of pattern matching are not (yet) supported." in
    ...
    | `Concrete_jsligo_funarg_tuple_type_mismatch (region, pattern, texpr) ->
      ...
      make ~stage ~content
    | `Concrete_jsligo_not_in_switch_or_loop reg ->
      let message = Format.sprintf "Invalid let declaration.@.Only functions can be recursive." in
      let location = Location.File reg in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_statement_not_supported_at_toplevel s ->
      let message = Format.sprintf "Statement not supported at toplevel.@.Only let, const, and type statements are supported at the toplevel." in
      let location = Location.File (Raw.statement_to_region s) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_not_a_valid_parameter p ->
      let message = Format.sprintf "Not a valid function parameter." in
      let location = Location.File (Raw.expr_to_region p) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_rest_not_supported_here p ->
      let message = Format.sprintf "Rest property not supported here." in
      let location = Location.File (Raw.property_to_region p) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_property_not_supported p ->
    ...
    | `Concrete_jsligo_expected_an_expression a ->
      ...
      let location = Location.File (Raw.array_item_to_region a) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_new_not_supported p ->
      ...
      make ~stage ~content
    | `Concrete_jsligo_invalid_case (s, e) ->
      let message = Format.sprintf "Invalid '%s' field value. An anonymous arrow function was expected, eg. `None: () => foo`." s in
      let location = Location.File (Raw.expr_to_region e) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_invalid_constructor e ->
      let message = Format.sprintf "Invalid constructor. Expected a constructor like: `[\"Foo\"]` or `[\"Foo\", int, string]`." in
      let location = Location.File (Raw.type_expr_to_region e) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_unsupported_match_pattern e ->
    ...
    | `Concrete_jsligo_array_rest_not_supported e ->
      let message = Format.sprintf "Rest property not supported here." in
      let location = Location.File (Raw.array_item_to_region e) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_expected_a_variable reg ->
    ...
    | `Concrete_jsligo_expected_a_field_name s ->
      ...
      let location = Location.File (Raw.selection_to_region s) in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_expected_an_int e ->
    ...
    | `Concrete_jsligo_invalid_list_pattern_match _l ->
      ...
      make ~stage ~content
    | `Concrete_jsligo_no_shared_fields r ->
      let message = Format.sprintf "All the objects are expected to have a shared field with an unique value." in
      let location = Location.File r in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_unexpected -> 
    ...
    | `Concrete_jsligo_wrong_matchee_disc r ->
      ...
      make ~stage ~content
    | `Concrete_jsligo_case_break_disc r ->
      let message = Format.sprintf "A discriminated union case needs to end with a `break` or `return` statement. A fallthrough to another case is currently not supported." in
      let location = Location.File r in
      let content = make_content ~message ~location () in
      make ~stage ~content
    | `Concrete_jsligo_not_implemented r ->
