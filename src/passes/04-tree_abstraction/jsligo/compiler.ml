...
and compile_let_binding ~raise : CST.attributes -> CST.val_binding Region.reg -> Region.t -> AST.declaration =
  fun attributes val_binding region ->
    ...
    | `Val pattern -> Location.wrap ~loc:expr.location (AST.D_irrefutable_match  {pattern; attr; expr})
  ...
  | SSwitch s' ->
    ...
    | None -> (
      ...
      let initial = Binding (fun x ->
        e_let_mut_in (Pattern.var dummy_binder) [] switch_expr (* this is done so that in case of only default we don't the un-used variable warning *)
          (e_let_mut_in (Pattern.var fallthrough_binder) [] (e_false ())
            (e_let_mut_in (Pattern.var found_case_binder) [] (e_false ()) x))) in
