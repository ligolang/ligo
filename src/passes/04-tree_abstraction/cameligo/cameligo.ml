...
let decompile_pattern_to_string p =
  ...
  let p = Parsing.Cameligo.pretty_print_pattern ~cols:80 Parsing.Cameligo.Pretty.default_environment p in
  Buffer.contents p


let decompile_type_expression_to_string p =
  let t = Decompiler.decompile_type_expr p in
  let t = Parsing.Cameligo.pretty_print_type_expr Parsing.Cameligo.Pretty.default_environment t in
  Buffer.contents t
