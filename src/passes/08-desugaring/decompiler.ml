...
let decompile_exp_attributes : O.known_attributes -> I.attributes = fun { inline ; no_mutation ; view ; public ; hidden ; force_inline } ->
  ...
  List.fold ~init:[] ~f:aux
    [
      ...
      (fun () -> if force_inline then Some "force_inline" else None) ;
    ]
...
let rec decompile_expression : O.expression -> I.expression =
  ...
  | None ->
    ...
    | O.E_let_in {let_binder = {var; ascr;attributes=_};attr={inline=false;no_mutation=_;view=_;public=_;hidden=_;force_inline=false};rhs=expr1;let_result=expr2}
