...
let is_in_tail : position -> bool = function
  | `Tail -> true
  | `Non_tail -> false
...
let rec is_tail_recursive ~position : Value_var.t -> expression -> bool =
 fun n e ->
  ...
  | E_application { lamb; args } ->
    is_tail_recursive ~position n lamb && is_tail_recursive ~position:`Non_tail n args
  ...
  | E_type_inst { forall; type_ = _ } -> is_tail_recursive ~position n forall
  (* Mutation *)
  | E_assign { expression; _ } -> is_tail_recursive ~position:`Non_tail n expression
...
let is_tail_recursive : _ -> _ -> bool =
 fun fun_name lambda ->
  is_rec_binder_shadowed ~fun_name ~lambda
  || is_tail_recursive ~position:`Tail fun_name lambda.result
