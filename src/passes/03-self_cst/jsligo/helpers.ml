...
let rec fold_expression : ('a, 'err) folder -> 'a -> expr -> ('a, 'err) result = fun f init e  ->
  ...
  | EProj    {value; _} ->
    ...
    | Component {value = {inside; _}; _} ->
      let%bind res = self init inside in
      ok res
    in
    let%bind res = self init expr in
...
let rec map_expression : 'err mapper -> expr -> (expr, 'err) result = fun f e  ->
  ...
  | EArray {value;region} ->
      ...
      in
      let%bind inside = bind_map_npseq map_array_item value.inside in
      let value = {value with inside} in
  ...
  | EObject {value;region} ->
    ...
    in
    let%bind inside = bind_map_npseq map_property value.inside in
    let value = {value with inside} in
...
and map_statement : ('err) mapper -> statement -> (statement, 'err) result =
  ...
  | SLet {value; region} ->
      ...
      in
      let%bind bindings = bind_map_npseq map_binding value.bindings in
      return @@ SLet {
