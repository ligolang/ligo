...
module Environment (* : ENVIRONMENT *) = struct
  ...
  let get_opt : Value_var.t -> t -> type_expression option = fun e lst -> List.Assoc.find ~equal:Value_var.equal lst e
  let has : Value_var.t -> t -> bool = fun s t ->
    match get_opt s t with
    | None -> false
    | Some _ -> true
  let get_i_opt : Value_var.t -> t -> (type_expression * int) option =fun x lst -> List.find_mapi ~f:(fun i (e,t) -> if Value_var.equal e x then Some (t,i) else None) lst
  ...
  let select ?(rev = false) ?(keep = true) : Value_var.t list -> t -> t = fun lst env ->
    let e_lst =
