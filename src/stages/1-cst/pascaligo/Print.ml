...
and print_T_App state (node : (type_expr * type_tuple) reg) =
  ...
  let children =
    [ Tree.mk_child print_type_expr type_expr; Tree.mk_child print_type_tuple tuple ]
  in
...
and print_T_Attr state (node : attribute * type_expr) =
  ...
  let children =
    [ Tree.mk_child print_attribute attribute; Tree.mk_child print_type_expr type_expr ]
  in
...
and print_T_Fun state (node : (type_expr * arrow * type_expr) reg) =
  ...
  let children =
    [ Tree.mk_child print_type_expr domain; Tree.mk_child print_type_expr codomain ]
  in
...
and print_T_Int state (node : (lexeme * Z.t) wrap) = Tree.print_int "T_Int" state node
...
and print_compound : 'a. 'a Tree.printer -> label -> state -> 'a compound reg -> unit =
 fun print label state { value; region } ->
  let children =
...
and print_T_Sum state (node : sum_type reg) =
  let node = node.value in
  let variants =
    List.map ~f:(Tree.mk_child print_variant) @@ Utils.nsepseq_to_list node.variants
  in
...
and print_S_Attr state (node : attribute * statement) =
  ...
  let children =
    [ Tree.mk_child print_attribute attribute; Tree.mk_child print_statement statement ]
  in
...
and print_case_clause : 'a. 'a Tree.printer -> state -> 'a case_clause reg -> unit =
 fun print state { value; _ } ->
  let print_clause_pattern state = Tree.print_unary state "<pattern>" print_pattern in
  let children =
    [ Tree.mk_child print_clause_pattern value.pattern; Tree.mk_child print value.rhs ]
  in
...
and print_I_Cond state (node : test_clause conditional reg) =
  print_conditional state "I_Cond" ~if_so:print_test_clause ~if_not:print_test_clause node
...
and print_I_For state (node : for_int reg) =
  ...
  and print_step state (_, expr) = Tree.print_unary state "<step>" print_expr expr in
  let children =
...
and print_ForMap state (node : for_map reg) =
  ...
  let print_binding state (source, _arrow, image) =
    let children =
      [ Tree.mk_child Tree.print_literal source; Tree.mk_child Tree.print_literal image ]
    in
    Tree.print state "<binding>" children
  and print_collection state = Tree.print_unary state "<collection>" print_expr in
  let children =
...
and print_I_Patch state (node : patch reg) =
  let Region.{ value; region } = node in
  let children =
    [ Tree.mk_child print_expr value.collection; Tree.mk_child print_expr value.patch ]
  in
...
and print_I_Remove state (node : removal reg) =
  let Region.{ value; region } = node in
  let children =
    [ Tree.mk_child print_expr value.item; Tree.mk_child print_expr value.collection ]
  in
...
and print_ctor_args : 'a. 'a Tree.printer -> state -> ('a, comma) nsepseq par reg -> unit =
 fun print state node -> print_nsepseq state "<arguments>" print node.value.inside
...
and print_P_Attr state (node : attribute * pattern) =
  ...
  let children =
    [ Tree.mk_child print_attribute attribute; Tree.mk_child print_pattern pattern ]
  in
...
and print_P_Cons state (node : (pattern * sharp * pattern) reg) =
  ...
  let children = [ Tree.mk_child print_pattern head; Tree.mk_child print_pattern tail ] in
...
and print_P_Int state (node : (lexeme * Z.t) wrap) = Tree.print_int "P_Int" state node
...
and print_P_Nat state (node : (lexeme * Z.t) wrap) = Tree.print_int "P_Nat" state node

(* The pattern for the empty list *)

and print_P_Nil state (node : kwd_nil) = Tree.print_node ~region:node#region state "P_Nil"
...
and print_E_And state (node : kwd_and bin_op reg) = print_bin_op state "E_And" node
...
and print_E_App state (node : (expr * expr tuple option) reg) =
  ...
  let children =
    [ Tree.mk_child print_expr ctor; Tree.mk_child_opt (print_ctor_args print_expr) args ]
  in
...
and print_E_Block state (node : block_with reg) =
  let Region.{ value; region } = node in
  let children =
    [ Tree.mk_child print_block value.block; Tree.mk_child print_block_expr value.expr ]
  in
...
and print_E_Case state (node : expr case reg) = print_case print_expr "E_Case" state node

(* String catenation *)

and print_E_Cat state (node : caret bin_op reg) = print_bin_op state "E_Cat" node
...
and print_code state (node : expr) = Tree.print_unary state "<code>" print_expr node
...
and print_E_Equal state (node : equal bin_op reg) = print_bin_op state "E_Equal" node
...
and print_E_Cons state (node : sharp bin_op reg) = print_bin_op state "E_Cons" node

(* The Euclidean quotient *)

and print_E_Div state (node : slash bin_op reg) = print_bin_op state "E_Div" node
...
and print_E_Int state (node : (lexeme * Z.t) wrap) = Tree.print_int "E_Int" state node
...
and print_E_Mod state (node : kwd_mod bin_op reg) = print_bin_op state "E_Mod" node
...
and print_E_Mult state (node : times bin_op reg) = print_bin_op state "E_Mult" node
...
and print_E_Nat state (node : (lexeme * Z.t) wrap) = Tree.print_nat "E_Nat" state node
...
and print_E_Nil state (node : kwd_nil) = Tree.print_node ~region:node#region state "E_Nil"

(* Boolean negation *)

and print_E_Not state (node : kwd_not un_op reg) = print_un_op state "E_Not" node
...
and print_E_Sub state (node : minus bin_op reg) = print_bin_op state "E_Sub" node
...
and print_E_Typed state (node : typed_expr par reg) =
  ...
  let children =
    [ Tree.mk_child print_expr expr; Tree.mk_child print_type_annotation annotation ]
  in
...
and print_E_Update state (node : update reg) =
  let Region.{ value; region } = node in
  let children =
    [ Tree.mk_child print_expr value.structure; Tree.mk_child print_expr value.update ]
  in
