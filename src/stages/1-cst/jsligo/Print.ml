...
and print_ctor_app_kind state (node : ctor_app_kind) =
  match node with
    CtorStr  s -> Tree.make_node ~region:s#region state s#payload
  | CtorName c -> Tree.make_node ~region:c#region state c#payload
