...
let mappers ?wrap_map:_ : W.variant -> P.structure_item list = fun variant ->
  ...
  let case (label_main , _) (label , tes) =
    ...
    let body =
      if String.(label <> label_main) then (
        e_var "x"
      ) else (
    ...
    (label , names , body)
  in
  let single : string * W.type_expression list -> _ = fun ((label , tes) as main) ->
    let names = var_names "f" @@ List.length tes in
    d_value ("map_" ^ label) @@
    e_funs names @@ e_fun "x" @@ e_match ~polymorphic (e_var "x") @@
...
let setters ?wrap_map:_ : W.variant -> P.structure_item list = fun variant ->
  ...
  let single : string * W.type_expression list -> _ = fun ((label , tes) as main) ->
    ...
    d_value ("set_" ^ label) @@
    e_fun "x" @@ e_funs names @@ e_match ~polymorphic (e_var "x") @@
    List.map ~f:(case main) lst
  in
  let all = List.map ~f:single lst in
  all
