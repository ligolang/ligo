...
module Make(Params : Woo_helpers.PARAMS) = struct
  module Generate = struct
    ...
    let declaration_of_prefix (name , body) =
      Helpers.declaration ~name ~body

    let str ~prefixes ?wrap_constructor ?wrap_get ?wrap_map ?default_get : (P.rec_flag * P.type_declaration list) -> P.structure_item list = fun (rec_flag , tds) ->
      ...
      let items = List.filter ~f:(fun x -> not @@ Helpers.is_tauto_type_declaration x) items in
      items
...
let make_PARAMS loc : (module Woo_helpers.PARAMS)  = (module struct let location = loc end : Woo_helpers.PARAMS)
...
let deriver =
  let str_type_decl =
    let generate ~loc ~path:_ input prefix prefixes wrap_constructor wrap_get wrap_map default_get =
      let prefixes =
        let prefix =
          match prefix with
          | Some x -> [ x ]
