...
let matching ?wrap_get ~namer case : W.variant -> W.labelled_types -> P.structure_item = fun variant (base_label , _ty) ->
...
let matching_full ?wrap_get ~namer rhs : W.variant -> P.structure_item list = fun variant ->
  let matching_single =
...
let matching_default ?(default_get = "Option") variant : P.structure_item list =
  ...
  let string = match default_get with
    | "Exception" -> "_exn"
    | _ -> "_opt" in
  let namer_default x = "get_" ^ x ^ string in
  let matching_single (base_label, _ty) =
    let body = A.pexp_ident ~loc @@ Location.mkloc (Longident.Lident (namer_default @@ label_to_variable base_label)) loc in
    let function_name = p_var @@ namer @@ label_to_variable base_label in
    let declaration = A.value_binding ~loc ~expr:body ~pat:function_name in
    let declarations = A.pstr_value ~loc Nonrecursive [ declaration ] in
    declarations
