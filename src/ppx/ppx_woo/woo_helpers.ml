...
module Generate(Params : PARAMS) = struct
  ...
  let var_names prefix n =
    List.map ~f:(fun i -> prefix ^ (string_of_int i)) @@
    Base.List.range 0 n
  ...
  let p_record_pun lst =
    A.ppat_record ~loc
      ...
      Closed
  let e_record lst = A.pexp_record ~loc (List.map ~f:(fun (var , expr) -> (lident var , expr)) lst) None
  let e_with record property content = A.pexp_record ~loc [lident property , content] (Some (e_var record))
  let e_fun ?(label = P.Nolabel) ?default var body =
    A.pexp_fun ~loc label default (p_var var) body
  let e_funs vars body =
    List.fold_right ~f:e_fun vars ~init:body
  ...
  let e_match : polymorphic:bool -> P.expression -> (string * string list * P.expression) list -> P.expression =
    fun ~polymorphic matchee lst ->
    let case : (string * string list * P.expression) -> P.case = fun (constructor , params , rhs) ->
      let lhs =
