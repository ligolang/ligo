...
let mapper : W.labelled_record_field -> P.structure_item list = fun lts ->
  let (label , W.{ type_expression = ty ; default_value = _ ; index = _ }) = lts in
  ignore ty ;
  let body =
      e_fun "f" @@ e_fun "x" @@
...
let mappers : W.record -> P.structure_item list = fun property_declarations ->
  let ltss = SMap.to_kv_list property_declarations in
  let itemss = List.map ~f:mapper ltss in
  let items = List.concat itemss in
  items

let setter : W.labelled_record_field -> P.structure_item list = fun lts ->
  let (label , W.{ type_expression = ty ; default_value = _ ; index = _ }) = lts in
  ignore ty ;
  let body =
      e_fun "x" @@ e_fun "content" @@
      e_with
        "x"
        label
        (e_var "content")
...
let setters : W.record -> P.structure_item list = fun property_declarations ->
  let ltss = SMap.to_kv_list property_declarations in
