...
let return = ref Done
let compile_file =
  let f source_file entry_point views syntax protocol_version display_format disable_michelson_typechecking enable_typed_opt no_stdlib michelson_format output_file show_warnings warning_as_error michelson_comments constants file_constants project_root warn_unused_rec () =
    let raw_options = Compiler_options.make_raw_options ~entry_point ~syntax ~views ~protocol_version ~disable_michelson_typechecking ~enable_typed_opt ~no_stdlib ~warning_as_error ~constants ~file_constants ~project_root ~warn_unused_rec () in
  ...
  (f <$> source_file <*> entry_point <*> on_chain_views <*> syntax <*> protocol_version <*> display_format <*> disable_michelson_typechecking <*> enable_michelson_typed_opt <*> no_stdlib <*> michelson_code_format <*> output_file <*> warn <*> werror <*> michelson_comments <*> constants <*> file_constants <*> project_root <*> warn_unused_rec)
...
let compile_expression =
  let f syntax expression protocol_version init_file display_format without_run no_stdlib michelson_format show_warnings warning_as_error constants file_constants project_root warn_unused_rec () =
    let raw_options = Compiler_options.make_raw_options ~syntax ~protocol_version ~without_run ~no_stdlib ~warning_as_error ~constants ~file_constants ~project_root ~warn_unused_rec () in
