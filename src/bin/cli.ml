...
let compile_parameter =
  let f source_file entry_point expression syntax amount balance sender source now display_format michelson_format output_file brief =
    ...
    in
  let term =
    Term.(const f $ source_file 0 $ entry_point 1 $ expression "PARAMETER" 2 $ syntax $ amount $ balance $ sender $ source $ now $ display_format $ michelson_code_format $ output_file $ brief) in
...
let interpret =
  let f expression init_file syntax amount balance sender source now display_format brief =
  ...
  in
  let term =
    Term.(const f $ expression "EXPRESSION" 0 $ init_file $ syntax $ amount $ balance $ sender $ source $ now $ display_format $ brief) in
...
let compile_storage =
  ...
  let term =
    Term.(const f $ source_file 0 $ entry_point 1 $ expression "STORAGE" 2 $ syntax $ amount $ balance $ sender $ source $ now $ display_format $ michelson_code_format $ output_file $ brief) in
...
let dry_run =
  let f source_file entry_point storage input amount balance sender source now syntax display_format brief =
    return_result ~display_format ~brief (Decompile.Formatter.expression_format) @@
      let%bind typed_prg,state = Compile.Utils.type_file source_file syntax (Contract entry_point) in
      let      env             = Ast_typed.program_environment Environment.default typed_prg in
      let%bind mini_c_prg      = Compile.Of_typed.compile typed_prg in
    ...
    in
  let term =
    Term.(const f $ source_file 0 $ entry_point 1 $ expression "PARAMETER" 2 $ expression "STORAGE" 3 $ amount $ balance $ sender $ source $ now $ syntax $ display_format $ brief) in
  let cmdname = "dry-run" in
  let doc = "Subcommand: Run a smart-contract with the given storage and input." in
  (Term.ret term , Term.info ~doc cmdname)

let run_function =
  let f source_file entry_point parameter amount balance sender source now syntax display_format brief =
    return_result ~display_format ~brief (Decompile.Formatter.expression_format) @@
    ...
    in
  let term =
    Term.(const f $ source_file 0 $ entry_point 1 $ expression "PARAMETER" 2 $ amount $ balance $ sender $ source $ now $ syntax $ display_format $ brief) in
  let cmdname = "run-function" in
  let doc = "Subcommand: Run a function with the given parameter." in
  (Term.ret term , Term.info ~doc cmdname)

let evaluate_value =
  let f source_file entry_point amount balance sender source now syntax display_format brief =
    return_result ~display_format ~brief Decompile.Formatter.expression_format @@
    ...
    in
  let term =
    Term.(const f $ source_file 0 $ entry_point 1 $ amount $ balance $ sender $ source $ now $ syntax $ display_format $ brief) in
