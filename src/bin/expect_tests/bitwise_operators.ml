...
let%expect_test _ =
  ...
  [%expect{|
    Invalid arguments.
    Expected an argument of type (sum[False -> unit , True -> unit], sum[False -> unit , True -> unit]) or (nat, nat) or (int, nat), but got an argument of type int, int. |}]
let%expect_test _ =
  run_ligo_bad [ "compile" ; "expression" ; "cameligo" ; "14n lor 2" ] ;
  [%expect{|
    Invalid arguments.
    Expected an argument of type (sum[False -> unit , True -> unit], sum[False -> unit , True -> unit]) or (nat, nat), but got an argument of type nat, int. |}]
let%expect_test _ =
  run_ligo_bad [ "compile" ; "expression" ; "cameligo" ; "14 lor 2n" ] ;
  [%expect{|
    Invalid arguments.
    Expected an argument of type (sum[False -> unit , True -> unit], sum[False -> unit , True -> unit]) or (nat, nat), but got an argument of type int, nat. |}]
let%expect_test _ =
