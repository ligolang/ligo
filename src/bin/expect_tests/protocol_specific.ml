open Cli_expect

let contract basename =
  "../../test/contracts/" ^ basename

let%expect_test _ =
  run_ligo_good [ "compile-contract" ; contract "protocol_dalphanet.mligo" ; "main" ; "--protocol=dalphanet" ; "--disable-michelson-typechecking" ] ;
  [%expect {|
    { parameter (list (pair bls12_381_g1 bls12_381_g2)) ;
      storage (option bool) ;
      code { CAR ; PAIRING_CHECK ; SOME ; NIL operation ; PAIR } } |}] ;

  run_ligo_good [ "print-ast-typed" ; contract "protocol_dalphanet.mligo" ; "--protocol=dalphanet" ; ] ;
  [%expect {xxx|
    type bls_l = list (( bls12_381_g1 * bls12_381_g2 ))
    type bool_option = option (sum[false -> unit , true -> unit])
    const a = [%Michelson {|
      { PUSH @vk_gamma_c bls12_381_g1 0x063bd6e11e2fcaac1dd8cf68c6b1925a73c3c583e298ed37c41c3715115cf96358a42dbe85a0228cbfd8a6c8a8c54cd015b5ae2860d1cc47f84698d951f14d9448d03f04df2ca0ffe609a2067d6f1a892163a5e05e541279134cae52b1f23c6b; }
      |}]
    const b = [%Michelson {|
        { PUSH @vk_delta bls12_381_g2 ; }
      |}]
    const pairing_check = lambda (n) return ([%Michelson {| {PAIRING_CHECK; SOME} |}])@(n)
    const main = lambda (#1) return let s = #1.1 in let p = #1.0 in ( LIST_EMPTY() , (pairing_check)@(p) ) |xxx}] 

let%expect_test _ =
  run_ligo_bad [ "compile-contract" ; contract "protocol_dalphanet.mligo" ; "main" ] ;
  [%expect {|
    in file "../../test/contracts/protocol_dalphanet.mligo", line 1, characters 14-26
      1 | type bls_l = (bls12_381_g1 * bls12_381_g2) list
      2 | type bool_option = bool option

    Type "bls12_381_g1" not found. |}] ;

  run_ligo_bad [ "print-ast-typed" ; contract "protocol_dalphanet.mligo" ] ;
  [%expect {|
    in file "../../test/contracts/protocol_dalphanet.mligo", line 1, characters 14-26
      1 | type bls_l = (bls12_381_g1 * bls12_381_g2) list
      2 | type bool_option = bool option

    Type "bls12_381_g1" not found. |}] ;