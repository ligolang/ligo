...
let%expect_test _ =
  ...
  [%expect {|
  let #A#current_turn#6 = lambda (i) return ADD(i , +1) in
  let #A#other#7 = lambda (n) return let current_turn = (#A#current_turn#6)@(+1) in
  ASSERTION(EQ(n ,
  current_turn)) in
  let main = lambda (gen#1) return  match gen#1 with
                                     | ( _p , _s ) ->
                                     ( LIST_EMPTY() , (#A#other#7)@(+2) ) in
  unit |}]
let%expect_test _ =
