...
let params ~add_warning counter msg keys is_validl f =
...
let not_enough_1_of_2 ~add_warning f () =
  let* (program,env) = get_program ~add_warning f () in
  let exp_failwith = "Not enough signatures passed the check" in
  let keys = gen_keys () in
  let* test_params = params ~add_warning 0 empty_message [keys] [true] f in
...
let unmatching_counter ~add_warning f () =
  ...
  let keys = gen_keys () in
  let* test_params = params ~add_warning 1 empty_message [keys] [true] f in
  let* () = expect_string_failwith
...
let invalid_1_of_1 ~add_warning f () =
  ...
  let keys = [gen_keys ()] in
  let* test_params = params ~add_warning 0 empty_message keys [false] f in
  let* () = expect_string_failwith
    (program,env) "main" (e_pair test_params (init_storage 1 0 keys)) exp_failwith in
  ok ()
...
let valid_1_of_1 ~add_warning f () =
  ...
  let* () = expect_eq_n_trace_aux [0;1;2] (program,env) "main"
      (fun n ->
        let* params = params ~add_warning n empty_message [keys] [true] f in
...
let valid_2_of_3 ~add_warning f () =
  ...
  let* () = expect_eq_n_trace_aux [0;1;2] (program,env) "main"
      (fun n ->
        let* params = params ~add_warning n empty_message param_keys [true;true] f in
...
let invalid_3_of_3 ~add_warning f () =
  ...
  let* test_params = params ~add_warning 0 empty_message param_keys [false;true;true] f in
  let exp_failwith = "Invalid signature" in
  let* () = expect_string_failwith
...
let not_enough_2_of_3 ~add_warning f () =
  ...
  let* test_params = params ~add_warning 0 empty_message (valid_keys) [true;true] f in
  let exp_failwith = "Not enough signatures passed the check" in
  let* () = expect_string_failwith
