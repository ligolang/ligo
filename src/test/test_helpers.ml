...
let pack_payload ~raise (program:Ast_typed.program) (payload:Ast_imperative.expression) : bytes =
  let code =
    ...
    let aggregated = Ligo_compile.Of_typed.compile_expression ~raise ~options typed in
...
let typed_program_to_michelson ~raise (program, env) =
  ignore env;
  let aggregated = Ligo_compile.Of_typed.compile_expression ~raise ~options program in
  let mini_c = Ligo_compile.Of_aggregated.compile_expression ~raise aggregated in
...
let typed_program_with_imperative_input_to_michelson ~raise (program : Ast_typed.program) (entry_point: string) (input: Ast_imperative.expression) : Stacking.compiled_expression *  Ast_aggregated.type_expression =
  ...
  let aggregated = Ligo_compile.Of_typed.compile_expression_in_context ~raise ~options typed_app program in
  let mini_c = Ligo_compile.Of_aggregated.compile_expression ~raise aggregated in
...
let expect_evaluate ~raise (program : Ast_typed.program) entry_point expecter =
  trace ~raise (test_run_tracer entry_point) @@
  let aggregated      = Ligo_compile.Of_typed.apply_to_entrypoint ~raise ~options program entry_point in
  let mini_c          = Ligo_compile.Of_aggregated.compile_expression ~raise aggregated in
...
let compile_main ~raise ~add_warning f () =
  let agg = Ligo_compile.Of_typed.apply_to_entrypoint_contract ~raise ~options (get_program ~raise ~add_warning f ()) @@ Ast_typed.ValueVar.of_input_var "main" in
