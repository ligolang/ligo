...
let raw_options = RawOptions.make ()
...
let test_use_external_packages ~raise ~(raw_options : RawOptions.t) () =
...
let test_import_external_packages ~raise ~(raw_options : RawOptions.t) () =
  let project_root = Some "projects/demo" in
  let raw_options = { raw_options with project_root = project_root } in
...
let test_use_scoped_package ~raise ~(raw_options : RawOptions.t) () =
...
let test_import_scoped_packages ~raise ~(raw_options : RawOptions.t) () =
  let project_root = Some "projects/using_scope_pkg_project" in
  let raw_options = { raw_options with project_root = project_root } in
  test_seq ~raise ~raw_options (make_init_state_cameligo ~project_root ()) [
