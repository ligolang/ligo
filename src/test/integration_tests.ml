...
let type_alias () : (unit,_) result =
  let* program = type_file "./contracts/type-alias.ligo" in
  expect_eq_evaluate program "foo" (e_int 23)

let function_ () : (unit,_) result =
  let* program = type_file "./contracts/function.ligo" in
  let make_expect = fun n -> n in
  expect_eq_n_int program "main" make_expect

let blockless () : (unit,_) result =
  let* program = type_file "./contracts/blockless.ligo" in
...
let assign () : (unit,_) result =
  let* program = type_file "./contracts/assign.ligo" in
  let make_expect = fun n -> n + 1 in
  expect_eq_n_int program "main" make_expect

let annotation () : (unit,_) result =
  let* program = type_file "./contracts/annotation.ligo" in
  let* () =
...
let complex_function () : (unit,_) result =
  let* program = type_file "./contracts/function-complex.ligo" in
  let make_expect = fun n -> (3 * n + 2) in
  expect_eq_n_int program "main" make_expect

let anon_function () : (unit, _) result =
  let* program = type_file "./contracts/function-anon.ligo" in
  let* () =
...
let application () : (unit, _) result =
  let* program = type_file "./contracts/application.ligo" in
  let* () =
...
let variant () : (unit, _) result =
  let* program = type_file "./contracts/variant.ligo" in
  let* () =
...
let variant_mligo () : (unit, _) result =
  let* program = type_file "./contracts/variant.mligo" in
  let* () =
    let expected = e_constructor "Foo" (e_int 42) in
    expect_eq_evaluate program "foo" expected in
  let* () =
...
let variant_religo () : (unit, _) result =
  let* program = type_file "./contracts/variant.religo" in
  let* () =
    let expected = e_constructor "Foo" (e_int 42) in
    expect_eq_evaluate program "foo" expected in
  let* () =
...
let variant_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/variant.jsligo" in
  let* () =
    let expected = e_constructor "Foo" (e_int 42) in
    expect_eq_evaluate program "foo" expected in
  let* () =
...
let variant_matching () : (unit, _) result =
  let* program = type_file "./contracts/variant-matching.ligo" in
  let* () =
...
let closure () : (unit, _) result =
  let* program = type_file "./contracts/closure.ligo" in
  let* program_1 = type_file "./contracts/closure-1.ligo" in
  let* program_2 = type_file "./contracts/closure-2.ligo" in
  let* program_3 = type_file "./contracts/closure-3.ligo" in
  let* _ =
...
let closure_mligo () : (unit, _) result =
  let* program = type_file "./contracts/closure.mligo" in
  let* _ =
...
let closure_religo () : (unit, _) result =
  let* program = type_file "./contracts/closure.religo" in
  let* _ =
    let input = e_int 0 in
    let expected = e_int 25 in
    expect_eq program "test" input expected
...
let closure_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/closure.jsligo" in
  let* _ =
    let input = e_int 0 in
    let expected = e_int 25 in
    expect_eq program "test" input expected
...
let shadow () : (unit, _) result =
  let* program = type_file "./contracts/shadow.ligo" in
  let make_expect = fun _ -> 0 in
  expect_eq_n_int program "foo" make_expect

let shadowing () : (unit, _) result =
  let* program = type_file "./contracts/shadowing.mligo" in
  let* _ =
...
let higher_order () : (unit, _) result =
  let* program = type_file "./contracts/high-order.ligo" in
  let make_expect = fun n -> n in
...
let higher_order_mligo () : (unit, _) result =
  let* program = type_file "./contracts/high-order.mligo" in
  let make_expect = fun n -> n in
  let* _ = expect_eq_n_int program "foobar" make_expect in
  let* _ = expect_eq_n_int program "foobar2" make_expect in
  let* _ = expect_eq_n_int program "foobar3" make_expect in
...
let higher_order_religo () : (unit, _) result =
  let* program = type_file "./contracts/high-order.religo" in
  let make_expect = fun n -> n in
  let* _ = expect_eq_n_int program "foobar" make_expect in
  let* _ = expect_eq_n_int program "foobar2" make_expect in
  let* _ = expect_eq_n_int program "foobar3" make_expect in
...
let higher_order_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/high-order.jsligo" in
  let make_expect = fun n -> n in
  let* _ = expect_eq_n_int program "foobar" make_expect in
  let* _ = expect_eq_n_int program "foobar2" make_expect in
  let* _ = expect_eq_n_int program "foobar3" make_expect in
...
let shared_function () : (unit, _) result =
  let* program = type_file "./contracts/function-shared.ligo" in
  let* () =
    let make_expect = fun n -> (n + 1) in
...
let shared_function_mligo () : (unit, _) result =
  let* program = type_file "./contracts/function-shared.mligo" in
  let* () =
...
let shared_function_religo () : (unit, _) result =
  let* program = type_file "./contracts/function-shared.religo" in
  let* () =
    let make_expect = fun n -> (2 * n + 70) in
    expect_eq_n_int program "foobar" make_expect
  in
...
let shared_function_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/function-shared.jsligo" in
  let* () =
    let make_expect = fun n -> (2 * n + 70) in
    expect_eq_n_int program "foobar" make_expect
  in
...
let bool_expression () : (unit, _) result =
  let* program = type_file "./contracts/boolean_operators.ligo" in
  let* _ =
...
let bool_expression_mligo () : (unit, _) result =
  let* program = type_file "./contracts/boolean_operators.mligo" in
  let* _ =
    let aux (name, f) = expect_eq_b_bool program name f in
    bind_map_list aux [
      ("or_true", fun b -> b || true) ;
...
let bool_expression_religo () : (unit, _) result =
  let* program = type_file "./contracts/boolean_operators.religo" in
  let* _ =
    let aux (name, f) = expect_eq_b_bool program name f in
    bind_map_list aux [
      ("or_true", fun b -> b || true) ;
...
let bool_expression_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/boolean_operators.jsligo" in
  let* _ =
    let aux (name, f) = expect_eq_b_bool program name f in
    bind_map_list aux [
      ("or_true", fun b -> b || true) ;
...
let arithmetic () : (unit, _) result =
  let* program = type_file "./contracts/arithmetic.ligo" in
  let* _ =
...
let arithmetic_mligo () : (unit, _) result =
  let* program = type_file "./contracts/arithmetic.mligo" in
  let* _ =
    let aux (name, f) = expect_eq_n_int program name f in
    bind_map_list aux [
      ("plus_op", fun n -> (n + 42)) ;
...
let arithmetic_religo () : (unit, _) result =
  let* program = type_file "./contracts/arithmetic.religo" in
  let* _ =
    let aux (name, f) = expect_eq_n_int program name f in
    bind_map_list aux [
      ("plus_op", fun n -> (n + 42)) ;
...
let arithmetic_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/arithmetic.jsligo" in
  let* _ =
    let aux (name, f) = expect_eq_n_int program name f in
    bind_map_list aux [
      ("plus_op", fun n -> (n + 42)) ;
...
let bitwise_arithmetic () : (unit, _) result =
  let* program = type_file "./contracts/bitwise_arithmetic.ligo" in
...
let bitwise_arithmetic_mligo () : (unit, _) result =
  let* program = type_file "./contracts/bitwise_arithmetic.mligo" in
  let* () = expect_eq program "or_op" (e_nat 7) (e_nat 7) in
  let* () = expect_eq program "or_op" (e_nat 3) (e_nat 7) in
  let* () = expect_eq program "or_op" (e_nat 2) (e_nat 6) in
  let* () = expect_eq program "or_op" (e_nat 14) (e_nat 14) in
...
let bitwise_arithmetic_religo () : (unit, _) result =
  let* program = type_file "./contracts/bitwise_arithmetic.religo" in
  let* () = expect_eq program "or_op" (e_nat 7) (e_nat 7) in
  let* () = expect_eq program "or_op" (e_nat 3) (e_nat 7) in
  let* () = expect_eq program "or_op" (e_nat 2) (e_nat 6) in
  let* () = expect_eq program "or_op" (e_nat 14) (e_nat 14) in
...
let bitwise_arithmetic_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/bitwise_arithmetic.jsligo" in
  let* () = expect_eq program "or_op" (e_nat 7) (e_nat 7) in
  let* () = expect_eq program "or_op" (e_nat 3) (e_nat 7) in
  let* () = expect_eq program "or_op" (e_nat 2) (e_nat 6) in
  let* () = expect_eq program "or_op" (e_nat 14) (e_nat 14) in
...
let string_arithmetic () : (unit, _) result =
  let* program = type_file "./contracts/string_arithmetic.ligo" in
...
let string_arithmetic_mligo () : (unit, _) result =
  let* program = type_file "./contracts/string_arithmetic.mligo" in
...
let string_arithmetic_religo () : (unit, _) result =
  let* program = type_file "./contracts/string_arithmetic.religo" in
  let* () = expect_eq program "size_op"  (e_string "tata") (e_nat 4) in
  let* () = expect_eq program "slice_op" (e_string "tata") (e_string "at") in
  let* () = expect_eq program "slice_op" (e_string "foo") (e_string "oo") in
  let* () = expect_eq program "concat_syntax" (e_string "string_") (e_string "string_test_literal")
...
let string_arithmetic_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/string_arithmetic.jsligo" in
  let* () = expect_eq program "size_op"  (e_string "tata") (e_nat 4) in
  let* () = expect_eq program "slice_op" (e_string "tata") (e_string "at") in
  let* () = expect_eq program "slice_op" (e_string "foo") (e_string "oo") in
  let* () = expect_eq program "concat_syntax" (e_string "string_") (e_string "string_test_literal")
...
let bytes_arithmetic () : (unit, _) result =
  let* program = type_file "./contracts/bytes_arithmetic.ligo" in
...
let crypto () : (unit, _) result =
  let* program = type_file "./contracts/crypto.ligo" in
  let* foo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f00" in
  let* foototo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f007070" in
...
let crypto_mligo () : (unit, _) result =
  let* program = type_file "./contracts/crypto.mligo" in
  let* foo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f00" in
  let* foototo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f007070" in
  let* b1 = Test_helpers.run_typed_program_with_imperative_input program "hasherman512" foo in
  let* () = expect_eq_core program "hasherman512" foo b1 in
...
let crypto_religo () : (unit, _) result =
  let* program = type_file "./contracts/crypto.religo" in
  let* foo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f00" in
  let* foototo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f007070" in
  let* b1 = Test_helpers.run_typed_program_with_imperative_input program "hasherman512" foo in
  let* () = expect_eq_core program "hasherman512" foo b1 in
...
let crypto_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/crypto.jsligo" in
  let* foo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f00" in
  let* foototo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f007070" in
  let* b1 = Test_helpers.run_typed_program_with_imperative_input program "hasherman512" foo in
  let* () = expect_eq_core program "hasherman512" foo b1 in
...
let bytes_arithmetic_mligo () : (unit, _) result =
  let* program = type_file "./contracts/bytes_arithmetic.mligo" in
  let* foo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f00" in
  let* foototo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f007070" in
  let* toto = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "7070" in
  let* empty = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "" in
...
let bytes_arithmetic_religo () : (unit, _) result =
  let* program = type_file "./contracts/bytes_arithmetic.religo" in
  let* foo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f00" in
  let* foototo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f007070" in
  let* toto = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "7070" in
  let* empty = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "" in
...
let bytes_arithmetic_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/bytes_arithmetic.jsligo" in
  let* foo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f00" in
  let* foototo = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "0f007070" in
  let* toto = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "7070" in
  let* empty = trace_option (test_internal __LOC__) @@ e_bytes_hex_ez "" in
...
let set_arithmetic () : (unit, _) result =
  let* program = type_file "./contracts/set_arithmetic.ligo" in
  let* program_1 = type_file "./contracts/set_arithmetic-1.ligo" in
  let* () =
...
let set_arithmetic_mligo () : (unit, _) result =
  let* program = type_file "./contracts/set_arithmetic.mligo" in
  let* program_1 = type_file "./contracts/set_arithmetic-1.mligo" in
  let* () =
...
let set_arithmetic_religo () : (unit, _) result =
  let* program = type_file "./contracts/set_arithmetic.religo" in
  let* program_1 = type_file "./contracts/set_arithmetic-1.ligo" in
  let* () =
    expect_eq program "literal_op"
      (e_unit ())
...
let set_arithmetic_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/set_arithmetic.jsligo" in
  let* program_1 = type_file "./contracts/set_arithmetic-1.ligo" in
  let* () =
    expect_eq program "literal_op"
      (e_unit ())
...
let unit_expression () : (unit, _) result =
  let* program = type_file "./contracts/unit.ligo" in
  expect_eq_evaluate program "u" (e_unit ())

let string_expression () : (unit, _) result =
  let* program = type_file "./contracts/string.ligo" in
...
let include_ () : (unit, _) result =
  let* program = type_file "./contracts/includer.ligo" in
  expect_eq_evaluate program "bar" (e_int 144)

let include_mligo () : (unit, _) result =
  let* program = type_file "./contracts/includer.mligo" in
  expect_eq_evaluate program "bar" (e_int 144)

let include_religo () : (unit, _) result =
  let* program = type_file "./contracts/includer.religo" in
  expect_eq_evaluate program "bar" (e_int 144)

let include_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/includer.jsligo" in
  expect_eq_evaluate program "bar" (e_int 144)
...
let modules_ligo () : (unit, _) result =
  let* program = type_file "./contracts/modules.ligo" in
  modules program

let modules_mligo () : (unit, _) result =
  let* program = type_file "./contracts/modules.mligo" in
  modules program

let modules_religo () : (unit, _) result =
  let* program = type_file "./contracts/modules.religo" in
  modules program

let modules_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/modules.jsligo" in
  modules program
...
let multiple_parameters_mligo () : (unit, _) result  =
  let* program = type_file "./contracts/multiple-parameters.mligo" in
  let aux ((name : string) , make_input , make_output) =
    let make_output' = fun n -> e_int @@ make_output n in
    expect_eq_n program name make_input make_output'
  in
...
let multiple_parameters_religo () : (unit, _) result  =
  let* program = type_file "./contracts/multiple-parameters.religo" in
  let aux ((name : string) , make_input , make_output) =
    let make_output' = fun n -> e_int @@ make_output n in
    expect_eq_n program name make_input make_output'
  in
...
let multiple_parameters_jsligo () : (unit, _) result  =
  let* program = type_file "./contracts/multiple-parameters.jsligo" in
  let aux ((name : string) , make_input , make_output) =
    let make_output' = fun n -> e_int @@ make_output n in
    expect_eq_n program name make_input make_output'
  in
...
let record () : (unit, _) result  =
  let* program = type_file "./contracts/record.ligo" in
  let* () =
...
let record_mligo () : (unit, _) result  =
  let* program = type_file "./contracts/record.mligo" in
  let* () =
    let expected = record_ez_int ["foo" ; "bar"] 0 in
    expect_eq_evaluate program "fb" expected
  in
...
let record_religo () : (unit, _) result  =
  let* program = type_file "./contracts/record.religo" in
  let* () =
    let expected = record_ez_int ["foo" ; "bar"] 0 in
    expect_eq_evaluate program "fb" expected
  in
...
let record_jsligo () : (unit, _) result  =
  let* program = type_file "./contracts/record.jsligo" in
  let* () =
    let expected = record_ez_int ["foo" ; "bar"] 0 in
    expect_eq_evaluate program "fb" expected
  in
...
let option () : (unit, _) result =
  let* program = type_file "./contracts/option.ligo" in
  let* () =
...
let moption () : (unit, _) result =
  let* program = type_file "./contracts/option.mligo" in
  let* () =
    let expected = e_some (e_int 42) in
    expect_eq_evaluate program "s" expected
  in
...
let reoption () : (unit, _) result =
  let* program = type_file "./contracts/option.religo" in
  let* () =
    let expected = e_some (e_int 42) in
    expect_eq_evaluate program "s" expected
  in
...
let jsoption () : (unit, _) result =
  let* program = type_file "./contracts/option.jsligo" in
  let* () =
    let expected = e_some (e_int 42) in
    expect_eq_evaluate program "s" expected
  in
...
let condition () : (unit, _) result =
  let* program = type_file "./contracts/condition.ligo" in
  let* _ =
...
let sequence_mligo () : (unit, _) result =
  let* program = type_file "./contracts/sequence.mligo" in
...
let eq_bool () : (unit, _) result =
  let* program = type_file "./contracts/eq_bool.ligo" in
  eq_bool_common program

let eq_bool_mligo () : (unit, _) result =
  let* program = type_file "./contracts/eq_bool.mligo" in
  eq_bool_common program

let eq_bool_religo () : (unit, _) result =
  let* program = type_file "./contracts/eq_bool.religo" in
  eq_bool_common program

let eq_bool_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/eq_bool.jsligo" in
  eq_bool_common program

let condition_simple () : (unit, _) result =
  let* program = type_file "./contracts/condition-simple.ligo" in
  let make_input = e_int in
  let make_expected = fun _ -> e_int 42 in
  expect_eq_n program "main" make_input make_expected

let loop1 () : (unit, _) result =
  let* _program = type_file "./contracts/loop1.ligo" in
  ok ()

let loop2 () : (unit, _) result =
  let* program = type_file "./contracts/loop2.ligo" in
  let* () =
...
let loop3 () : (unit, _) result =
  let* program = type_file "./contracts/loop3.ligo" in
  let* () =
    let make_input = e_nat in
    let make_expected = e_nat in
...
let loop4 () : (unit, _) result =
  let* program = type_file "./contracts/loop4.ligo" in
  let* () =
    let make_input = e_nat in
...
let loop5 () : (unit, _) result =
  let* program = type_file "./contracts/loop5.ligo" in
  let* () =
    let make_input = e_nat in
...
let loop6 () : (unit, _) result =
  let* program = type_file "./contracts/loop6.ligo" in
  let* () =
    let make_input = e_nat in
...
let loop7 () : (unit, _) result =
  let* program = type_file "./contracts/loop7.ligo" in
  let input = e_unit () in
  let* () =
...
let loop8 () : (unit, _) result =
  let* program = type_file "./contracts/loop8.ligo" in
  let input = e_unit () in
  let* () =
...
let loop9 () : (unit, _) result =
  let* program = type_file "./contracts/loop9.ligo" in
  let input = e_unit () in
  let* () =
...
let loop10 () : (unit, _) result =
  let* program = type_file "./contracts/loop10.ligo" in
  let input = e_unit () in
  let* () =
...
let loop11 () : (unit, _) result =
  let* program = type_file "./contracts/loop11.ligo" in
  let input = e_unit () in
  let* () =
...
let loop12 () : (unit, _) result =
  let* program = type_file "./contracts/loop12.ligo" in
  let input = e_unit () in
  let* () =
...
let loop13 () : (unit, _) result =
  let* program = type_file "./contracts/loop13.ligo" in
  let input = e_unit () in
  let* () =
...
let loop14 () : (unit, _) result =
  let* program = type_file "./contracts/loop14.ligo" in
  let input = e_unit () in
  let* () =
...
let loop15 () : (unit, _) result =
  let* program = type_file "./contracts/loop15.ligo" in
  let input = e_unit () in
  let* () =
...
let loop16 () : (unit, _) result =
  let* program = type_file "./contracts/loop16.ligo" in
  let input = e_unit () in
  let* () =
    let expected = e_pair (e_int 24)
...
let loop17 () : (unit, _) result =
  let* program = type_file "./contracts/loop.ligo" in
  let input = e_unit () in
  let* () =
...
let loop_mligo () : (unit, _) result =
  let* program = type_file "./contracts/loop.mligo" in
  let* () =
    let input = e_int 0 in
...
let loop_religo () : (unit, _) result =
  let* program = type_file "./contracts/loop.religo" in
  let* () =
    let input = e_int 0 in
    let expected = e_int 100 in
    expect_eq program "counter_simple" input expected
...
let loop_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/loop.jsligo" in
  let* () =
    let input = e_int 0 in
    let expected = e_int 100 in
    expect_eq program "counter_simple" input expected
...
let loop2_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/loop2.jsligo" in
  (* let* () =
    let make_input = e_nat in
    let make_expected = e_nat in
    expect_eq_n_pos program "dummy" make_input make_expected in *)
...
let matching () : (unit, _) result =
  let* program = type_file "./contracts/match.ligo" in
  let* () =
    let make_input = e_int in
    let make_expected = fun n -> e_int (if n = 2 then 42 else 0) in
...
let declarations () : (unit, _) result =
  let* program = type_file "./contracts/declarations.ligo" in
  let make_input = e_int in
...
let declaration_local () : (unit, _) result =
  let* program = type_file "./contracts/declaration-local.ligo" in
  let make_input = e_int in
  let make_expected = fun _ -> e_int 42 in
  expect_eq_n program "main" make_input make_expected

let quote_declaration () : (unit, _) result =
  let* program = type_file "./contracts/quote-declaration.ligo" in
  let make_input = e_int in
  let make_expected = fun n -> e_int (42 + 2 * n) in
  expect_eq_n program "main" make_input make_expected

let quote_declarations () : (unit, _) result =
  let* program = type_file "./contracts/quote-declarations.ligo" in
  let make_input = e_int in
  let make_expected = fun n -> e_int (74 + 2 * n) in
  expect_eq_n program "main" make_input make_expected

let counter_contract () : (unit, _) result =
  let* program = type_file "./contracts/counter.ligo" in
...
let super_counter_contract () : (unit, _) result =
  let* program = type_file "./contracts/super-counter.ligo" in
  let make_input = fun n ->
...
let super_counter_contract_mligo () : (unit, _) result =
  let* program = type_file "./contracts/super-counter.mligo" in
  let make_input = fun n ->
    let action = if n mod 2 = 0 then "Increment" else "Decrement" in
    e_pair (e_constructor action (e_int n)) (e_int 42) in
  let make_expected = fun n ->
...
let super_counter_contract_religo () : (unit, _) result =
  let* program = type_file "./contracts/super-counter.religo" in
  let make_input = fun n ->
    let action = if n mod 2 = 0 then "Increment" else "Decrement" in
    e_pair (e_constructor action (e_int n)) (e_int 42) in
  let make_expected = fun n ->
...
let super_counter_contract_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/super-counter.jsligo" in
  let make_input = fun n ->
    let action = if n mod 2 = 0 then "Increment" else "Decrement" in
    e_pair (e_constructor action (e_int n)) (e_int 42) in
  let make_expected = fun n ->
...
let dispatch_counter_contract () : (unit, _) result =
  let* program = type_file "./contracts/dispatch-counter.ligo" in
  let make_input = fun n ->
    let action = if n mod 2 = 0 then "Increment" else "Decrement" in
    e_pair (e_constructor action (e_int n)) (e_int 42) in
  let make_expected = fun n ->
...
let failwith_ligo () : (unit, _) result =
  let* program = type_file "./contracts/failwith.ligo" in
...
let failwith_mligo () : (unit, _) result =
  let* program = type_file "./contracts/failwith.mligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  expect_fail program "main" make_input

let failwith_religo () : (unit, _) result =
  let* program = type_file "./contracts/failwith.religo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  expect_fail program "main" make_input

let failwith_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/failwith.jsligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  expect_fail program "main" make_input

let assert_mligo () : (unit, _) result =
  let* program = type_file "./contracts/assert.mligo" in
...
let assert_religo () : (unit, _) result =
  let* program = type_file "./contracts/assert.religo" in
  let make_input b = e_pair (e_bool b) (e_unit ()) in
  let make_expected = e_pair (e_typed_list [] (t_operation())) (e_unit ()) in
  let* _ = expect_fail program "main" (make_input false) in
  let* _ = expect_eq program "main" (make_input true) make_expected in
...
let assert_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/assert.jsligo" in
  let make_input b = e_pair (e_bool b) (e_unit ()) in
  let make_expected = e_pair (e_typed_list [] (t_operation())) (e_unit ()) in
  let* _ = expect_fail program "main" (make_input false) in
  let* _ = expect_eq program "main" (make_input true) make_expected in
...
let recursion_ligo () : (unit, _) result =
  let* program = type_file "./contracts/recursion.ligo" in
  let* _ =
...
let recursion_mligo () : (unit, _) result =
  let* program = type_file "./contracts/recursion.mligo" in
  let* _ =
    let make_input = e_pair (e_int 10) (e_int 0) in
    let make_expected = e_int 55 in
    expect_eq program "sum" make_input make_expected
...
let recursion_religo () : (unit, _) result =
  let* program = type_file "./contracts/recursion.religo" in
  let* _ =
    let make_input = e_pair (e_int 10) (e_int 0) in
    let make_expected = e_int 55 in
    expect_eq program "sum" make_input make_expected
...
let recursion_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/recursion.jsligo" in
  let* _ =
    let make_input = e_pair (e_int 10) (e_int 0) in
    let make_expected = e_int 55 in
    expect_eq program "sum" make_input make_expected
...
let guess_string_mligo () : (unit, _) result =
  let* program = type_file "./contracts/guess_string.mligo" in
  let make_input = fun n -> e_pair (e_int n) (e_int 42) in
...
let basic_mligo () : (unit, _) result =
  let* typed = type_file "./contracts/basic.mligo" in
  expect_eq_evaluate typed "foo" (e_int (42+127))

let basic_religo () : (unit, _) result =
  let* typed = type_file "./contracts/basic.religo" in
  expect_eq_evaluate typed "foo" (e_int (42+127))

let counter_mligo () : (unit, _) result =
  let* program = type_file "./contracts/counter.mligo" in
...
let counter_religo () : (unit, _) result =
  let* program = type_file "./contracts/counter.religo" in
  let make_input n = e_pair (e_int n) (e_int 42) in
  let make_expected n = e_pair (e_typed_list [] (t_operation ())) (e_int (42 + n)) in
  expect_eq_n program "main" make_input make_expected

let counter_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/counter.jsligo" in
  let make_input n = e_pair (e_int n) (e_int 42) in
  let make_expected n = e_pair (e_typed_list [] (t_operation ())) (e_int (42 + n)) in
  expect_eq_n program "main" make_input make_expected

let let_in_mligo () : (unit, _) result =
  let* program = type_file "./contracts/letin.mligo" in
  let* () =
...
let let_in_religo () : (unit, _) result =
  let* program = type_file "./contracts/letin.religo" in
  let* () =
    let make_input n = e_pair (e_int n) (e_pair (e_int 3) (e_int 5)) in
    let make_expected n =
      e_pair (e_typed_list [] (t_operation ())) (e_pair (e_int (7+n)) (e_int (3+5)))
...
let let_in_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/letin.jsligo" in
  let* () =
    let make_input n = e_pair (e_int n) (e_pair (e_int 3) (e_int 5)) in
    let make_expected n =
      e_pair (e_typed_list [] (t_operation ())) (e_pair (e_int (7+n)) (e_int (3+5)))
...
let local_type_decl_ligo () : (unit, _) result =
  let* program = type_file "./contracts/local_type_decl.ligo" in
  local_type_decl program

let local_type_decl_mligo () : (unit, _) result =
  let* program = type_file "./contracts/local_type_decl.mligo" in
  local_type_decl program

let local_type_decl_religo () : (unit, _) result =
  let* program = type_file "./contracts/local_type_decl.religo" in
  local_type_decl program

let local_type_decl_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/local_type_decl.jsligo" in
  local_type_decl program  

let match_variant () : (unit, _) result =
  let* program = type_file "./contracts/match.mligo" in
  let* () =
    let make_input n =
...
let match_variant_re () : (unit, _) result =
  let* program = type_file "./contracts/match.religo" in
  let make_input n =
    e_pair (e_constructor "Sub" (e_int n)) (e_int 3) in
  let make_expected n =
    e_pair (e_typed_list [] (t_operation ())) (e_int (3-n))
...
let match_variant_js () : (unit, _) result =
  let* program = type_file "./contracts/match.jsligo" in
  let make_input n =
    e_pair (e_constructor "Sub" (e_int n)) (e_int 3) in
  let make_expected n =
    e_pair (e_typed_list [] (t_operation ())) (e_int (3-n))
...
let match_matej () : (unit, _) result =
  let* program = type_file "./contracts/match_bis.mligo" in
  let make_input n =
...
let match_matej_re () : (unit, _) result =
  let* program = type_file "./contracts/match_bis.religo" in
  let make_input n =
    e_pair (e_constructor "Decrement" (e_int n)) (e_int 3) in
  let make_expected n =
    e_pair (e_typed_list [] (t_operation ())) (e_int (3-n))
...
let match_matej_js () : (unit, _) result =
  let* program = type_file "./contracts/match_bis.jsligo" in
  let make_input n =
    e_pair (e_constructor "Decrement" (e_int n)) (e_int 3) in
  let make_expected n =
    e_pair (e_typed_list [] (t_operation ())) (e_int (3-n))
...
let lambda_mligo () : (unit, _) result =
  let* program = type_file "./contracts/lambda.mligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  let make_expected = (e_unit ()) in
  expect_eq program "main" make_input make_expected

let lambda_religo () : (unit, _) result =
  let* program = type_file "./contracts/lambda.religo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  let make_expected = (e_unit ()) in
  expect_eq program "main" make_input make_expected

let lambda_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/lambda.jsligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  let make_expected = (e_unit ()) in
  expect_eq program "main" make_input make_expected

let lambda_ligo () : (unit, _) result =
  let* program = type_file "./contracts/lambda.ligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  let make_expected = (e_unit ()) in
  expect_eq program "main" make_input make_expected

let lambda2_mligo () : (unit, _) result =
  let* program = type_file "./contracts/lambda2.mligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  let make_expected = (e_unit ()) in
  expect_eq program "main" make_input make_expected

let lambda2_religo () : (unit, _) result =
  let* program = type_file "./contracts/lambda2.religo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  let make_expected = (e_unit ()) in
  expect_eq program "main" make_input make_expected

let lambda2_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/lambda2.jsligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
  let make_expected = (e_unit ()) in
  expect_eq program "main" make_input make_expected
...
let fibo_mligo () : (unit, _) result =
  let* program = type_file "./contracts/fibo.mligo" in
  let make_input = e_pair (e_unit ()) (e_unit ()) in
...
let tez_ligo () : (unit, _) result =
  let* program = type_file "./contracts/tez.ligo" in
...
let tez_mligo () : (unit, _) result =
  let* program = type_file "./contracts/tez.mligo" in
  let* _ = expect_eq_evaluate program "add_tez" (e_mutez 42) in
  let* _ = expect_eq_evaluate program "sub_tez" (e_mutez 1) in
  let* _ = expect_eq_evaluate program "not_enough_tez" (e_mutez 4611686018427387903) in
...
let website2_mligo () : (unit, _) result =
  let* program = type_file "./contracts/website2.mligo" in
  let make_input = fun n ->
    let action = if n mod 2 = 0 then "Increment" else "Decrement" in
    e_pair (e_constructor action (e_int n)) (e_int 42) in
  let make_expected = fun n ->
...
let website2_religo () : (unit, _) result =
  let* program = type_file "./contracts/website2.religo" in
  let make_input = fun n ->
    let action = if n mod 2 = 0 then "Increment" else "Decrement" in
    e_pair (e_constructor action (e_int n)) (e_int 42) in
  let make_expected = fun n ->
...
let website2_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/website2.jsligo" in
  let make_input = fun n ->
    let action = if n mod 2 = 0 then "Increment" else "Decrement" in
    e_pair (e_constructor action (e_int n)) (e_int 42) in
  let make_expected = fun n ->
...
let mligo_let_multiple () : (unit, _) result =
  let* program = type_file "./contracts/let_multiple.mligo" in
  let* () =
    let input = e_unit () in
    let expected = e_int 3 in
    expect_eq program "main" input expected
...
let religo_let_multiple () : (unit, _) result =
  let* program = type_file "./contracts/let_multiple.religo" in
  let* () =
    let input = e_unit () in
    let expected = e_int 3 in
    expect_eq program "main" input expected
...
let jsligo_let_multiple () : (unit, _) result =
  let* program = type_file "./contracts/let_multiple.jsligo" in
  let* () =
    let input = e_unit () in
    let expected = e_int 3 in
    expect_eq program "main" input expected
...
let balance_constant () : (unit, _) result =
  let* program = type_file "./contracts/balance_constant.ligo" in
...
let balance_constant_mligo () : (unit, _) result =
  let* program = type_file "./contracts/balance_constant.mligo" in
  let input = e_tuple [e_unit () ; e_mutez 0]  in
  let expected = e_tuple [e_list []; e_mutez 4000000000000] in
  let* options = balance_test_options () in
  expect_eq ~options program "main" input expected
...
let balance_constant_religo () : (unit, _) result =
  let* program = type_file "./contracts/balance_constant.religo" in
  let input = e_tuple [e_unit () ; e_mutez 0]  in
  let expected = e_tuple [e_list []; e_mutez 4000000000000] in
  let* options = balance_test_options () in
  expect_eq ~options program "main" input expected
...
let balance_constant_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/balance_constant.jsligo" in
  let input = e_tuple [e_unit () ; e_mutez 0]  in
  let expected = e_tuple [e_list []; e_mutez 4000000000000] in
  let* options = balance_test_options () in
  expect_eq ~options program "main" input expected
...
let amount () : (unit, _) result =
  let* program = type_file "./contracts/amount.ligo" in
  let input = e_unit () in
  let expected = e_int 42 in
  let amount =
...
let amount_mligo () : (unit, _) result =
  let* program = type_file "./contracts/amount.mligo" in
  let input = e_unit () in
  let expected = e_int 42 in
  let amount =
    match Memory_proto_alpha.Protocol.Alpha_context.Tez.of_string "100" with
...
let amount_religo () : (unit, _) result =
  let* program = type_file "./contracts/amount.religo" in
  let input = e_unit () in
  let expected = e_int 42 in
  let amount =
    match Memory_proto_alpha.Protocol.Alpha_context.Tez.of_string "100" with
...
let amount_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/amount.jsligo" in
  let input = e_unit () in
  let expected = e_int 42 in
  let amount =
    match Memory_proto_alpha.Protocol.Alpha_context.Tez.of_string "100" with
...
let address () : (unit, _) result =
  let* program = type_file "./contracts/address.ligo" in
  addr_test program

let address_mligo () : (unit, _) result =
  let* program = type_file "./contracts/address.mligo" in
  addr_test program

let address_religo () : (unit, _) result =
  let* program = type_file "./contracts/address.religo" in
  addr_test program

let address_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/address.jsligo" in
  addr_test program
  

let self_address () : (unit, _) result =
  let* _ = type_file "./contracts/self_address.ligo" in
  ok ()

let self_address_mligo () : (unit, _) result =
  let* _ = type_file "./contracts/self_address.mligo" in
  ok ()

let self_address_religo () : (unit, _) result =
  let* _ = type_file "./contracts/self_address.religo" in
  ok ()

let self_address_jsligo () : (unit, _) result =
  let* _ = type_file "./contracts/self_address.jsligo" in
  ok ()

let implicit_account () : (unit, _) result =
  let* _ = type_file "./contracts/implicit_account.ligo" in
  ok ()

let implicit_account_mligo () : (unit, _) result =
  let* _ = type_file "./contracts/implicit_account.mligo" in
  ok ()


let implicit_account_religo () : (unit, _) result =
  let* _ = type_file "./contracts/implicit_account.religo" in
  ok ()

let implicit_account_jsligo () : (unit, _) result =
  let* _ = type_file "./contracts/implicit_account.jsligo" in
  ok ()
  

let tuples_sequences_functions_religo () : (unit, _) result =
  let* _ = type_file "./contracts/tuples_sequences_functions.religo" in
  ok ()

let tuples_sequences_functions_jsligo () : (unit, _) result =
  let* _ = type_file "./contracts/tuples_sequences_functions.jsligo" in
  ok ()
  

let is_nat () : (unit, _) result =
  let* program = type_file "./contracts/isnat.ligo" in
  let* () =
...
let is_nat_mligo () : (unit, _) result =
  let* program = type_file "./contracts/isnat.mligo" in
  let* () =
    let input = e_int 10 in
    let expected = e_some (e_nat 10) in
    expect_eq program "main" input expected
...
let is_nat_religo () : (unit, _) result =
  let* program = type_file "./contracts/isnat.religo" in
  let* () =
    let input = e_int 10 in
    let expected = e_some (e_nat 10) in
    expect_eq program "main" input expected
...
let is_nat_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/isnat.jsligo" in
  let* () =
    let input = e_int 10 in
    let expected = e_some (e_nat 10) in
    expect_eq program "main" input expected
...
let simple_access_ligo () : (unit, _) result =
  let* program = type_file "./contracts/simple_access.ligo" in
  let make_input = e_tuple [e_int 0; e_int 1] in
  let make_expected = e_int 2 in
  expect_eq program "main" make_input make_expected

let deep_access_ligo () : (unit, _) result =
  let* program = type_file "./contracts/deep_access.ligo" in
  let* () =
...
let attributes_ligo () : (unit, _) result =
  let* program = type_file "./contracts/attributes.ligo" in
  let* () =
...
let attributes_mligo () : (unit, _) result =
  let* program = type_file "./contracts/attributes.mligo" in
  let* () =
    let input = e_int 3 in
    let expected = e_int 5 in
    expect_eq program "foo" input expected
...
let attributes_religo () : (unit, _) result =
  let* program = type_file "./contracts/attributes.religo" in
  let* () =
    let input = e_int 3 in
    let expected = e_int 5 in
    expect_eq program "foo" input expected
...
let attributes_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/attributes.jsligo" in
  let* () =
    let input = e_int 3 in
    let expected = e_int 5 in
    expect_eq program "foo" input expected
...
let get_contract_ligo () : (unit, _) result =
  let* program = type_file "./contracts/get_contract.ligo" in
  let* () =
...
let entrypoints_ligo () : (unit, _) result =
  let* _program = type_file "./contracts/entrypoints.ligo" in
  (* hmm... *)
  ok ()

let simple1 () : (unit,_) result =
  let* program = type_file "./contracts/simple1.ligo" in
  expect_eq_evaluate program "i" (e_int 42)

let simple2 () : (unit,_) result =
  let* program = type_file "./contracts/simple2.ligo" in
  expect_eq_evaluate program "i" (e_int 42)

let simple3 () : (unit,_) result =
  let* program = type_file "./contracts/simple3.ligo" in
  expect_eq_evaluate program "my_address" (e_address "tz1KqTpEZ7Yob7QbPE4Hy4Wo8fHG8LhKxZSx")

let simple4 () : (unit,_) result =
  let* program = type_file "./contracts/simple4.ligo" in
  expect_eq_evaluate program "my_string_option" (e_string "hello")

let chain_id () : (unit, _) result =
  let* program = type_file "./contracts/chain_id.ligo" in
...
let key_hash () : (unit, _) result =
  ...
  let* program = type_file "./contracts/key_hash.ligo" in
...
let key_hash_mligo () : (unit, _) result =
  ...
  let pkh_str = Signature.Public_key_hash.to_b58check raw_pkh in
  let pk_str = Signature.Public_key.to_b58check raw_pk in
  let* program = type_file "./contracts/key_hash.mligo" in
  let make_input = e_pair (e_key_hash pkh_str) (e_key pk_str) in
  let make_expected = e_pair (e_bool true) (e_key_hash pkh_str) in
  let* () = expect_eq program "check_hash_key" make_input make_expected in
  ok ()
...
let key_hash_religo () : (unit, _) result =
  ...
  let pkh_str = Signature.Public_key_hash.to_b58check raw_pkh in
  let pk_str = Signature.Public_key.to_b58check raw_pk in
  let* program = type_file "./contracts/key_hash.religo" in
  let make_input = e_pair (e_key_hash pkh_str) (e_key pk_str) in
  let make_expected = e_pair (e_bool true) (e_key_hash pkh_str) in
  let* () = expect_eq program "check_hash_key" make_input make_expected in
  ok ()
...
let key_hash_jsligo () : (unit, _) result =
  ...
  let pkh_str = Signature.Public_key_hash.to_b58check raw_pkh in
  let pk_str = Signature.Public_key.to_b58check raw_pk in
  let* program = type_file "./contracts/key_hash.jsligo" in
  let make_input = e_pair (e_key_hash pkh_str) (e_key pk_str) in
  let make_expected = e_pair (e_bool true) (e_key_hash pkh_str) in
  let* () = expect_eq program "check_hash_key" make_input make_expected in
  ok ()
...
let check_signature () : (unit, _) result =
  ...
  let* program = type_file "./contracts/check_signature.ligo" in
...
let check_signature_mligo () : (unit, _) result =
  ...
  let pk_str = Signature.Public_key.to_b58check raw_pk in
  let signed = Signature.sign sk (Bytes.of_string "hello world") in
  let* program = type_file "./contracts/check_signature.mligo" in
  let make_input = e_tuple [e_key pk_str ;
                            e_signature (Signature.to_b58check signed) ;
                            e_bytes_string "hello world"] in
  let make_expected = e_bool true in
...
let check_signature_religo () : (unit, _) result =
  ...
  let pk_str = Signature.Public_key.to_b58check raw_pk in
  let signed = Signature.sign sk (Bytes.of_string "hello world") in
  let* program = type_file "./contracts/check_signature.religo" in
  let make_input = e_tuple [e_key pk_str ;
                            e_signature (Signature.to_b58check signed) ;
                            e_bytes_string "hello world"] in
  let make_expected = e_bool true in
...
let check_signature_jsligo () : (unit, _) result =
  ...
  let pk_str = Signature.Public_key.to_b58check raw_pk in
  let signed = Signature.sign sk (Bytes.of_string "hello world") in
  let* program = type_file "./contracts/check_signature.jsligo" in
  let make_input = e_tuple [e_key pk_str ;
                            e_signature (Signature.to_b58check signed) ;
                            e_bytes_string "hello world"] in
  let make_expected = e_bool true in
...
let curry () : (unit, _) result =
  let* program = type_file "./contracts/curry.mligo" in
  let* () =
...
let set_delegate () : (unit, _) result =
  ...
  let pkh_str = Signature.Public_key_hash.to_b58check raw_pkh in
  let* program = type_file "./contracts/set_delegate.ligo" in
...
let set_delegate_mligo () : (unit, _) result =
  ...
  let (raw_pkh,_,_) = Signature.generate_key () in
  let pkh_str = Signature.Public_key_hash.to_b58check raw_pkh in
  let* program = type_file "./contracts/set_delegate.mligo" in
  let* () = expect_eq program "main" (e_key_hash pkh_str) (e_typed_list [] (t_operation ()))
  in ok ()
...
let set_delegate_religo () : (unit, _) result =
  ...
  let (raw_pkh,_,_) = Signature.generate_key () in
  let pkh_str = Signature.Public_key_hash.to_b58check raw_pkh in
  let* program = type_file "./contracts/set_delegate.religo" in
  let* () = expect_eq program "main" (e_key_hash pkh_str) (e_typed_list [] (t_operation ()))
  in ok ()
...
let set_delegate_jsligo () : (unit, _) result =
  ...
  let (raw_pkh,_,_) = Signature.generate_key () in
  let pkh_str = Signature.Public_key_hash.to_b58check raw_pkh in
  let* program = type_file "./contracts/set_delegate.jsligo" in
  let* () = expect_eq program "main" (e_key_hash pkh_str) (e_typed_list [] (t_operation ()))
  in ok ()
  

let type_tuple_destruct () : (unit, _) result =
  let* program = type_file "./contracts/type_tuple_destruct.mligo" in
...
let tuple_param_destruct () : (unit, _) result =
  let* program = type_file "./contracts/tuple_param_destruct.mligo" in
...
let tuple_param_destruct_religo () : (unit, _) result =
  let* program = type_file "./contracts/tuple_param_destruct.religo" in
  let* () = expect_eq program "sum" (e_tuple [e_int 20; e_int 10]) (e_int 10) in
  let* () = expect_eq program "parentheses" (e_tuple [e_int 20; e_int 10]) (e_int 10) in
  ok ()

let let_in_multi_bind () : (unit, _) result =
  let* program = type_file "./contracts/let_in_multi_bind.mligo" in
...
let bytes_unpack () : (unit, _) result =
  let* program = type_file "./contracts/bytes_unpack.ligo" in
...
let bytes_unpack_mligo () : (unit, _) result =
  let* program = type_file "./contracts/bytes_unpack.mligo" in
  let* () = expect_eq program "id_string" (e_string "teststring") (e_some (e_string "teststring")) in
  let* () = expect_eq program "id_int" (e_int 42) (e_some (e_int 42)) in
  let open Proto_alpha_utils.Memory_proto_alpha in
  let addr = Protocol.Alpha_context.Contract.to_b58check @@
...
let bytes_unpack_religo () : (unit, _) result =
  let* program = type_file "./contracts/bytes_unpack.religo" in
  let* () = expect_eq program "id_string" (e_string "teststring") (e_some (e_string "teststring")) in
  let* () = expect_eq program "id_int" (e_int 42) (e_some (e_int 42)) in
  let open Proto_alpha_utils.Memory_proto_alpha in
  let addr = Protocol.Alpha_context.Contract.to_b58check @@
...
let bytes_unpack_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/bytes_unpack.jsligo" in
  let* () = expect_eq program "id_string" (e_string "teststring") (e_some (e_string "teststring")) in
  let* () = expect_eq program "id_int" (e_int 42) (e_some (e_int 42)) in
  let open Proto_alpha_utils.Memory_proto_alpha in
  let addr = Protocol.Alpha_context.Contract.to_b58check @@
...
let empty_case () : (unit, _) result =
  let* program = type_file "./contracts/empty_case.ligo" in
  let* () =
...
let empty_case_mligo () : (unit, _) result =
  let* program = type_file "./contracts/empty_case.mligo" in
  let* () =
    let input _ = e_constructor "Bar" (e_int 1) in
    let expected _ = e_int 1 in
    expect_eq_n program "main" input expected
...
let empty_case_religo () : (unit, _) result =
  let* program = type_file "./contracts/empty_case.religo" in
  let* () =
    let input _ = e_constructor "Bar" (e_int 1) in
    let expected _ = e_int 1 in
    expect_eq_n program "main" input expected
...
let empty_case_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/empty_case.jsligo" in
  let* () =
    let input _ = e_constructor "Bar" (e_int 1) in
    let expected _ = e_int 1 in
    expect_eq_n program "main" input expected
...
let tuple_type_mligo () : (unit, _) result =
  let* program = type_file "./contracts/tuple_type.mligo" in
  let* () =
...
let tuple_type_religo () : (unit, _) result =
  let* program = type_file "./contracts/tuple_type.religo" in
  let* () =
    let input _ = e_int 0 in
    let expected _ = e_int 8 in
...
let tuple_type_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/tuple_type.jsligo" in
  let* () =
    let input _ = e_int 0 in
    let expected _ = e_int 8 in
    expect_eq_n program "arguments_test" input expected
...
let no_semicolon_religo () : (unit, _) result =
  let* program = type_file "./contracts/no_semicolon.religo" in
  let* () =
...
let no_semicolon_jsligo () : (unit, _) result =
  let* program = type_file "./contracts/no_semicolon.jsligo" in
  let* () =
    let input _ = e_int 2 in
    let expected _ = e_int 3 in
    expect_eq_n program "a" input expected
...
let tuple_list_religo () : (unit, _) result =
  let* _ = type_file "./contracts/tuple_list.religo" in
  ok ()

let tuple_list_jsligo () : (unit, _) result =
  let* _ = type_file "./contracts/tuple_list.jsligo" in
  ok ()

let single_record_expr_religo () : (unit, _) result =
  let* _ = type_file "./contracts/single_record_item.religo" in
  ok ()

let loop_bugs_ligo () : (unit, _) result =
  let* program = type_file "./contracts/loop_bugs.ligo" in
  let input = e_unit () in
  let* () =
