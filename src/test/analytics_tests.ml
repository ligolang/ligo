...
let test_compile_contract ~raise:_ () =
  let expected_registry_output_regex =
    Str.regexp
      ".*ligo_compile_compilation_size{repository=\"[a-z0-9-]+\", version=\"\", project=\"[0-9]+\", \
       syntax=\"jsligo\", protocol=\"[a-z]+\"} [0-9.]+.*"
  in
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", \
       ...
       repository=\"[a-z0-9-]+\", version=\"\", command=\"compile_contract\", syntax=\"JsLIGO\", \
       protocol=\"[a-z]+\"} 1.000000.*"
  in
...
let test_compile_expression ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"compile_expression\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"compile_expression\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_compile_parameter ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"compile_parameter\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"compile_parameter\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
  let _ =
    run_ligo
...
let test_compile_storage ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"compile_storage\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"compile_storage\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_run_dry_run ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"run_dry-run\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"run_dry-run\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
  let _ = run_ligo [ "run"; "dry-run"; "contracts/super-counter.mligo"; "test_param"; "test_storage" ] in
  check_analytics expected_registry_output_regex expected_agg_registry_content_regex
...
let test_run_evaluate_call ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"run_evaluate-call\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"run_evaluate-call\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
  let _ = run_ligo [ "run"; "evaluate-call"; "contracts/assert.mligo"; "with_error"; "(false, ())" ] in
  check_analytics expected_registry_output_regex expected_agg_registry_content_regex
...
let test_run_interpret ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"run_interpret\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"run_interpret\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
  let _ = run_ligo [ "run"; "interpret";  "t1 (Cons(1,2),Nil)"; "--init-file"; "contracts/deep_pattern_matching/pm_test.mligo" ] in
  check_analytics expected_registry_output_regex expected_agg_registry_content_regex
...
let test_run_test ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"run_test\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"run_test\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_run_test_expr ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"run_test-expr\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"run_test-expr\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"info_get-scope\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"info_get-scope\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
  let _ = run_ligo [ "info"; "get-scope"; "contracts/get_scope_tests/constant.mligo" ; "--format"; "dev"] in
  check_analytics expected_registry_output_regex expected_agg_registry_content_regex
...
let test_info_list_declaration ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"info_list-declarations\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"info_list-declarations\", syntax=\"JsLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_info_measure_contract ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"info_measure-contract\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"info_measure-contract\", syntax=\"JsLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_mutate_cst ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"mutate_cst\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"mutate_cst\", syntax=\"JsLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_changelog ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"changelog\"} \
       1.*"
  in
...
let test_print_pretty ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"print_pretty\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"print_pretty\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_print_preprocessed ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"print_preprocessed\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"print_preprocessed\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_print_cst ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"print_cst\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"print_cst\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_print_ast_core ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"print_ast-core\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"print_ast-core\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_print_dependency_graph ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"print_dependency-graph\"} \
       1.*ligo_cli_execution_by_syntax_and_protocol{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"print_dependency-graph\", syntax=\"CameLIGO\", \
       protocol=\"[a-z]+\"} 1.*"
  in
...
let test_transpile_contract ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"transpile_contract\"} \
       1.*ligo_cli_transpile{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"transpile_contract\", old_syntax=\"PascaLIGO\", \
       new_syntax=\"jsligo\"} 1.*"
  in
  let _ = run_ligo [ "transpile"; "contract"; "contracts/example.ligo"; "--to-syntax"; "jsligo" ] in
  check_analytics expected_registry_output_regex expected_agg_registry_content_regex
...
let test_init_contract ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"init_contract\"} \
       1.*ligo_cli_init{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"init_contract\", template=\"advisor-cameligo\"} 1.*"
  in
...
let test_init_library ~raise:_ () =
  ...
  let expected_agg_registry_content_regex =
    Str.regexp
      ".*ligo_cli_command{user=\"[a-z0-9-]+\", repository=\"[a-z0-9-]+\", version=\"\", command=\"init_library\"} \
       1.*ligo_cli_init{user=\"[a-z0-9-]+\", \
       repository=\"[a-z0-9-]+\", version=\"\", command=\"init_library\", template=\"ligo-fa\"} 1.*"
  in
...
let main =
  test_suite
    ...
    ; test "mutate cst " test_mutate_cst
    (* ; test "repl " test_mutate_cst *)
