...
let empty_message = e_lambda_ez (Var.of_name "arguments")
  ...
  empty_op_list
let empty_message2 = e_lambda_ez (Var.of_name "arguments")
  ~ascr:(t_bytes ()) (Some (t_list (t_operation ())))
 ( e_let_in_ez (Var.of_name "foo") ~ascr:(t_unit ()) [] (e_unit ()) empty_op_list)
...
let succeeded_storing ~raise ~add_warning () =
  ...
  let options =
    let sender = contract 1 in
    Proto_alpha_utils.Memory_proto_alpha.(make_options ~env:(test_environment ()) ~sender ()) in
  let () = expect_eq_n_trace_aux ~raise ~options [1;2] program "main"
      (fun th ->
