...
let get_program =
  ...
  fun () -> match !s with
    ...
    | None -> (
      ...
      let%bind program = Ligo.Compile.Utils.type_file ~init_env "./contracts/id.mligo" "cameligo" (Contract "main") in
      s := Some program ;
      ok program
    )
...
let buy_id_wrong_amount () =
  ...
  let%bind () = expect_string_failwith ~options (program, env, state) "buy" 
      (e_pair param storage)
...
let update_details_owner () =
  ...
  let%bind () = expect_eq ~options (program, env, state) "update_details"
      (e_pair param storage)
      (e_pair (e_list []) new_storage)
  in ok ()
...
let update_details_nonexistent () = 
  ...
  let param = e_record_ez [("id", e_int 2) ;
                           ("new_profile", e_some details) ;
                           ("new_controller", e_some (e_address owner_addr))] in
  let%bind () = expect_string_failwith ~options (program, env, state) "update_details"
      (e_pair param storage)
...
let update_owner () =
  ...
  let%bind () = expect_eq ~options (program, env, state) "update_owner"
      (e_pair param storage)
      (e_pair (e_list []) new_storage)
  in ok ()
...
let update_owner_nonexistent () =
  ...
  let%bind () = expect_string_failwith ~options (program, env, state) "update_owner"
      (e_pair param storage)
      "This ID does not exist."
  in ok ()
...
let skip () =
  ...
  let new_storage = e_record_ez [("identities", (e_big_map
                             ...
                             ("skip_price", e_mutez 1000000) ; ]
  in
  let%bind () = expect_eq ~options (program, env, state) "skip"
...
let skip_wrong_amount () =
  ...
  let storage = e_record_ez [("identities", (e_big_map
                             ...
                             ("skip_price", e_mutez 1000000) ; ]
  in
  let%bind () = expect_string_failwith ~options (program, env, state) "skip"
      (e_pair (e_unit ()) storage)
      "Incorrect amount paid."
  in ok ()
