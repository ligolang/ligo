...
function send (const param : send_pt; var s : storage) : return is
  block {
      ...
      Some (voters) ->
        block {
          ...
          then skip
                 ...
                 Map.find (Tezos.get_sender(unit), s.proposal_counters) + 1n;
                 new_store := Set.add (Tezos.get_sender(unit),voters)
        }
    | None ->
        block {
          // the message has never been received before
          s.proposal_counters[Tezos.get_sender(unit)] :=
             Map.find (Tezos.get_sender(unit), s.proposal_counters) + 1n;
    ...
    var sender_proposal_counter : nat :=
      Map.find (Tezos.get_sender(unit), s.proposal_counters);
      ...
      Some (voters) ->
        block {
          ...
          then s.proposal_counters[Tezos.get_sender(unit)] :=
                 abs (Map.find (Tezos.get_sender(unit), s.proposal_counters) - 1n)
          else skip;
