...
function buy (const parameter : buy; const storage : storage) : list(operation) * storage is
  begin
    if Tezos.get_amount(unit) = storage.name_price
    then skip
...
function update_owner (const parameter : update_owner; const storage : storage) :
  ...
  begin
    if (Tezos.get_amount(unit) =/= 0mutez)
    then
      begin
    ...
    if Tezos.get_sender(unit) =/= id_details.owner then failwith("You are not the owner of this ID.");
...
function update_details (const parameter : update_details; const storage : storage ) :
         list(operation) * storage is
  begin
    if (Tezos.get_amount(unit) =/= 0mutez) then failwith("Updating details doesn't cost anything.") ;
    const id : int = parameter.id;
    ...
    if not ((Tezos.get_sender(unit) = id_details.controller) or (Tezos.get_sender(unit) = id_details.owner)) then failwith("You are not the owner or controller of this ID.");
...
function skip_ (const _ : unit; const storage: storage) : list(operation) * storage is
  begin
    if Tezos.get_amount(unit) =/= storage.skip_price then failwith("Incorrect amount paid.");
