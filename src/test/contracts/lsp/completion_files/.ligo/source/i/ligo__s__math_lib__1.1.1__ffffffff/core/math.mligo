...
let factorial (n : nat) : nat =
  let rec fact (acc, i : nat * nat) : nat =
    if (i < 2n)
    then acc
    else fact (acc * i, abs (i - 1n)) in
  fact (1n, n)

let min (a, b : nat * nat) : nat = if (a < b) then a else b

let max (a, b : nat * nat) : nat = if (a > b) then a else b

// TODO : optimize log algorithm !

let log_10 (x : nat) : nat =
  let rec check_power (x, i : nat * nat) : nat =
    if (x mod power (10n, i) > 0n)
    then abs (i - 1n)
    else check_power (x, i + 1n) in
  check_power (x, 1n)
