...
let check_new_origination (src :address) : address =
  ...
  match Map.find_opt src last_origs with
    | Some new_lst -> ( 
      let unt = assert (List.length new_lst = 1n) in
      match new_lst with
      | new_acc::rst -> new_acc
...
let test =
  ...
  let unt = assert (Test.michelson_equal init_storage (Test.get_storage addr)) in
  let unt = assert (size < 300) in
  let new_account1 = check_new_origination src in
  
  let param = Test.compile_expression (Some cut) [%cameligo ({| Two |} : ligo_program)] in
  let unt = Test.transfer_exn addr param 10n in
  ...
  in
  let unt = assert (Test.michelson_equal new_storage expected_new_storage) in
  ...
  | Fail x -> (
    ...
    | Rejected reject_data ->
      ...
      let unt = assert (addr = new_account2) in
      let unt = assert (addr = new_account2) in
      let unt = assert (Test.michelson_equal v (Test.compile_value 111)) in
      v
...
let test2 =
  ...
  let unt = Test.reset_state number_of_account ([1n;2n]: nat list) in
  ...
  let unt = Test.set_source bsa3 in
  let unt = Test.set_baker bsa2 in
  // some balance tests:
  let tz = fun (n:nat) ->
  ...
  in
  let unt = assert (Test.michelson_equal (Test.get_balance bsa0) (tz 1n)) in
  let unt = assert (Test.michelson_equal (Test.get_balance bsa1) (tz 2n)) in
  let unt = assert (Test.michelson_equal (Test.get_balance bsa2) (tz 4000000000000n)) in
  let unt = assert (Test.michelson_equal (Test.get_balance bsa3) (tz 4000000000000n)) in
  ()
