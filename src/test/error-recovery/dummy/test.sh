...
if [[ $# -eq 0 ]] ; then
    ...
    echo "  ./test.sh '*.mligo'                 # calculate results.csv for all files"
    echo "  ./test.sh test.mligo -v             # run calculation on the given file and print diff"
    echo "  ./test.sh 'test1.mligo test2.mligo' # calculate results.csv only for two files"
    exit 1
fi

PARSER=./../../../../_build/default/src/passes/02-parsing/cameligo/ParserMain.exe
RESULTS=results.csv
...
for f in $1; do
    ...
    $PARSER --tokens -- original/"$f"                | sed 's/^[^\ ]*://' > original/"$f".tokens
