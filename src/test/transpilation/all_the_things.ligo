...
function transfer (const p : transfer; const s: storage) : list (operation) * storage is block {
  ...
  if Tezos.get_sender(unit) = p.address_from
  then { new_allowances := s.allowances; }
  else {
    var authorized_value : nat :=
    case (Big_map.find_opt ((Tezos.get_sender(unit),p.address_from), s.allowances)) of [
        Some (value) -> value
      |  None       -> 0n
    ];
    if (authorized_value < p.value)
    then { failwith("Not Enough Allowance")}
    else { new_allowances := Big_map.update ((Tezos.get_sender(unit),p.address_from), (Some (abs(authorized_value - p.value))), s.allowances) }
  };
...
function approve (const p : approve; const s : storage) : list (operation) * storage is block {
  var previous_value : nat :=
    case Big_map.find_opt ((p.spender, Tezos.get_sender(unit)), s.allowances) of [
      Some (value) -> value
    | None -> 0n
    ];
  var new_allowances : allowances := Big_map.empty;
  ...
  then failwith ("Unsafe Allowance Change")
    ...
    new_allowances := Big_map.update ((p.spender, Tezos.get_sender(unit)), (Some (p.value)), s.allowances);
  }
...
function main (const param : parameter; const store: storage) : return is
  ((nil : list (operation)), Tezos.get_balance(unit))
type parameter is unit
...
function chain_id (const tt : chain_id) : chain_id is Tezos.get_chain_id(unit)
...
function cb (const s : storage) : return is block {
  const c : contract (unit) = get_contract (Tezos.get_sender(unit))
} with (list [Tezos.transaction (unit, 0tez, c)], s)
...
function send (const param : send_pt; var s : storage) : return is
  block {
    ...
    if not Set.mem (Tezos.get_sender(unit), s.authorized_addresses)
    ...
    case map_get (packed_msg, s.message_store) of [
      Some (voters) ->
        block {
          ...
          if Set.mem (Tezos.get_sender(unit), voters)
          then skip
          else s.proposal_counters[Tezos.get_sender(unit)] :=
                 get_force (Tezos.get_sender(unit), s.proposal_counters) + 1n;
                 new_store := Set.add (Tezos.get_sender(unit),voters)
        }
    | None ->
        block {
          ...
          s.proposal_counters[sender] :=
             get_force (Tezos.get_sender(unit), s.proposal_counters) + 1n;
             new_store := set [Tezos.get_sender(unit)]
        }
    ];
    ...
    var sender_proposal_counter : nat :=
      get_force (Tezos.get_sender(unit), s.proposal_counters);
    ...
    case s.message_store[packed_msg] of [
      Some (voters) ->
        block {
          ...
          const new_set : addr_set = Set.remove (Tezos.get_sender(unit), voters);

          (* Decrement the counter only if the sender was already
             associated with the message *)

          if Set.cardinal (voters) =/= Set.cardinal (new_set)
          then s.proposal_counters[Tezos.get_sender(unit)] :=
                 abs (get_force (Tezos.get_sender(unit), s.proposal_counters) - 1n)
          else skip;
...
function check_message (const param : check_message_pt;
  ...
  if param.counter =/= s.counter then
    ...
    const packed_payload : bytes =
      Bytes.pack ((message, param.counter, s.id, Tezos.get_chain_id(unit)));
    var valid : nat := 0n;
...
function main (const p : unit) : address is Tezos.get_self_address(unit)
