...
module TestExpressions = struct
  ...
  let application ~raise  () : unit =
    test_expression ~raise
      I.(e_application (e_lambda_ez (Var.of_name "x") ~ascr:(t_int ()) (Some (t_int ())) (e_var "x")) @@ e_int Z.one)
      O.(t_int ())

  let lambda ~raise  () : unit =
    test_expression ~raise
      I.(e_lambda_ez (Var.of_name "x") ~ascr:(t_int ()) (Some (t_int ())) (e_var "x"))
      O.(t_arrow (t_int ()) (t_int ()))

  let recursive ~raise  () : unit =
    let fun_name = Var.of_name "sum" in
    let var      = Var.of_name "n" in
  ...
  let let_in ~raise  () : unit =
    test_expression  ~raise
      I.(e_let_in_ez (Var.of_name "x") {inline=false;no_mutation=false;view=false;public=true} (e_int Z.zero) @@ e_var "x")
      O.(t_int ())

  let let_in_ascr ~raise  () : unit =
    test_expression  ~raise
      I.(e_let_in_ez (Var.of_name "x") ~ascr:(t_int ()) {inline=false;no_mutation=false;view=false;public=true} (e_int Z.zero) @@ e_var "x")
      O.(t_int ())
  ...
  let matching ~raise  () : unit =
    ...
    in
    let binder_wild : type_expression binder = {var=(Var.of_name "_");ascr=None;attributes=Stage_common.Helpers.empty_attribute} in
    let binder_x : type_expression binder = {var=(Var.of_name "x");ascr=None;attributes=Stage_common.Helpers.empty_attribute} in
    let binder_y : type_expression binder = {var=(Var.of_name "y");ascr=None;attributes=Stage_common.Helpers.empty_attribute} in
    test_expression ~raise
      ~env:(E.add_type (Var.of_name "test_t") variant_foo_bar E.empty)
