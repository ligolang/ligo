...
let parse j =
  ...
  | Error _ ->
    ...
    | Some tud -> tud
    | None -> { type_ = "git"; url = short; directory = None })
...
let%test _ =
  ...
  | Some { type_; url; directory }
    when String.(type_ = "git")
         && Option.is_none directory
         && String.(url = "https://bitbucket.org/accountname/reponame.git") -> true
  | _ -> false
...
let%test _ =
  ...
  | Some { type_; url; directory }
    when String.(type_ = "git")
         && Option.is_none directory
         && String.(url = "https://gitlab.example.com/path/repo.git") -> true
  | _ -> false
