...
module Test = struct
  ...
  let transfer (a : address) (s : michelson_program) (t : tez) : test_exec_result = [%ext ("TEST_EXTERNAL_CALL_TO_ADDRESS", a, (None : string option), s, t)]
  let transfer_exn (a : address) (s : michelson_program) (t : tez) : nat = [%ext ("TEST_EXTERNAL_CALL_TO_ADDRESS_EXN", a, (None : string option), s, t)]
  ...
  let reset_state (n : nat) (l : tez list) : unit = [%ext ("TEST_STATE_RESET", (None : timestamp option), n, l)]
  let reset_state_at (t:timestamp) (n : nat) (l : tez list) : unit = [%ext ("TEST_STATE_RESET", (Some t), n, l)]
  ...
  let bootstrap_contract (type p s) (f : p * s -> operation list * s) (s : s) (t : tez) : unit = [%ext ("TEST_BOOTSTRAP_CONTRACT", f, s, t)]
  ...
  let mutate_value (type a) (n : nat) (v : a) : (a * mutation) option = [%ext ("TEST_MUTATE_VALUE", n, v)]
  let save_mutation (s : string) (m : mutation) : string option = [%ext ("TEST_SAVE_MUTATION", s, m)]
  let mutation_test (type a b) (v : a) (f : a -> b) : (b * mutation) option = [%ext ("TEST_MUTATION_TEST", v, f)]
  let mutation_test_all (type a b) (v : a) (f : a -> b) : (b * mutation) list = [%ext ("TEST_MUTATION_TEST_ALL", v, f)]
  let run (type a b) (f : a -> b) (v : a) : michelson_program = [%ext ("TEST_RUN", f, v)]
  let decompile (type a) (m : michelson_program) : a = [%ext ("TEST_DECOMPILE", m)]
  let random (type a) (u : unit) : a = [%ext ("TEST_RANDOM", u)]
  let add_account (s : string) (k : key) : unit = [%ext ("TEST_ADD_ACCOUNT", s, k)]
  let new_account (u : unit) : string * key = [%ext ("TEST_NEW_ACCOUNT", u)]
  let baker_account (p : string * key) (o : tez option) : unit = [%ext ("TEST_BAKER_ACCOUNT", p, o)]
  ...
  let set_big_map (type a b) (i : int) (m : (a, b) big_map) : unit = [%ext ("TEST_SET_BIG_MAP", i, m)]
  let create_chest (b : bytes) (n : nat) : chest * chest_key = [%ext ("TEST_CREATE_CHEST", b, n)]
  let create_chest_key (c : chest) (n : nat) : chest_key = [%ext ("TEST_CREATE_CHEST_KEY", c, n)]
  ...
  let originate_contract (c : michelson_contract) (s : michelson_program) (t : tez) : address = [%ext ("TEST_ORIGINATE", c, s, t)]
  ...
  let originate (type p s) (f : p * s -> operation list * s) (s : s) (t : tez) : ((p, s) typed_address * michelson_contract * int) =
    ...
    (a, f, c)
  let compile_contract_from_file (fn : string) (e : string) (v : string list) : michelson_contract = [%ext ("TEST_COMPILE_CONTRACT_FROM_FILE", fn, e, v)]
  ...
  let sign (sk : string) (d : bytes) : signature = [%ext ("TEST_SIGN", sk, d)]
end