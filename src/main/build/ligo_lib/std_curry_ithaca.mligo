module Tezos = struct
  let get_contract (type a) (a : address) : (a contract) = [%ext ("CONTRACT", a)]
  let get_contract_opt (type a) (a : address) : (a contract) option = [%ext ("CONTRACT_OPT", a)]
  let get_contract_with_error (type a) (a : address) (s : string) : a contract = [%ext ("CONTRACT_WITH_ERROR", a, s)]
  (* let get_entrypoint_opt (type a) (s : string) (a : address) : (a contract) option = [%ext ("CONTRACT_ENTRYPOINT_OPT", s, a)] *)
  [@thunk]
  ...
  [@thunk]
    let self_address : address = [%ext ("SELF_ADDRESS")]
  [@thunk]
  ...
  let get_self_address (_u : unit) : address = [%ext ("SELF_ADDRESS")]
  ...
  (* [@thunk] let self (type a) (s : string) : a contract = [%ext ("SELF", s)] *)
  let voting_power (kh : key_hash) : nat = [%Michelson ({| { VOTING_POWER } |} : key_hash -> nat)] kh
  let address (type a) (c : a contract) : address = [%ext ("ADDRESS", c)]
  let implicit_account (kh : key_hash) : unit contract = [%ext ("IMPLICIT_ACCOUNT", kh)]
  ...
  (* let create_contract (type a b) (c : a * b -> operation list * b) (kh : key_hash) (mu : tez) (s : b) : operation * address = [%ext ("CREATE_CONTRACT", c, kh, mu, s)] *)
  ...
  let open_chest (ck : chest_key) (c : chest) (n : nat) : chest_opening_result = [%ext ("OPEN_CHEST", ck, c, n)]
  let call_view (type a b) (s : string) (x : a) (a : address)  : b option = [%ext ("VIEW", s, x, a)]
  let constant (type a) (s : string) : a = [%ext ("GLOBAL_CONSTANT", s)]
  let transaction (type a) (a : a) (mu : tez) (c : a contract) : operation = [%ext ("CALL", a, mu, c)]
  let set_delegate (o : key_hash option) : operation = [%ext ("SET_DELEGATE", o)]
end

module Bitwise = struct
  (* let and (type a b) (l : a) (r : b) : (a, b) external_and = [%ext ("AND", l, r)] *)
  let xor (l : nat) (r : nat) : nat = [%ext ("XOR", l, r)]
  (* let or (l : nat) (r : nat) : nat = [%ext ("OR", l, r)] *)
  let shift_left (l : nat) (r : nat) : nat = [%ext ("LSL", l, r)]
  let shift_right (l : nat) (r : nat) : nat = [%ext ("LSR", l, r)]
end

module Big_map = struct
  [@thunk] let empty (type k v) : (k, v) big_map = [%ext ("BIG_MAP_EMPTY")]
  let mem (type k v) (k : k) (m : (k, v) big_map) : bool = [%Michelson ({| { UNPAIR ; MEM } |} : k * (k, v) big_map -> bool)] (k, m)
  let add (type k v) (k : k) (v : v) (m : (k, v) big_map) : (k, v) big_map = [%ext ("MAP_ADD", k, v, m)]
  let remove (type k v) (k : k) (m : (k, v) big_map) : (k, v) big_map = [%ext ("MAP_REMOVE", k, m)]
  let update (type k v) (k : k) (v : v option) (m : (k, v) big_map) : (k, v) big_map = [%ext ("MAP_UPDATE", k, v, m)]
  let get_and_update (type k v) (k : k) (v : v option) (m : (k, v) big_map) : v option * (k, v) big_map = [%ext ("BIG_MAP_GET_AND_UPDATE", k, v, m)]
  let find_opt (type k v) (k : k) (m : (k, v) big_map) : v option = [%ext ("MAP_FIND_OPT", k, m)]
  let find (type k v) (k : k) (m : (k, v) big_map) : v = [%ext ("MAP_FIND", k, m)]
end

module Map = struct
  let empty (type k v) : (k, v) map = [%ext ("MAP_EMPTY")]
  ...
  let add (type k v) (k : k) (v : v) (m : (k, v) map) : (k, v) map = [%ext ("MAP_ADD", k, v, m)]
  let remove (type k v) (k : k) (m : (k, v) map) : (k, v) map = [%ext ("MAP_REMOVE", k, m)]
  let update (type k v) (k : k) (v : v option) (m : (k, v) map) : (k, v) map = [%ext ("MAP_UPDATE", k, v, m)]
  let get_and_update (type k v) (k : k) (v : v option) (m : (k, v) map) : v option * (k, v) map = [%ext ("MAP_GET_AND_UPDATE", k, v, m)]
  let find (type k v) (k : k) (m : (k, v) map) : v = [%ext ("MAP_FIND", k, m)]
  let find_opt (type k v) (k : k) (m : (k, v) map) : v option = [%ext ("MAP_FIND_OPT", k, m)]
  let iter (type k v) (f : k * v -> unit) (m : (k, v) map) : unit = [%ext ("MAP_ITER", f, m)]
  let map (type k v w) (f : k * v -> w) (m : (k, v) map) : (k, w) map = [%ext ("MAP_MAP", f, m)]
  let fold (type k v c) (f : c * (k * v) -> c) (m : (k, v) map) (i : c) : c = [%ext ("MAP_FOLD", f, m, i)]
end

module Set = struct
  let empty (type a) : a set = [%ext ("SET_EMPTY")]
  let mem (type a) (x : a) (s : a set) : bool = [%ext ("SET_MEM", x, s)]
  let add (type a) (x : a) (s : a set) : a set = [%ext ("SET_ADD", x, s)]
  let remove (type a) (x : a) (s : a set) : a set = [%ext ("SET_REMOVE", x, s)]
  let update (type a) (x : a) (b : bool) (s : a set) = [%ext ("SET_UPDATE", x, b, s)]
  let iter (type a) (f : a -> unit) (s : a set) : unit = [%ext ("SET_ITER", f, s)]
  let fold (type a b) (f : b * a -> b) (s : a set) (i : b) : b = [%ext ("SET_FOLD", f, s, i)]
  let fold_desc (type a b) (f : a * b -> b) (s : a set) (i : b) : b = [%ext ("SET_FOLD_DESC", f, s, i)]
...
module List = struct
  ...
  let map (type a b) (f : a -> b) (xs : a list) : b list = [%ext ("LIST_MAP", f, xs)]
  let iter (type a) (f : a -> unit) (xs : a list): unit = [%ext ("LIST_ITER", f, xs)]
  let fold (type a b) (f : b * a -> b) (xs : a list) (i : b) : b = [%ext ("LIST_FOLD", f, xs, i)]
  let fold_left (type a b) (f : b * a -> b) (i : b) (xs : a list) : b = [%ext ("LIST_FOLD_LEFT", f, i, xs)]
  let fold_right (type a b) (f : a * b -> b) (xs : a list) (i : b) : b = [%ext ("LIST_FOLD_RIGHT", f, xs, i)]
  let cons (type a) (x : a) (xs : a list) : a list = [%ext ("CONS", x, xs)]
end
...
module Option = struct
  let unopt (type a) (v : a option) : a = [%ext ("UNOPT", v)]
  let unopt_with_error (type a) (v : a option) (s : string) : a = [%ext ("UNOPT_WITH_ERROR", v, s)]
  (* let map (type a b) (f : a -> b) (v : a option) : b option = [%ext ("OPTION_MAP", f, v)] *)
end
...
module Bytes = struct
  ...
  let unpack (type a) (b : bytes) : a option = [%ext ("BYTES_UNPACK", b)]
...
[@private]
  let true : bool = [%ext ("TRUE")]
[@private]
  let false : bool = [%ext ("FALSE")]
[@private]
  let unit : unit = [%ext ("UNIT")]
[@private]
