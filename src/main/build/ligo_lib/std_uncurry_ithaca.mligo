module Tezos = struct
  ...
  let get_contract_with_error (type a) ((a, s) : address * string) : a contract = [%ext ("CONTRACT_WITH_ERROR", a, s)]
  (* let get_entrypoint_opt (type a) ((s, a) : string * address) : (a contract) option = [%ext ("CONTRACT_ENTRYPOINT_OPT", s, a)] *)
  [@thunk]
  ...
  (* let create_contract (type a b) ((c, kh, mu, s) : (a * b -> operation list * b) * key_hash * tez * b) : operation * address = [%ext ("CREATE_CONTRACT", c, kh, mu, s)] *)
  ...
  let open_chest ((ck, c, n) : chest_key * chest * nat) : chest_opening_result = [%ext ("OPEN_CHEST", ck, c, n)]
  let call_view (type a b) ((s, x, a) : string * a * address)  : b option = [%ext ("VIEW", s, x, a)]
  let constant (type a) (s : string) : a = [%ext ("GLOBAL_CONSTANT", s)]
  let transaction (type a) ((a, mu, c) : a * tez * a contract) : operation = [%ext ("CALL", a, mu, c)]
  let set_delegate (o : key_hash option) : operation = [%ext ("SET_DELEGATE", o)]
end

module Bitwise = struct
  (* let and (type a b) ((l, r) : (a, b)) : (a, b) external_and = [%ext ("AND", l, r)] *)
  let xor ((l, r) : nat * nat) : nat = [%ext ("XOR", l, r)]
  (* let or ((l, r) : nat * nat) : nat = [%ext ("OR", l, r)] *)
  let shift_left ((l, r) : nat * nat) : nat = [%ext ("LSL", l, r)]
  let shift_right ((l, r) : nat * nat) : nat = [%ext ("LSR", l, r)]
end
...
module Big_map = struct
  ...
  let add (type k v) ((k, v, m) : k * v * (k, v) big_map) : (k, v) big_map = [%ext ("MAP_ADD", k, v, m)]
  let remove (type k v) ((k, m) : k * (k, v) big_map) : (k, v) big_map = [%ext ("MAP_REMOVE", k, m)]
  let update (type k v) ((k, v, m) : k * v option * (k, v) big_map) : (k, v) big_map = [%ext ("MAP_UPDATE", k, v, m)]
  let get_and_update (type k v) ((k, v, m) : k * v option * (k, v) big_map) : v option * (k, v) big_map = [%ext ("BIG_MAP_GET_AND_UPDATE", k, v, m)]
  let find_opt (type k v) ((k, m) : k * (k, v) big_map) : v option = [%ext ("MAP_FIND_OPT", k, m)]
  let find (type k v) ((k, m) : k * (k, v) big_map) : v = [%ext ("MAP_FIND", k, m)]
end
...
module Map = struct
  ...
  let add (type k v) ((k, v, m) : k * v * (k, v) map) : (k, v) map = [%ext ("MAP_ADD", k, v, m)]
  let remove (type k v) ((k, m) : k * (k, v) map) : (k, v) map = [%ext ("MAP_REMOVE", k, m)]
  let update (type k v) ((k, v, m) : k * v option * (k, v) map) : (k, v) map = [%ext ("MAP_UPDATE", k, v, m)]
  let get_and_update (type k v) ((k, v, m) : k * v option * (k, v) map) : v option * (k, v) map = [%ext ("MAP_GET_AND_UPDATE", k, v, m)]
  let find_opt (type k v) ((k, m) : k * (k, v) map) : v option = [%ext ("MAP_FIND_OPT", k, m)]
  let find (type k v) ((k, m) : k * (k, v) map) : v = [%ext ("MAP_FIND", k, m)]
  let iter (type k v) ((f, m) : (k * v -> unit) * (k, v) map) : unit = [%ext ("MAP_ITER", f, m)]
  let map (type k v w) ((f, m) : (k * v -> w) * (k, v) map) : (k, w) map = [%ext ("MAP_MAP", f, m)]
  let fold (type k v c) ((f, m, i) : (c * (k * v) -> c) * (k, v) map * c) : c = [%ext ("MAP_FOLD", f, m, i)]
end

module Set = struct
  let empty (type a) : a set = [%ext ("SET_EMPTY")]
  let mem (type a) ((x, s) : a * a set) : bool = [%ext ("SET_MEM", x, s)]
  let add (type a) ((x, s) : a * a set) : a set = [%ext ("SET_ADD", x, s)]
  let remove (type a) ((x, s) : a * a set) : a set = [%ext ("SET_REMOVE", x, s)]
  let update (type a) ((x, b, s) : a * bool * a set) = [%ext ("SET_UPDATE", x, b, s)]
  let iter (type a) ((f, s) : (a -> unit) * a set) : unit = [%ext ("SET_ITER", f, s)]
  let fold (type a b) ((f, s, i) : (b * a -> b) * a set * b) : b = [%ext ("SET_FOLD", f, s, i)]
  let fold_desc (type a b) ((f, s, i) : (a * b -> b) * a set * b) : b = [%ext ("SET_FOLD_DESC", f, s, i)]
...
module List = struct
  ...
  let map (type a b) ((f, xs) : (a -> b) * a list) : b list = [%ext ("LIST_MAP", f, xs)]
  let iter (type a) ((f, xs) : (a -> unit) * a list): unit = [%ext ("LIST_ITER", f, xs)]
  let fold (type a b) ((f, xs, i) : (b * a -> b) * a list * b) : b = [%ext ("LIST_FOLD", f, xs, i)]
  let fold_left (type a b) ((f, i, xs) : (b * a -> b) * b * a list) : b = [%ext ("LIST_FOLD_LEFT", f, i, xs)]
  let fold_right (type a b) ((f, xs, i) : (a * b -> b) * a list * b) : b = [%ext ("LIST_FOLD_RIGHT", f, xs, i)]
  let cons (type a) ((x, xs) : a * a list) : a list = [%ext ("CONS", x, xs)]
end
...
module Option = struct
  ...
  let unopt_with_error (type a) ((v, s) : (a option) * string) : a = [%ext ("UNOPT_WITH_ERROR", v, s)]
  (* let map (type a b) ((f, v) : (a -> b) * (a option)) : b option = [%ext ("OPTION_MAP", f, v)] *)
end
