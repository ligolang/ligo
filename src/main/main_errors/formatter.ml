...
let rec error_ppformat : display_format:string display_format ->
  ...
  | Human_readable | Dev -> (
    ...
    | `Main_view_not_a_function var -> Format.fprintf f "@[<hv>Invalid command line argument. @.View \"%a\" is not a function.@]" Stage_common.ValueVar.pp var
    ...
    | `Main_interpret_literal (loc,l) ->
      Format.fprintf f "@[<hv>%a@.Invalid interpretation of literal: %a@]"
        Snippet.pp loc
        Stage_common.Literal_value.pp l
