    ...
    format_result ~warning_as_error ~display_format (Formatter.Michelson_formatter.michelson_format michelson_format []) get_warnings @@
      fun ~raise ->
        ...
        let _contract : Mini_c.meta Run.Michelson.michelson =
          let aggregated_contract = Ligo_compile.Of_typed.apply_to_entrypoint_contract ~raise ~options app_typed_prg entry_point in
        ...
        let aggregated_param = Ligo_compile.Of_typed.compile_expression_in_context ~raise ~options typed_param aggregated_prg in
