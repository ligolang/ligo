...
let contract_template_url_map = Map.of_alist_exn (module String) [
...
let library_template_url_map = Map.of_alist_exn (module String) [
...
let new_project' ~(raise : (Main_errors.all, Main_warnings.all) Trace.raise) ~kind ~template ~project_name ~version = 
  ...
  let _ = (if is_release then
    let _ = Cli_helpers.run_command (Constants.git_checkout ~ref:version ~dir_path:project_name) in ()
  else
    ()) in
  let _ = OS.Dir.delete ~recurse:true @@ Fpath.(v "." / project_name / ".git")
  in [project_name]
