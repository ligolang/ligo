...
let measure_contract (raw_options : Compiler_options.raw) source_file display_format () =
    ...
    format_result ~werror:raw_options.warning_as_error ~display_format Formatter.contract_size_format get_warnings @@
      fun ~raise ->
      ...
      let michelson,e =  Build.build_contract ~raise ~add_warning ~options raw_options.syntax raw_options.entry_point source_file in
      let views = Build.build_views ~raise ~add_warning ~options raw_options.syntax raw_options.entry_point (raw_options.views,e) source_file in
...
let get_scope (raw_options : Compiler_options.raw) source_file display_format () =
    ...
    format_result ~display_format Scopes.Formatter.scope_format get_warnings @@
      fun ~raise ->
      let protocol_version = Helpers.protocol_to_variant ~raise raw_options.protocol_version in
      (* TODO libraries via raw options *)
      ...
      Scopes.scopes ~options:options.middle_end ~with_types:raw_options.with_types core_prg
