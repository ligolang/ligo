...
let measure_contract (raw_options : Compiler_options.raw) source_file display_format () =
    ...
    format_result ~warning_as_error ~display_format Formatter.contract_size_format get_warnings @@
      ...
      let contract = Compile.Of_michelson.build_contract ~raise ~add_warning ~enable_typed_opt:options.backend.enable_typed_opt ~has_env_comments:options.backend.has_env_comments ~protocol_version michelson views in
