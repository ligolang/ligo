...
let measure_contract source_file entry_point declared_views syntax protocol_version display_format enable_typed_opt werror project_root () =
    ...
    format_result ~werror ~display_format Formatter.contract_size_format get_warnings @@
      ...
      let contract = Compile.Of_michelson.build_contract ~raise ~options ~enable_typed_opt michelson views in
