...
let pretty_print (raw_options : RawOptions.t) source_file display_format () =
...
let dependency_graph (raw_options : RawOptions.t) source_file display_format () =
...
let preprocess (raw_options : RawOptions.t) source_file display_format () =
...
let cst (raw_options : RawOptions.t) source_file display_format () =
    format_result ~display_format (Parsing.Formatter.ppx_format) @@
      fun ~raise ->
      let syntax  = Syntax.of_string_opt ~raise (Syntax_name raw_options.syntax) (Some source_file) in
      let options = Compiler_options.make ~raw_options ~syntax () in
...
let ast (raw_options : RawOptions.t) source_file display_format () =
...
let ast_sugar (raw_options : RawOptions.t) source_file display_format () =
...
let ast_core (raw_options : RawOptions.t) source_file display_format () =
...
let ast_typed (raw_options : RawOptions.t) source_file display_format () =
...
let ast_aggregated (raw_options : RawOptions.t) source_file display_format () =
...
let mini_c (raw_options : RawOptions.t) source_file display_format optimize () =
