...
let mini_c (raw_options : Compiler_options.raw) source_file display_format optimize () =
    ...
    fun ~raise ->
      ...
      match optimize with
        | None ->
          let expr = Compile.Of_typed.compile_expression_in_context ~raise ~options (Ast_typed.e_a_unit ()) aggregated in
        ...
        | Some entry_point ->
          let expr = Compile.Of_typed.apply_to_entrypoint ~raise ~options typed entry_point in
          let mini_c = Compile.Of_aggregated.compile_expression ~raise expr in
