...
let pretty_print ~raise ~options:_ ~meta file_path =
  let buffer = buffer_from_path file_path in
  Of_c_unit.pretty_print ~raise ~meta buffer file_path

let pretty_print_cst ~raise ~options:_ ~meta file_path =
  let buffer = buffer_from_path file_path in
