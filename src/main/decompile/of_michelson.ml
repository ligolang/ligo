...
let decompile_value func_or_expr program entry (ty, value) =
  ...
  let%bind core      = trace inference_tracer @@ Inference.Untyper.untype_expression inferred in
  ok @@ core
...
let decompile_expression type_value runned_result =
  ...
  | Success (ty, value) ->
    ...
    let%bind decompiled_value = trace decompile_typed @@ Checking.untype_expression typed in
    ok (Success decompiled_value)
