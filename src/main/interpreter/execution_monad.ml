...
module Command = struct
  type 'a tezos_command = 
    | Set_big_map : Z.t * (LT.value * LT.value) list * Ast_aggregated.type_expression -> unit tezos_command
    | Unpack : Location.t * bytes * Ast_aggregated.type_expression -> LT.value tezos_command
    | Bootstrap_contract : int * LT.value * LT.value * Ast_aggregated.type_expression  -> unit tezos_command
    | Nth_bootstrap_contract : int -> Tezos_protocol.Protocol.Alpha_context.Contract.t tezos_command
    | Nth_bootstrap_typed_address : Location.t * int -> (Tezos_protocol.Protocol.Alpha_context.Contract.t * Ast_aggregated.type_expression * Ast_aggregated.type_expression) tezos_command
    | Reset_state : Location.t * Z.t option * LT.calltrace * LT.value * LT.value -> unit tezos_command
    ...
    | Get_storage : Location.t * Ligo_interpreter.Types.calltrace * LT.value * Ast_aggregated.type_expression -> LT.value tezos_command
    | Get_storage_of_address : Location.t * Ligo_interpreter.Types.calltrace * LT.value -> LT.value tezos_command
    | Get_size : LT.value -> LT.value tezos_command
    | Get_balance : Location.t * Ligo_interpreter.Types.calltrace * LT.value -> LT.value tezos_command
    | Get_last_originations : unit -> LT.value tezos_command
    | Get_last_events : string * LT.type_expression -> LT.value tezos_command
    | Compile_contract_from_file : string * string * string list * Z.t option -> LT.value tezos_command
    | Read_contract_from_file : Location.t * LT.calltrace * string -> LT.value tezos_command
    | Run : Location.t * LT.func_val * LT.value -> LT.value tezos_command
    | Eval : Location.t * LT.value * Ast_aggregated.type_expression -> LT.value tezos_command
    | Run_Michelson : Location.t * LT.calltrace * (execution_trace, string) Tezos_micheline.Micheline.node * Ast_aggregated.type_expression * LT.value * Ast_aggregated.type_expression -> LT.value tezos_command
    ...
    | Decompile : LT.mcode * LT.mcode * Ast_aggregated.type_expression -> LT.value tezos_command
    | To_contract : Location.t * LT.value * string option * Ast_aggregated.type_expression -> LT.value tezos_command
    | Check_storage_address : Location.t * Tezos_protocol.Protocol.Alpha_context.Contract.t * Ast_aggregated.type_expression -> unit tezos_command
    | Inject_script : Location.t * Ligo_interpreter.Types.calltrace * LT.value * LT.value * Z.t -> LT.value tezos_command
    | Set_source : LT.value -> unit tezos_command
    | Set_baker : Location.t * LT.calltrace * LT.value -> unit tezos_command
    | Get_voting_power : Location.t * Ligo_interpreter.Types.calltrace * Tezos_protocol.Protocol.Alpha_context.public_key_hash -> LT.value tezos_command
    | Get_total_voting_power : Location.t * Ligo_interpreter.Types.calltrace -> LT.value tezos_command
    ...
    | Add_cast : Location.t * LT.mcontract * Ast_aggregated.type_expression -> unit tezos_command
    | Michelson_equal : Location.t * LT.value * LT.value -> bool tezos_command
    | Implicit_account : Tezos_protocol.Protocol.Alpha_context.public_key_hash -> LT.value tezos_command
    | Contract : Location.t * LT.calltrace * LT.mcontract * string option * Ast_aggregated.type_expression -> LT.value tezos_command
    | Pairing_check : (Bls12_381.G1.t * Bls12_381.G2.t) list -> LT.value tezos_command
    | Add_account : Location.t * LT.calltrace * string * Tezos_protocol.Protocol.Alpha_context.public_key -> unit tezos_command
    | New_account : unit -> LT.value tezos_command
    | Baker_account : LT.value * LT.value -> unit tezos_command
    | Register_delegate : Location.t * Ligo_interpreter.Types.calltrace *  Tezos_protocol.Protocol.Alpha_context.public_key_hash -> LT.value tezos_command
    | Bake_until_n_cycle_end : Location.t * Ligo_interpreter.Types.calltrace *  Z.t -> LT.value tezos_command
    | Register_constant : Location.t * Ligo_interpreter.Types.calltrace * LT.mcode -> string tezos_command
    | Constant_to_Michelson : Location.t * Ligo_interpreter.Types.calltrace * string -> LT.mcode tezos_command
    | Register_file_constants : Location.t * Ligo_interpreter.Types.calltrace * string -> LT.value tezos_command
  ...
  let eval_tezos
    ...
    | Compile_contract (loc, v) ->
       let ast_aggregated = match v with
         | LT.V_Func_val { arg_binder ; arg_mut_flag = Immutable ; body ; orig_lambda ; env ; rec_name } ->
