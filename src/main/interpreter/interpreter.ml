...
let rec pattern_env_extend_ ~(attributes : ValueAttr.t) ~(mut : bool)
 ...
 fun (locs,env) pattern ty value ->
  ...
  | P_variant (label, p), V_Construct (label', value) ->
    if not (Label.equal label (Label label')) then fail @@ error_type () else
    ...
    let* (body, env) =
      ...
      let rec aux cases =
        ...
        | AST.Match_expr.{pattern ; body }::tl ->
          try_or (
              let* env = pattern_env_extend ~attributes:ValueAttr.default_attributes env pattern matchee.type_expression matchee' in
              return (body,env)
            )
            (aux tl)
      in
...
let eval_test ~raise ~steps ~options
 ...
 fun prg ->
  ...
  let aux decl r =
    ...
    | Ast_typed.D_irrefutable_match  { pattern = { wrap_content = P_var binder ; _ }; expr; _ }
