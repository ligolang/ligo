...
let scopes : Format.formatter -> scopes -> unit = fun f s ->
  let pp_scope f (s:scope) =
    ...
    let a = Def_map.to_kv_list_rev s.env in
...
let definitions : Format.formatter -> def_map -> unit = fun f dm ->
  let kvl = Def_map.to_kv_list_rev dm in
...
let defs_json d : Yojson.Safe.t =
  let get_defs d = 
    let (v,tv) = List.partition (fun (_,def) -> match def with Variable _ -> true | Type _ -> false) (Def_map.to_kv_list_rev d) in
    [ 
...
let scopes_json s : Yojson.Safe.t = `List (
  List.map
    (fun scope ->
      let sd = Def_map.to_kv_list_rev scope.env in
