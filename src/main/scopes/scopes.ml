...
let scopes : with_types:bool -> tenv -> tstate -> Ast_core.program -> ((def_map * scopes), Main_errors.all) result = fun ~with_types env state core_prg ->
  ...
  let rec find_scopes' = fun (i,all_defs,env,scopes,lastloc) (e : Ast_core.expression) ->
    ...
    | E_lambda { binder={var={wrap_content=fun_name;location=fn_loc};ascr=input_type} ; output_type = _ ; result } -> (
      ...
      | Match_variant lst -> (
        let aux = fun (i,all_defs,scopes) ({constructor;proj;body}:Ast_core.match_variant) ->
          let proj_f = fun (t:Ast_typed.type_expression) -> match Ast_typed.get_t_sum t with
            | Some t -> (Ast_typed.LMap.find constructor t.content).associated_type
