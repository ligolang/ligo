...
(** The specification for a single option is a tuple [(short_form,
   long_form, action, handler)] where: - [short_form] is the character
   for the short form of the option without the leading - (or
   [noshort='\000'] if the option does not have a short form)

   - [long_form] is the string for the long form of the option without
   the leading -- (or [nolong=""] if no long form)
   ...
   - [(Some _, Some _)] : the option may have an argument; the short
     form can't be concatenated with other options (even if the user
     does not want to provide an argument). The behaviour
     (handler/action) is determined by the presence of the argument.

   - [(Some _, None)] : the option must not have an argument; the
     short form, if it exists, may be concatenated

   - [(None, Some _)] : the option must have an argument; the short
     form can't be concatenated
...
val parse :
  opt list -> (string -> unit) -> string array -> int -> int -> unit
(**
...
(** @return an action that sets a reference with a given value *)
...
(** @return a handler that appends the argument to the end of a [string list]
   reference *)

val only_once : string ref -> string -> (string -> unit) option
(** @return a handler that stores the argument in a [string] reference if
    it is empty, raises an exception [Error string] *)
