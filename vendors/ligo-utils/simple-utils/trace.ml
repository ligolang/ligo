...
type nonrec ('value, 'error) result = ('value * 'error list, 'error * 'error list) result
...
let update_annotation w = function
    Error (e, a) -> Error (e, w :: a)
  | Ok (x, a) -> Ok (x, w :: a)
...
let trace tracer v = match v with
  | Ok (v', a) -> Ok (v', List.map tracer a)
  | Error (err, a) -> Error (tracer err, List.map tracer a)
