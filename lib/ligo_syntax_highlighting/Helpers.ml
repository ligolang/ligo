...
let macro : Core.pattern =
  { name = "macro"
  ; kind = Match { match_ = [ Regexp.macro_match, None ]; match_name = Some PreProc }
  }
  ...
  ; { name = "string"
    ; kind =
        Begin_end
          ...
          ; patterns = [ Name_ref "string_specialchar" ]
          }
    }
  ]
...
let c_comment : Core.pattern list =
  [ { name = "line_comment"
    ; kind =
        Match
          { match_name = Some Comment; match_ = [ Regexp.c_line_comment_match, None ] }
    }
  ; { name = "block_comment"
    ; kind =
        Begin_end
...
let numeric_literals : Core.pattern =
  ...
  ; kind =
      Match { match_name = Some Number; match_ = [ Regexp.numeric_literals_match, None ] }
  }

let attribute : Core.pattern =
  { name = "attribute"
  ; kind =
      Match { match_name = Some Attribute; match_ = [ Regexp.attributes_match, None ] }
  }
