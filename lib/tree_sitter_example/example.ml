...
let () =
  ...
  let expr_stmt_node : ts_tree =
    TS_fun.ts_node_named_child program_node (UInt32.of_int 0)
  in
  let array_node : ts_tree =
    TS_fun.ts_node_named_child expr_stmt_node (UInt32.of_int 0)
  in
  let number_node : ts_tree = TS_fun.ts_node_named_child array_node (UInt32.of_int 0) in
  let null_node : ts_tree = TS_fun.ts_node_named_child array_node (UInt32.of_int 1) in
