...
let read_prod_rule type_ named json : rule =
  ...
  let fields =
    try J.(member "fields" json |> to_assoc |> mk_fields) with
      J.Type_error _ -> []
  and children = J.(member "children" json |> to_option mk_children)
  in Prod {type_; named; fields; children}

let mk_subtype type_ named subtypes : rule =
  match J.to_list subtypes with
    [] -> empty_member "subtypes" subtypes
  | subtype :: subtypes ->
      Union {type_; named; subtypes = mk_types subtype subtypes}
...
let read_abs_gram (path: string) : (abs_gram, (error * json)) Result.t =
  try
    ...
    in Ok (List.map ~f:mk_rule list)
  with J.Type_error (msg, json) -> Error (msg, json)
     | Yojson.Json_error msg -> Error (msg, `Null)

(* Printing TS abstract grammar *)

let print_type state type_ =
  Tree.(make_unary state "type" make_node type_)

let print_bool state bool =
  Tree.make_node state (string_of_bool bool)

let print_named state named =
  Tree.make_unary state "named" print_bool named
...
let print_type_named state type_named =
  ...
  in Tree.make state "┐" children

let print_types_or_sub state root subtypes =
  let children =
    Nonempty_list.to_list subtypes
    |> List.map ~f:(Tree.mk_child print_type_named)
  in Tree.make state root children
...
let print_Union state rule =
  ...
  in Tree.make state "Union" children

let print_multiple state multiple =
  Tree.make_unary state "multiple" print_bool multiple

let print_required state required =
  Tree.make_unary state "require" print_bool required
...
let print_field state (key, children) =
  ...
  in Tree.make state key children

let print_fields state fields =
  let children = List.map ~f:(Tree.mk_child print_field) fields
  in Tree.make state "fields" children

let print_children state children =
  print_field state ("children", children)
...
let print_Prod state rule =
  ...
  in Tree.make state "Prod" children'

let print_rule state = function
  Union rule -> print_Union state rule
| Prod  rule -> print_Prod  state rule

let print_abs_gram state abs_gram =
  Tree.of_list state "<abs_gram>" print_rule abs_gram

let print_abs_gram_to_buffer state abs_gram : Buffer.t =
  print_abs_gram state abs_gram; Tree.to_buffer state
...
let () =
  match read_abs_gram "node-types.json" with
    Ok abs_gram ->
      ...
      let state = Tree.mk_state ~buffer ~regions:false ~layout:true
                                ~offsets:true `Point in
      let abs_gram = print_abs_gram_to_string state abs_gram
      in Printf.printf "%s\n%!"abs_gram
  | Error (msg, json) ->
      let str = Yojson.Basic.pretty_to_string json
