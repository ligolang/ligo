...
open Core
module Array  = Stdlib.Array (* Generated by ocamllex *)
...
module Make (Config : Config.S) (Options : Options.S) =
  struct
    ...
    let fail = E_Lexer.fail (* Raise exception [PreError.Error] *)
    and stop = E_Lexer.stop (* Raise exception [PreError.Error] *)
      ...
      | Ok (state, incl_directive, _, _) -> (* Directive dropped. *)
          ...
          if state#is_copy then
            ...
            (* We try to find the file to include. If missing, the
               ...
               [PreError.File_not_found]. Otherwise, we obtain a triple
            ...
            (* We check if the current file exists in the stack of ancestors
               in which case we fail with the error [PreError.Cyclic_inclusion]
            *)
            let () =
              if List.exists ~f:(String.equal state#pos#file) state#ancestors
              then
                fail state incl_region
                  (PreError.Cyclic_inclusion (state#ancestors, state#pos#file))
            in
        ...
        if state#is_copy then
          ...
          let import_path =
            match find state#incl import_file external_dirs with
              None -> PreError.File_not_found import_file
      ...
      let state =
        if state#is_copy then
          if String.(value = "") then
            fail state dir_region (PreError.Error_directive value)
          else
            fail state region (PreError.Error_directive value)
        else state
      in callback state lexbuf
  | Error (region, error) -> fail state region error
...
let linemarker_action ~callback region linenum state lexbuf =
  ...
  match Directive.scan_linemarker hash_pos linenum state lexbuf with
    Ok (state, _line_directive, dir, _) ->
...
rule scan state = parse
  ...
  nl? eof { if List.is_empty state#trace then state
              ...
              fail state region PreError.Missing_endif }
...
| block_comment_opening {
    state#copy lexbuf;
    let state, Region.{region = _; value} = state#sync lexbuf in
    let lexeme = value in
...
| nl  { state#copy lexbuf;
        let state = fst @@ state#newline lexbuf
        in in_block block opening state lexbuf }
| eof { PreError.Unterminated_comment block#closing
        |> fail state opening }
| _   { state#copy lexbuf;
...
| nl  { fail state opening.Region.region PreError.Newline_in_string }
...
| ['\000' - '\031'] | ['\128' - '\255'] as c
      ...
      { stop state lexbuf (PreError.Invalid_character_in_string c) }
| "\\\"" | "\\\\" | _
      { state#copy lexbuf;
...
(* START OF TRAILER *)
    ...
    let init_env =
      let f env symbol = Set.add env symbol
    ...
    let from_lexbuf state lexbuf =
      ...
      | exception PreError.Error (buffer, msg) ->
    ...
    let from_file file =
      ...
      with Sys_error msg ->
        let error  = PreError.Failed_opening (file, msg) in
        let msg    = PreError.to_string error in
